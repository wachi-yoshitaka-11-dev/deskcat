...
| 再現可能なbuild／flash／monitor command | Partial | **buildは検証済み。**確定commandは[検証済みコマンド](../toolchains/verified-commands.md)にある（[ADR-0018](../decisions/0018-instruction-file-structure.md)で`AGENTS.md`とroot READMEから移した）。**flashとserial monitorは2026-08-20に#6で検証した**（`espflash flash --monitor`。記録は[Version Record](../toolchains/version-records/2026-08-20-esp32-flash-boot-native.md)）。**Piのbuild・lint・testは2026-08-26に検証済みであり、[検証済みコマンド](../toolchains/verified-commands.md)へ反映済みである。残るのはPiの実機試験（実serial port、ESP32との通信）とHILである** |
