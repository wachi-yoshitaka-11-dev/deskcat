...
| **B-1** | ESP32を単体でPCのUSBから給電し、flashingとADC loggingを行う | **実施条件つき。**制限として働くのはPC hostのUSB portのOCPだが、**その値はport実装に依存する**。使用するhost portと公表供給能力を`段階B-2の測定`の記録項目に従って記録する。ESP32単体の文献値（spike約500 mA）はUSB 2.0の公称0.5 Aに近く、余裕は大きくない | 不要 |
...
A、B-1、B-2は通常の使い方であり、**測れないgateにも追加購入にも依存しない**。B-1とB-2に
付く実施条件は、使用するhost portとその公表供給能力を記録することだけである
（`段階B-2の測定`）。#8（Pi Rust環境）と#40（ESP32 toolchain）はここに収まる。
**部品待ちにするのはCだけである。**
...
| 記録する項目 | 内容 |
|---|---|
| 使用するhost port | PCの機種名とport種別（USB 2.0／3.x、給電専用かの別）。**この記録が無い状態でB-1もB-2も行わない** |
...
| 経路 | いつ | 電流制限 | 排他制約 |
|---|---|---|---|
| **段階B-1／B-2: PC → ESP32のMicro USB** | 記録を済ませれば可能 | **使用するPC host portのOCP。値はport実装に依存するため、公表値を記録してから使う**（`段階B-2の測定`）。USB 2.0の公称は0.5 Aで、ESP32単体の文献値（spike約500 mA）に対する余裕は大きくない。B-2で足す周辺module3点も同じOCP下にある | 満たす（USBのみ。`5V` pinへ何も繋がない） |
...
揃う前にできるのは、`5 V ingress`節の段階A、段階B-1、段階B-2（Pi単体の起動、ESP32の
...
| 段階Cのgate部品の実装 | **合成給電の前に必須。**(1) 過電流保護部品を実装し、trip値を`過電流保護（段階Cのgate）`節へ記録する。(2) gate対象の大電流経路を`WIRE-PWR-01`で構成し、breadboard接点とジャンパー線を通さない。(3) `経路部品と定格`表に未確定の行を残さない | テスターの読みと手動停止は検知であって保護ではない（M-12001は3 Aを供給できる）。定格不明の部品が経路にある間は最小値が出せず、上限を判定できない |
