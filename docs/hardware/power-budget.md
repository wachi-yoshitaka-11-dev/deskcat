...
| **B-2** | B-1に周辺module3点（MSP2807、ADXL345、BME280）を足し、**ESP32の`3V3` pinから給電**して定常電流を測る。5 V railもPiも使わない | **gate必要。**boardの設計想定を超えて`3V3` pinから外部負荷を取るため、**`3V3` pinの電流定格と許容電圧範囲を確認し、かつ故障電流を制限する手段（board上regulatorの過電流／短絡保護の確認、または電流制限付きの外部3.3 V電源）が成立するまで実施しない**。停止条件と手順は`段階B-2の測定` | 不要 |
...
**B-2だけはgateが要る。**boardの設計想定を超えて`3V3` pinから外部負荷を取るため、
`3V3` pinの電流定格と許容電圧範囲を確認し、かつ**故障電流を制限する手段**が成立するまで
実施しない（`段階B-2の測定`）。**手段によっては外部電源の追加購入が要る。**
...
| `3V3` railの許容電圧範囲 | 周辺module3点の動作電圧範囲の**積集合**（ADXL345、BME280、MSP2807の各下限の最大値〜各上限の最小値。値は`hardware-bom.md`の各行）と、ESP32のlogic levelから決める。**確認できるまでB-2を実施しない** |
| regulatorの種別 | LDOかswitchingか。5 V側への換算根拠に使う（下記） |
| 許容する電流の上限 | 上記電流定格の80%（この文書のderating規則と揃える）。**定格そのものを上限にしない** |

###### 通常動作の上限は保護ではない

**上の電流上限と電圧範囲は、通常動作で守るべき値であって、短絡や配線ミスのときの
電流を制限しない。**上限を超えたら人が止めるという運用は、段階Cについて
`過電流保護（段階Cのgate）`で否定したものと同じである。**人による監視は保護機構ではない。**

したがってB-2は、**故障電流を制限する手段が次のどちらかで成立するまで実施しない**。

| 手段 | 確認すること |
|---|---|
| board上regulatorの過電流保護／短絡保護に依存する | その保護の有無と動作（制限電流、折り返し特性、熱shutdownの有無）を**一次資料または現物回路で確認する**。保護が無い、または確認できない場合はこの手段を採らない |
| 周辺module3点を外部の3.3 V電源から給電する | **電流制限値を設定できる電源**を使い、設定値を記録する。この場合`3V3` pinからは取らない |

**どちらも成立しない場合は、B-2を行わない。**MSP2807の実測を段階Cまで延ばし、
変換基板の選定根拠を別に立て直す。
...
| 項目 | 内容 |
|---|---|
| 停止条件（電流） | 3.3 V branchの定常電流が上記の上限（電流定格の80%）に達した時点で中止する |
| 停止条件（電圧） | `3V3` railの電圧が上記の許容電圧範囲を外れた時点で中止する。**下限割れはregulatorが電流を出しきれていない兆候**であり、電流計の読みが上限内でも中止する |
| 停止条件（その他） | ESP32のbrownout、reset、LCDの表示異常、regulator周辺の発熱を認めた場合も中止する |
| 停止手順 | **USB cableをhost側から抜く**（外部電源を使う場合はその出力を切る）。これは**停止手順であって保護機構ではない。**人が手を掛けられる状態で行い、無人で継続しない |
...
| **段階B-2: 同上＋`3V3` pinから周辺module3点** | `3V3`側のgateを満たしてから | 通常動作の上限は`3V3` pin電流定格の80%。**故障電流を制限するのはboard上regulatorの過電流／短絡保護（要確認）か、電流制限付きの外部3.3 V電源である。**PC hostのOCPは`3V3` branchを守らない（`段階B-2の測定`） | 満たす（USBのみ。`5V` pinへ何も繋がない） |
...
| 段階B-2の実施条件 | **(1) `3V3` pinの電流定格と許容電圧範囲を確認し、定常電流を定格の80%以下、rail電圧を範囲内に保つ。(2) 故障電流を制限する手段を成立させる**（board上regulatorの過電流／短絡保護を一次資料か現物回路で確認するか、電流制限値を設定した外部3.3 V電源から給電する）。**どちらも成立しない場合はB-2を実施せず、測定を段階Cまで延ばす** | PC hostのOCPはUSB入力を制限するだけで`3V3` branchを守らない。**先に壊れうるのはboard上のregulatorである。**通常動作の上限は短絡・配線ミス時の電流を制限せず、人による監視も保護機構ではない（`段階B-2の測定`） |
