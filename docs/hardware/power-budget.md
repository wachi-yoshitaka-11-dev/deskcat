...
| **B-2** | B-1に周辺module3点（MSP2807、ADXL345、BME280）を足し、**ESP32の`3V3` pinから給電**して定常電流を測る。5 V railもPiも使わない | gate不要。B-1と同じくPC hostのOCP下にある。**測定手段は`段階B-2の測定`を参照** | 不要 |
...
| 電流制限 | PC hostのUSB portのOCP。段階B-1と同じで、追加のgateも追加購入も要らない |
...
| servo rail低側のshunt抵抗 | 電力定格5 W／0.1 Ωから逆算して約7 A相当 | `hardware-bom.md` MEAS-01。servo戻り専用であり、logic側は通らない | Selected |
...
保護部品も経路上の部品であり、その**保持電流は`経路部品と定格`表の1行**である。
したがって上限の求め方は変わらない。

```text
上限 = min(経路部品の定格すべて。保護部品の保持電流を含む) × 0.8
```

保持電流の下限は**負荷側の見積もり**から、上限は**保護対象の部品定格**から決まる。
どちらもgate値を参照しないため、保護部品を先に選べる。選べば最小値が決まり、
gate値が決まる。「gateが決まらないと保護部品が選べない」という循環にはならない。
...
- [ ] **その最悪動作でservo railの定常電流が250 mA（`変換基板に必要な定格の見積もり`で
      ...
      可動域・速度・duty cycleを締めて再測定するか、ingressの定格を上げる判断へ戻す
...
| **合成給電時にservo railを通る定常電流** | **250 mA以下**（`変換基板に必要な定格の見積もり`で割り当てた予算）。承認範囲内の最悪動作で確認する | 予算を超えるとingressの見積もりが崩れる。強制点は`servo-safety-limits.md`の可動域・速度・duty cycleである |
