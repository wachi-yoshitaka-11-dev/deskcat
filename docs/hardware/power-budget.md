...
| 項目 | 照合できる段階 | gate |
|---|---|---|
| (a) Pi入力の最低電圧 | **段階Aで探索的に測るだけ**（Piを単体でアダプターから起動）。**合否判定はここでは行わない** | **測定にgateは不要。判定は`Blocked`**（判定値が`HW-TBD-028`(a)として未確定であり、`ADC-5V`の測定系も未実装） |
| (b) ESP32入力／3.3 Vの最低電圧 | **段階B-1**（ESP32を単体でPCのUSBから給電） | **不要。**ただし3.3 V rail側は周辺moduleを足す段階B-2の話であり、そちらはgateが要る |
| (c) 最大定常ripple | 段階A／B-1で各railを単独で、段階Cで合成後を測る。**両者を混ぜない** | 段階Cはgate必要 |
| (d) 最大transient droopと継続時間 | **段階C**（servo起動過渡を含む）。段階A／B-1では出ない | **必要** |
| (e) connector／wireの最大温度上昇 | **段階C**。`HW-TBD-021`／`HW-TBD-022`の部品が揃ってから | **必要** |

**段階AとB-1はgateも追加購入も要らない**ため、**測定そのものは値が決まる前でも進められる。**

**ただし「測れる」と「合否を判定できる」を混同しない。**(a)は判定に使う値が無く、`ADC-5V`の
測定系も未実装であるため、**段階Aで得た値から受け入れの合否を導かない**（`servo-safety-limits.md`の
受け入れchecklistでも同じ項目を`Blocked`としている）。(b)の3.3 V rail側も同様に未確定である。

**段階Aを実施してよいこと自体は変えていない。**変えたのは、そこで得た値を受け入れ判定に使わない
という点だけである。
