...
| **ESP32の5 V入力** | **未決定。**下記「ESP32の給電経路（案Aで確定。実測待ち）」で選択する |
...
**この1つで2つを同時に判定できる。**ケーブル抵抗による電圧降下と、PiのOTG port供給能力である。
どちらが原因でも、ESP32の入力電圧が下限を割る形で現れる。

| 項目 | 内容 |
|---|---|
| 判定量 | **ESP32入力電圧** |
| 下限 | **暫定4.6 V。確定値ではない** |
| 下限の出所 | 参照設計のdropout最大1.3 Vからの導出 |
| **下限が暫定である理由** | **現物の`U2`は刻印`LD1117AG`で、参照設計の`AMS1117`と異なり未確認である**（`HW-TBD-023`） |
| あわせて記録する量 | **ESP32の定常電流とpeak** |

**下限が確定するのは`HW-TBD-023`が解けてからである。**それまでこの判定を合格として扱わない。
**4.6 Vを満たしたという観測は記録してよいが、合否の判定には使わない。**
...
- [ ] **ESP32の給電経路のgateを実測で埋める**（`ESP32の給電経路（案Aで確定。実測待ち）`節）。**案の選択は2026-08-22に済んでいる**（案Aで確定。人間の判断）。**残るのは実測だけである。**PiのUSB OTG portからESP32＋3V3負荷を給電したときの電流とESP32入力電圧を実測し、ESP32のundervoltageとbrownoutが起きないことを確認する。**Pi側はundervoltage警告が出ないため、`ADC-5V`の実測で見る。ただし合否の判定は`HW-TBD-028`(a)が確定するまでできない**（`Pi Zero Wには低電圧検出が無い`）。**不足した場合に案Bへ切り替える**。そのとき秋月基板のVBUS保護diodeの有無を回路で確認してから`5V` pinとUSBを同時接続する
