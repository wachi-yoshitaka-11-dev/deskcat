...
| **手持ちの候補（購入不要）その2** | **2026-08-20に現物の写真でdigital storage oscilloscopeの所持が判明した**（[hardware-bom.md](hardware-bom.md)の`MEAS-05`）。筐体の表示は`DSO Shell`／`QUIMAT`、操作部は`V/DIV`／`SEC/DIV`／`TRIGGER`／`OK`と`ADJ`ノブ、入力は**BNC 1系統**である。**帯域・sample rate・入力範囲・channel数・確度はいずれも未取得である。**したがって**上記(1)（2 ch以上）を満たすかは判定できない。**入力が1系統である点は、`ADC-5V`と`ADC-3V3`を同時に見られない可能性を示す。**電源入力の種別（DC jack／USB／電池）も未確認であり、独立給電の条件を満たすかは判定できない。****通電していない。**確認項目が埋まるまで採否を判断しない（[AGENTS.md](../../AGENTS.md) 推測禁止） |
...
**2026-08-21時点で2は解けた。1は大きく進んだが未解決である**（AV<sub>CC</sub>を代理値で測っており、`5V` pinと`AVCC` pinの差が未評価である）。**3は未着手である。**
