...
| **B-2** | B-1に周辺module3点（MSP2807、ADXL345、BME280）を足し、**ESP32の`3V3` pinから給電**して定常電流を測る。5 V railもPiも使わない | **gate必要。**給電元により**B-2a**（`3V3` pinから。board上regulatorの定格と過電流／短絡保護の確認が条件）と**B-2b**（外部の電流制限付き3.3 V電源から。設定値の根拠と上限が条件）に分かれる。**どちらの条件も満たせない場合は実施しない。**経路ごとの実施条件・測定点・停止条件は`段階B-2の測定` | 不要 |
...
| 電流制限値の上限 | 上げてよい上限は、**判明している中で最も低い定格**とする。周辺module3点のうち定格が公開されているものの値と、給電に使う配線の許容電流（`WIRE-PWR-01`と同じく公開値のある品を使う）の小さい方。**MSP2807の定格は未公開のため、これを根拠にできない。**上限まで上げても正常動作しない場合はB-2bを成立させられない |
...
| MSP2807（LCD＋backlight＋touch） | **未確認。段階B-2（3.3 V側で定常電流を測る）で実測できる。ここが唯一の未知数である。**ただしB-2の実施にはB-2a（`3V3` pinの定格とregulator保護の確認）かB-2b（電流制限値の根拠と上限）のどちらかの条件が要り、5 V側へ足すにはregulatorの種別確認も要る（`段階B-2の測定`）。**どちらの条件も満たせない場合、この行は段階Cまで実測待ちのまま残る** | **実測待ち** |
...
| 段階B-2の実施条件 | **共通:** 許容電圧範囲（周辺module3点の動作電圧範囲の積集合とESP32のlogic level）を確認する。**B-2a（`3V3` pin給電）:** `3V3` pinの電流定格を確認し定常電流をその80%以下に保つ。かつ**board上regulatorの過電流／短絡保護を一次資料か現物回路で確認する。****B-2b（外部3.3 V電源給電）:** 電流制限値を設定できる電源を使い、設定値の根拠（低い値から段階的に上げる）と上限（判明している中で最も低い定格）を記録する。`3V3` pinは使わない。**どちらも成立しない場合はB-2を実施せず、測定を段階Cまで延ばす** | PC hostのOCPはUSB入力を制限するだけで`3V3` branchを守らない。**先に壊れうるのはboard上のregulatorである。**通常動作の上限は短絡・配線ミス時の電流を制限せず、人による監視も保護機構ではない。**B-2aとB-2bでは守るべき対象も保護手段も違うため、片方の条件をもう片方へ流用しない**（`段階B-2の測定`） |
