...
**下表は段階C（合成給電）の構成についてのものである。**段階A・B-1・B-2にはbreadboard railが
無く、M-12001はPiの`PWR IN`へ、ESP32はUSBへ直挿しするため、**そもそも極性を間違えられない**
（`合成給電を部品が揃うまで行わない理由`）。**段階B-2で周辺module3点へ引く配線だけが例外である。**
