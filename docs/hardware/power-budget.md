...
| **手持ちの候補（購入不要）その2** | **2026-08-20に現物の写真でdigital storage oscilloscopeの所持が判明した**（[hardware-bom.md](hardware-bom.md)の`MEAS-05`）。筐体の表示は`DSO Shell`／`QUIMAT`、操作部は`V/DIV`／`SEC/DIV`／`TRIGGER`／`OK`と`ADJ`ノブ、入力は**BNC 1系統**である。**2026-08-21に製造元の公式manualを取得し、仕様を確定した**（値の正は[hardware-bom.md](hardware-bom.md)の`MEAS-05`。**ここへ再掲しない**）。**channel数は1と確定したため、上記(1)（2 ch以上）を満たさない。**`ADC-5V`と`ADC-3V3`を同時に見られない。**これは判定できないのではなく、満たさないと確定した。**帯域・sample rate・入力範囲・記録長も同manualで確定した。**`Record length`が固定である点が採否に効く**（`2つの手持ち候補は代替ではなく相補である（2026-08-21の材料）`）。**垂直確度は同manualに記載が無く、未取得のままである。****電源は外部のDC入力で、PCにも測定対象にも依存しない。**適合する電源も手元にあり、plugの実物適合も確認した。**したがって独立給電の条件は満たす**（[hardware-bom.md](hardware-bom.md)の`MEAS-05`）。**ただし「独立した観測」の成立は別である**（同じ行の`その1`と同じ扱いにする）。独立給電は必要条件の1つにすぎず、**GND topology、入力headroom、実機での動作は別に検証が要る。****この品ではそのいずれも検証していない。****channel数が1で(1)を満たさないことも変わらない。****2026-08-21に初回通電し、画面が表示されることを確認した**（異音・発熱・異臭なし。人間が基板のそばで観察）。**BNCには何も接続していない。**確認したのは電源が入り画面が出ることまでで、**波形の取得も垂直確度の確認もしていない。**したがって**測定器として使えることは依然として確認できていない。**確認項目が埋まるまで採否を判断しない（[AGENTS.md](../../AGENTS.md) 推測禁止） |
...
- **DSO Shellは連続observationができない。**1024 pointsで固定であるため、
...
**2026-08-22時点で2は解けた。1は大きく進んだが未解決である**（AV<sub>CC</sub>を代理値で測っており、`5V` pinと`AVCC` pinの差が未評価である）。**3は段階B-1の範囲では解けたが、star point構成の確認が残るため項目としては完了しない。**
**ただし3が解けても`HW-TBD-034`はcloseしない。**この3点はclose条件の一部であって全数ではない
（全数は[tbd-register.md](tbd-register.md)の`HW-TBD-034`の`必要な根拠`欄が持つ）。

| # | 未解決 | 状態（2026-08-21時点） | 解けないと何が起きるか | どこで解くか |
|---|---|---|---|---|
...
|  3 | **GND topologyへの影響** | **段階B-1の範囲では解けた**（2026-08-22。未加工の観測は[experiment-log.md](experiment-log.md)の`EXP-003`）。**ただし項目としては完了しない。**star point構成そのものの確認が残り、それは変換基板`PSU-INGRESS-01`が揃うまでできない（`方式1の記録係をこのtopologyへ足すとき（HW-TBD-034 作業3）`）。**段階B-1で実測できたのは3項目**（ADC値の妥当性、GNDの結合の影響、入力headroom）**である。****異常は無かった**（異音・発熱・reset・brownoutのいずれも無し。人間が基板のそばで観察） | star point構成を崩すと**全ADC値がずれる。**PCのGNDが経路へ入る影響も未評価 | `GND topology（測定前に必ず確定させる）`と、`電源過渡の測定方式のdesign review`の方式1の表にある`手持ち候補に固有の論点`の行  |
