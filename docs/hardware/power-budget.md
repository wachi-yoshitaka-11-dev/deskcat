...
**どちらも未実施であるため、(1)は未解決のままである。**
**ただし未知の中身は変わった。**「差が未評価」から「**特定された1部品のDCRが未取得**」になった。
...
**2026-08-22時点で2は解けた。1は大きく進んだが未解決である**（AV<sub>CC</sub>を代理値で測っており、`5V` pinと`AVCC` pinの差が未評価である）。**3は段階B-1で実測はできたが未解決である**（GNDの確認が片側だけであり、star point構成の確認も残る）。
...
|---|---|---|---|---|
| 1 | **基準電圧の校正** | **大きく進んだが未解決**（2026-08-21）。比率法で比を実測し、**AV<sub>CC</sub>の代理値**（`5V` pin）を実測して周囲温度を記録した。得られた`V_INT`は**代理値に基づく推定であって絶対値ではない。****`5V` pinとATmega328Pの`AVCC` pinは同一nodeではなく、その差を測ってもいないし上限も与えていない。**差が未評価である以上、**絶対電圧としての`V_INT`を主張できず、この項目を解決済みにできない**（[PR #158](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/158)の自動reviewの指摘。**限定を書いたうえで解決済みにするのは、限定を書いていないのと同じである**）。**解けた部分**: 方法（比率法）、`AVCC`代理値の実測、周囲温度の記録、再現できる形での記載、および**確度が取得できないことの確定と、不確かさの下限の導出**（`最小単位`とADC量子化から。**下限であって確度ではない**）。**残る作業**: `5V` pinと`AVCC` pinの差を測るか上限を与えること。未加工の観測は[experiment-log.md](experiment-log.md)の`EXP-002` | 測定値が基準に対する相対値のままになり、`HW-TBD-028`(b)のような**絶対電圧の閾値と照合できない** | `基準電圧が未解決である（設計上の論点）`  |
