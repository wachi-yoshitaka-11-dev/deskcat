...
**あわせて、上表に現れていない経路要素が1つある**（次節`rail側のAメスは経路に現れるかが未決である（2026-08-22に判明した論点）`）。

#### rail側のAメスは経路に現れるかが未決である（2026-08-22に判明した論点）

**上表を読んだだけでは、最弱部品が1.5 Aの要素1つ（嵌合接点、変換基板側で決まる）だと読める。
そう読むと「変換基板を別品へ替えれば上限が上がる」という逃げ道があることになるが、それは
実装方法によって成立しない。**この節はその条件を明示するために置く。

`CABLE-PI-PWR-01`（breadboard rail → Piの給電）をrail側で受ける方法が決まっていない。

| 方法 | 上表への影響 |
|---|---|
| ① rail側にAメスの口を立て、市販のA-microBケーブルを挿す | **1.5 Aの経路要素が1つ増える**（下記の候補品を使う場合） |
| ② ケーブルを加工し、線をrailへ直付けする | この要素は経路に現れない。導体の定格は`CABLE-PI-PWR-01`の行で扱う |

①の候補として挙がっているのは秋月 通販107429「USBコネクターDIP化キット (Aメス)」（2.54 mmピッチ）で、
**定格は1ピンあたり1.5 A、価格は¥140とされている。この値は作業指示由来であり、商品ページを
一次資料として確認していない。①を採る場合は選定時に確認する。**

**①を採っても上限は動かない。**1.5 Aは`PSU-INGRESS-01`（変換基板）と同値であり、最小値は変わらない。
**動くのは「1.5 Aの要素が1つか2つか」である。**①では変換基板を別品へ替えても、もう一方の1.5 Aが
残るため上限は上がらない。**②ではその制約が無い。**

**購入待ちリストへは足さない。**実装方法の決定が人間待ちであり、②なら購入自体が不要である。
**起票も人間の判断とする**（作業指示が「方法が決まり次第、別途起票する」と定めている）。

**この論点は`HW-TBD-021`の前提に効く。**保護部品の選定は「判明している最弱部品の定格」を上限に置くため、
①②のどちらでも現時点の上限は変わらないが（**上限の数値はここへ再掲しない。**正は
`窓が閉じる境界（2026-08-22の計算）`）、**①を採ると上限を上げる手段が変換基板の交換だけでは
足りなくなる。**同節で出した境界も、同じ最弱部品の定格から導いている。

**なお、この節に出てくる1.5 Aは変換基板の候補品の値であり、品が変われば変わる**（`経路部品と定格`）。
**この節の主張は「rail側のAメスと変換基板が同値の定格になる」ことであって、1.5 Aという値そのものではない。**
品が変わって両者が同値でなくなれば、低い方が効く。
...
- **上限の1500 mAは確定値ではない。**`経路部品と定格`には未確定の行が残っており、
  ...
  **なお、上限を上げる手段はrail側の実装方法に依存する**（`rail側のAメスは経路に現れるかが未決である（2026-08-22に判明した論点）`）。
