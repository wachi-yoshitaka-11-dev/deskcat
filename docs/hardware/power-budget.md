...
|---|---|---|---|
| (a) Pi入力の最低電圧 | **段階A**（Piを単体でアダプターから起動） | **不要**（追加購入も不要） | **Blocked。**閾値が`HW-TBD-028`(a)として未確定であり、`ADC-5V`の測定系も未実装（分圧抵抗が未購入） |
...
| (c) 最大定常ripple | 段階A／B-1で各railを単独で、段階Cで合成後を測る。**両者を混ぜない** | 段階A／B-1は**不要**。段階Cは**必要** | **Blocked。**許容電圧範囲の下限が決まらないと不等式を評価できない（(a)(b)に従属） |
| (d) 最大transient droopと継続時間 | **段階C**（servo起動過渡を含む）。段階A／B-1では出ない | **必要** | **Blocked。**継続時間0は確定しているが、**深さの判定に使う下限が未確定**（(a)(b)に従属） |
| (e) connector／wireの最大温度上昇 | **段階C**。`HW-TBD-021`／`HW-TBD-022`の部品が揃ってから | **必要** | **Blocked。**経路部品が未選定で、比較対象となるメーカー定義の温度上昇条件が存在しない |

**段階AとB-1はgateも追加購入も要らない**ため、**測定そのものは閾値が決まる前でも進められる。**
探索的に値を取ること自体は妨げない。
