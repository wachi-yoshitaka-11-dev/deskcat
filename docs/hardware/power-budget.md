...
**2026-08-21時点で1と2は解けた。残るのは3（GND topologyへの影響）だけである。**
...
|---|---|---|---|---|
| 1 | **基準電圧の校正** | **解けた**（2026-08-21）。**`V_INT_actual` = 1.112 V、不確かさは少なくとも ±0.003 V。**AV<sub>CC</sub>を`5V` pinで実測（5.04 V）し、**周囲温度23.4 ℃を記録した。**`DS40002061B` Table 29-15の規定内である（typ から +12 mV）。**テスターの確度はメーカーが公開しておらず取得できないことが確定したため、`最小単位`（20 Vrangeで10 mV）とADC量子化から不確かさの下限を出した。****確度を超える主張はしていない。****測定条件をすべて記録したため別の人が再現できる**（未加工の観測は[experiment-log.md](experiment-log.md)の`EXP-002`）。**残る限定**: 測ったのは`5V` pinで`AVCC` pinと同一nodeではない代理値である。温度補正はしていない（一次資料に328Pの図が無い）。**別のAV<sub>CC</sub>へ持ち越せない** | 測定値が基準に対する相対値のままになり、`HW-TBD-028`(b)のような**絶対電圧の閾値と照合できない** | `基準電圧が未解決である（設計上の論点）` |
...
**1と2は解けた。**どちらもArduino単体で確認でき、DeskCat側へ接続していない。
**残るのは3だけである。**3はDeskCat側の配線を伴う。
