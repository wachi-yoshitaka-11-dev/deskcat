...
| 実施条件（保護） | board上regulatorの過電流保護／短絡保護の有無と動作（制限電流、折り返し特性、熱shutdownの有無）を**一次資料または現物回路で確認する**。**保護が無い、または確認できない場合はB-2aを採らない**（B-2bへ移る） |
...
| **全moduleの安全な上限** | **周辺module3点それぞれについて、耐えられる電流の上限を持つこと。**1点でも上限が無ければ、その moduleへB-2bで給電しない。**MSP2807は定格が未公開であり、現時点で上限が無い。**得るには現物のbacklight回路（電流制限抵抗等）の確認か一次資料が要る。得られない限り、**B-2bでMSP2807へ給電しない**（ADXL345とBME280だけを測っても未知数は埋まらないため、実質B-2bは成立しない） |
...
**MSP2807の安全な電流上限の追跡は[HW-TBD-024](tbd-register.md)で行う。**これが得られるまでB-2bは成立しない。
...
| MSP2807（LCD＋backlight＋touch） | **未確認。段階B-2（3.3 V側で定常電流を測る）で実測できる。ここが唯一の未知数である。**実施にはB-2a（`3V3` pinの定格とregulator保護の確認）かB-2b（全moduleの安全な上限、電流制限の動作確認、無負荷出力電圧の確認）のどちらかの条件が要る。**どちらの経路で得た値かを必ず記録する。**5 V側へ足せるのは**B-2aで得た値だけ**である（B-2bはregulatorを通らないため入力側の電流が発生しない。`B-2bの結果で言えないこと`）。**どちらの条件も満たせない場合、この行は段階Cまで実測待ちのまま残る** | **実測待ち** |
