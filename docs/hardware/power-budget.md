...
| **B-1** | ESP32を単体でPCのUSBから給電し、flashingとADC loggingを行う | gate不要。**PC hostのUSB portが持つ過電流保護（OCP）が制限として働く**。USB 2.0で公称0.5 A、多くのhostは1 A前後でtripする。ESP32単体の文献値（spike約500 mA）はこの範囲に収まる | 不要 |
| **B-2** | B-1に周辺module3点（MSP2807、ADXL345、BME280）を足し、**ESP32の`3V3` pinから給電**して定常電流を測る。5 V railもPiも使わない | **実施条件つき。**使用するhost portとその公表供給能力を記録し、停止条件と手順を`段階B-2の測定`に定める。**hostのtrip値を一般値で仮定しない** | 不要 |
| **C** | 変換基板でM-12001を受けてbreadboard railへ引き出し、そこからPi（`PWR IN`へcable）・ESP32・LCD・sensorへ合成給電する。**Piの5 V GPIO pinを経由して配電しない** | **gate必要。変換基板と過電流保護部品が揃い、ingressで実測できるようになるまで実施しない**（`過電流保護（段階Cのgate）`） | 変換基板、Piへの給電cable、過電流保護部品、大電流経路の線材・接続部材 |

A、B-1、B-2は通常の使い方であり、測れないgateに依存しない。#8（Pi Rust環境）と
#40（ESP32 toolchain）はここに収まる。Cだけを部品待ちにする。

段階Cの配電先は下の`servo試験以降で用いる構成`表と同じであり、**Piの5 V GPIO pinから
他へ配る構成ではない**（定格の理由は同表の`分岐点`行）。`hardware-bom.md`の
...
| 電流制限 | 使用するPC host portのOCP。**下記のとおり実施前に記録する** |
| 5 V railとPi | 使わない。合成給電ではないため段階Cのgateの対象外である |

**「PC hostのOCPが効く」だけでは実施条件にならない。**hostのtrip値はport実装に依存し、
一般値で仮定すると、測れないgateを置いたのと同じことになる。B-2の実施前に次を記録する。

| 記録する項目 | 内容 |
|---|---|
| 使用するhost port | PCの機種名とport種別（USB 2.0／3.x、給電専用かの別）。**この記録が無い状態でB-2を行わない** |
| 公表された供給能力 | 当該hostまたはUSB規格が公表する値。**「多くのhostは1 A前後」という一般値を実施条件に使わない** |
| 計画上の上限 | 上で確認できた公表値。確認できない場合は**USB 2.0の公称0.5 A**を上限として扱う（規格値であり、host固有の実力値ではない。安全側） |
| 上限と比べる量 | **USB portを通る合計**である。この測定のテスターは3.3 V branchしか通らないため、合計は`3.3 V branchの実測値 ＋ ESP32 board自身の消費`で見積もる。ESP32側は負荷表の文献値を上限側で採る。**Wi-Fiを停止した状態で測り**、この見積もりの不確かさを減らす |
| 停止条件 | 上の見積もり合計が計画上の上限に達した時点で中止する。ESP32のbrownout、reset、LCDの表示異常が出た場合も中止する |
| 停止手順 | **USB cableをhost側から抜く。**人が手を掛けられる状態で行い、無人で継続しない |
| 上限に達した場合 | MSP2807のbacklightを含む3点は、hostによっては0.5 Aの枠に収まらない可能性がある。その場合はB-2を成立させられないため、**測定を中止し、より供給能力の高いhost portを使うか、段階Cまで測定を延ばす**。枠を超えたまま続けない |

段階B-1もこの記録の対象とする。B-2はB-1に周辺module3点を足すため、B-1の条件が
記録されていなければB-2の増分も評価できない。
...
`hardware-bom.md`の部品受け入れchecklist（module回路図または検証済みpinout）で行う。
...
| 経路 | いつ | 電流制限 | 排他制約 |
|---|---|---|---|
| **段階B-1／B-2: PC → ESP32のMicro USB** | いま可能 | **PC hostのUSB portのOCP。**USB 2.0の公称は0.5 A、多くのhostは1 A前後でtripする。ESP32単体の文献値（spike約500 mA）はこの範囲に収まる。B-2で足す周辺module3点も同じOCP下にある | 満たす（USBのみ。`5V` pinへ何も繋がない） |
...
| M-12001に直付けされたcableとMicro-Bオスplug | TBD | **メーカーは出力定格3 Aを示すが、cable導体とplug接点の個別の定格は示していない。**出力定格でこの区間を代用しない | **未確定** |
| Micro-Bオスplugとreceptacleの嵌合接点 | TBD | 同上。接点はplug側とreceptacle側の低い方で決まる | **未確定** |
...
| `CABLE-PI-PWR-01` Micro-Bオスcable | TBD | 未選定。cableの導体はplugの定格より細いことがあり、plug定格で代用しない | **未確定** |
...
M-12001は5 V／3 Aを供給でき、候補構成の最弱部品は1.5 Aである。上限を超えた状態は、
...
これを段階Cのgateとする。測定計画の`サーボ接続前`checklistにも同じ確認を置く。
...
| MSP2807（LCD＋backlight＋touch） | **未確認。段階B-2（ESP32の`3V3` pinから給電し、3.3 V側で定常電流を測る）で実測できる。ここが唯一の未知数である。**5 V側へ足すにはregulatorの種別確認が要る（`段階B-2の測定`） | **実測待ち** |
...
未知数はMSP2807の1つだけである。段階B-2で測り、regulatorの種別を確認して5 V側へ
換算すれば合計が確定し、**変換基板の品をingressの実測を待たずに選べる**。
選定後の実測で見積もりを超えていた場合は、その時点で定格を見直す。
...
**この段階は変換基板（`PSU-INGRESS-01`）と過電流保護部品（`PROT-OC-01`）が揃ってから
実施する。**合成給電の電流をingressで測る必要があり、変換基板が無いとアダプターとPiの
間に測定器を挿入できない（`合成給電を変換基板の到着まで行わない理由`）。保護部品が無いと
上限を超えた電流を止められない（`過電流保護（段階Cのgate）`）。**servoはまだ繋がない。**

到着前にできるのは、`5 V ingress`節の段階A、段階B-1、段階B-2（Pi単体の起動、ESP32の
PC USBからのflashing、周辺module3点の3.3 V側定常電流の実測）だけである。これらは
合成給電ではないため、下記の測定を必要としない。

- [ ] **過電流保護部品が選定・実装され、そのtrip値が`過電流保護（段階Cのgate）`節に記録されている**
...
- [ ] ESP32 board上3V3 pinの外部供給可能電流の定格を確認し、周辺module3点（MSP2807、ADXL345、BME280）を接続した状態で3V3 rail電圧と電流を実測する。3V3 pinの供給能力を超える場合は別途3.3V regulatorを追加する。**この実測は段階B-2で先に済ませてよい**（`段階B-2の測定`）。段階Cで再測するのは、5 V railからの給電に切り替えた後の値を確認するためである
...
| 過電流保護部品の実装 | **段階C（合成給電）の前に必須。**trip値を`過電流保護（段階Cのgate）`節へ記録する | テスターの読みと手動停止は検知であって保護ではない。M-12001は3 Aを供給できる |
