...
| 3 | **GND topologyへの影響** | **未解決**（2026-08-22に段階B-1を実施。未加工の観測は[experiment-log.md](experiment-log.md)の`EXP-003`）。**実測はできたが解けていない。理由が2つある。****(i) GNDの確認が片側だけである。**単端ADCの`0`は「ESP32の`GND`がArduinoの`GND`より1 LSB以上高くはない」ことしか意味せず、**低い側にずれている場合は測定範囲の外で同じく`0`と観測されうる。**両極性を測れる手段が要る。**(ii) star point構成そのものの確認が残る。**変換基板`PSU-INGRESS-01`が揃うまでできない。**段階B-1で実測できたのは3項目**（ADC値の妥当性、GNDの結合の影響、入力headroom）**である。****異常は無かった**（異音・発熱・reset・brownoutのいずれも無し。人間が基板のそばで観察） | star point構成を崩すと**全ADC値がずれる。****PCのGNDが経路へ入る影響は段階B-1で静的なA/B比較まで評価した**（`EXP-003`）。**未完了なのは、star point・5 V ingress・shuntを含む完全なtopologyと、GNDの両極性の確認である** | `GND topology（測定前に必ず確定させる）`と、`電源過渡の測定方式のdesign review`の方式1の表にある`手持ち候補に固有の論点`の行   |
