...
| 参照設計の`U2 = AMS1117-3.3`の出力電圧 | datasheetは`VIN = 4.8V`に対し**2行**を与える。狭い方が3.251–3.349 V、広い方が**3.201–3.399 V**。**採るのは広い方である**（理由は下記）。**ただし現物の`U2`は`LD1117AG`であり参照設計と違う**（2026-08-13確認）。**この値は現物の値ではない**（`HW-TBD-023`） | [AMS1117 datasheet](http://www.advanced-monolithic.com/pdf/ds1117.pdf)の`Electrical Characteristics`、`Output Voltage`行（`AMS1117-3.3`、`VIN = 4.8V`） | **参照設計由来であり現物の事実ではない**（`HW-TBD-023`。現物の`U2`の品もメーカーも未確認） |
...
**参照設計由来である。****2026-08-13の現物確認で、参照設計と現物が違うことが判明した。**
現物の`U2`の刻印は`LD1117AG` / `33AQVCUV`であり、参照設計の`AMS1117-3.3`ではない。
どちらもSOT-223の3.3V LDOでpin互換の二次ソースだが部品が違う。
**したがって以下の`AMS1117`のdatasheet値は、現物の値ではない。**
`LD1117AG`のメーカーが特定できていないためそのdatasheetを当てられず、
**現物のdropout電圧は未確定である**（`HW-TBD-023`）。
以下は参照設計に基づく暫定値として残すが、**現物の合格判定には使えない。**
