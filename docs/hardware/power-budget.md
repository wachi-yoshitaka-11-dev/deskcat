...
| 理由 | 内容 |
|---|---|
| **電流の桁が違う** | この経路を流れるのはESP32の消費分（文献値でpeak約500 mA）である。ingressのgate（暫定1.2 A以下）とは桁が違う |
...
最弱部品の定格」を上限に置く（**上限の数値はここへ再掲しない。**正は`窓が閉じる境界（2026-08-22の計算）`）。
...
未知数はMSP2807の1つだけである。**この未知数には上限がある。****360 mA以上になると過電流保護部品の選定の窓が閉じる**（境界そのものが閉じる側である）（`窓が閉じる境界（2026-08-22の計算）`。**この値は判明している最弱部品の定格に依存するため、より低い定格が現れれば下がる**）。**したがって段階B-2の測定は、この境界に対する合否の判定でもある。**`3V3` pinの定格を確認したうえで段階B-2で測り、regulatorの種別を確認して5 V側へ
...
| 逆極性保護 | 配線リスク | **要否と方式は2026-08-24に人間が承認した。**ingressへP-ch MOSFETを置き、下流の手配線区間は極性が固定される接続方法と通電前の配線確認手順で守る（承認範囲の全文は`逆極性保護のdesign review`の`決定（2026-08-24）`が持つ）。**品番と定格はTBD**（発注直前に一次資料で確定する） | Design review。**逆接が起こりうる区間の洗い出しと方式ごとの電圧降下の代償**は同節 | **Blocked（[HW-TBD-030](tbd-register.md)。方式は承認済みだが品番・定格が未確定で、通電前の配線確認手順も未作成）** |
...
**この承認はgateを開けない。**品番と定格が未確定である以上、段階Cの配線承認は与えられない。
**通電前の配線確認手順そのものも、まだ書かれていない。**2で守ると決めたのは方針であって手順ではなく、
手順の作成は`HW-TBD-030`に残る。
