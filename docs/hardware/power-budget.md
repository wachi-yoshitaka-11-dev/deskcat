...
②では1.5 Aの要素が嵌合接点の1つだけであるため、**上限を上げたい場合は変換基板の交換で足りる。**
...
- **上限の1500 mAは確定値ではない。**`経路部品と定格`には未確定の行が残っており、
  ...
  **上限を上げる手段はrail側の実装方法に依存するが、2026-08-24に②（直付け）で確定したため、
  この上限を上げるには変換基板の交換で足りる**（`rail側の受け方は②（直付け）で確定した（2026-08-24、人間の判断）`）。
