...
| 項目 | 選定 |
|---|---|
| ヒューズ | Littelfuse **218 Series**（5×20 mm、Time-Lag、ガラス管）、品番 **`02181.25HXP`**（1.25 A／250 V） |
...
| 定格電圧 | 250 V | 同表 |
| 遮断定格（`Interrupting Rating`） | **35 A @ 250 VAC。**同列は`0.032 A`〜`3.15 A`の行を覆う結合cellであり、**1.25 A行にも適用される**（**text抽出では空欄に見えるため、2026-08-25に図で確認した**） | 同表 |
...
| 過電流保護 | 故障電流の制限。M-12001は3 Aを供給でき、テスターの読みと手動停止では最弱部品が発熱する前に電流を止められない | **方式・品番とも2026-08-25に決定した。**ガラス管ヒューズ（Littelfuse 218 Series `02181.25HXP`、1.25 A／250 V、Time-Lag）＋ミゼットホルダ（秋月 通販110521、`FUC-03A`）。**PTCは採らない。****未購入である**（[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)が発注対象）。選定の全文と一次資料値は`過電流保護（段階Cのgate）`の`方式と品番を決定した（2026-08-25）`。**この品の定格電流が経路の最小値になり、ingressの上限が下がった**（同節の`判明している最小定格が1.25 Aへ下がった（2026-08-25）`） | `5 V ingress`節の`過電流保護（段階Cのgate）`。品の一次資料は[Littelfuse 218 Series datasheet](https://www.farnell.com/datasheets/3689252.pdf) | **Blocked（段階Cのgate。**方式と品番は決まったが**未購入であり、`記録するtrip動作`のうち実装後にしか埋まらない欄が空である。**加えて**ホルダの定格がメーカー資料に無く**、`経路部品と定格`表の未確定行が残る。**trip値の締め直しの判断は段階B-2に従属する。**[HW-TBD-021](tbd-register.md)） |
...
3. **テスターで導通を読む。**目視で追えない箇所（加工したcableの導体とplug pinの対応、
   端子台の内部結線）は、**DT830Bの抵抗rangeでpinと導体の対応を読む。**
...
4. **記録する。**確認した箇所、方法（目視／導通）、日付を
   [experiment-log.md](experiment-log.md)へ残す。**記録が無い確認は、次の通電ではやり直しになる**
