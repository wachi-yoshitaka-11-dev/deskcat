...
**上表の`TBD`のうち6箇所は`WIRE-PWR-01`と`CABLE-PI-PWR-01`の未選定に起因する**
...
1. **`WIRE-PWR-01`**: 許容電流そのものか、`導体抵抗`（`r0`）のいずれかを**製造者が公開している**線材を選ぶ。
   後者なら上の式で評価でき、**周囲温度と条数を明示した導出**として記録できる。
2. **分岐点の接続部材**: logic railとservo railへ分ける部材。**未調査。**
   ねじ端子台のように定格電流が公開されている部材が候補になりうる。
3. **`CABLE-PI-PWR-01`**: Micro-Bオスcable。**未調査。**
   同資料の規則により、**plugの定格で導体を代用しない。**
...
```text
下限  I >= (定常電流の合計) / 0.8        ... 連続通電できる電流の80%が想定負荷を上回る
...
| 量 | 値 |
|---|---|
| 下限（MSP2807を除く） | 840 / 0.8 = **1050 mA** |
| 上限（判明している最弱部品） | **1500 mA** |
| **窓** | **1050 〜 1500 mA**（幅 450 mA） |

**MSP2807の定常電流を`x`とすると、窓が空でない条件は次になる。**

```text
(840 + x) / 0.8 <= 1500     =>     x <= 360 mA
```

**したがって、MSP2807の定常電流が360 mAを超えると窓は閉じる。**
そのとき下限が上限を上回り、**両方を満たす保護部品は存在しない。**
...
`段階B-2の測定`でMSP2807の3.3 V側定常電流を測り5 V側へ換算した値が360 mAを超えたら、
...
未知数はMSP2807の1つだけである。**この未知数には上限がある。****360 mAを超えると過電流保護部品の選定の窓が閉じる**（`窓が閉じる境界（2026-08-22の計算）`。**この値は判明している最弱部品の定格に依存するため、より低い定格が現れれば下がる**）。**したがって段階B-2の測定は、この境界に対する合否の判定でもある。**`3V3` pinの定格を確認したうえで段階B-2で測り、regulatorの種別を確認して5 V側へ
