...
| 3 | bus全体の実効pull-upの計算と、その結果に基づく`J1`／`J2`の判断 | **まだできない。**[gpio-assignment.md](gpio-assignment.md)の`I2C sensor bus`。**ADXL345側のpin接続の確認が先に要る** |
| 4 | **I2Cアドレスの配線**（`SDO`→GNDで`0x76`、`SDO`→VDDで`0x77`） | **行われた記録が無い。**基板側に既定値が無いため、配線しないとアドレスが定まらない。**（2026-09-22追記: 2026-08-22時点では「未実施」と書いていた。`J3`の件で、記録の不在を未実施と書く型が実際に外れたため、言い方を改めた。現況をユーザーへ確認したわけではない。）** |
| 5 | 実装後の動作確認（Device ID register `0xD0`のreset値`0x60`の読み出し） | **行われた記録が無い**（2026-09-22追記: #4と同じ理由で「未実施」から改めた） |
