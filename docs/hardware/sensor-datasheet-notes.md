...
**この実測で済んだのは(1)だけである。**追跡は[tbd-register HW-TBD-005](tbd-register.md)が行う。

| # | 要ること | 状態 |
|---|---|---|
| 1 | `J1`／`J2`／`J3`の実装状態の確定 | **済**（2026-08-22。上の実測） |
| 2 | **I2Cで使う場合の`J3`のはんだ付け** | **未実施。**開放では`CSB`が`VDD`へ繋がらないため、説明書が定めるとおりはんだジャンパが要る |
...
| 4 | **I2Cアドレスの配線**（`SDO`→GNDで`0x76`、`SDO`→VDDで`0x77`） | **未実施。**基板側に既定値が無いため、配線しないとアドレスが定まらない |
| 5 | 実装後の動作確認（Device ID register `0xD0`のreset値`0x60`の読み出し） | **未実施** |
