...
> (a) 現物確認が要るもの（jumper状態、address、backlight回路）。**touch controller型番は2026-08-13に`XPT2046`と確定した**、**backlight回路は2026-09-05に`U1`の3本足（入力／GND／出力）を確定し、`LED` pinが公式User Manualの定める制御信号線であることも確認した。ただしLEDへの給電ライン（`VCC`側か`U1`出力側か）は引き続き未確定。LED個数はカプトンテープに隠れ非分解では未確認のまま残る**、
...
| Backlight回路／電流／polarity | polarityは**highで点灯**（datasheet記載）。**回路と電流は未公開**であり、datasheetの`Power Consumption`欄は`TBD`と印字されている。**`LED` pinは公式User Manual（LCDWIKI、`2.8inch_SPI_Module_MSP2807_User_Manual_EN.pdf`、Interface Description表、Pin 8）が「LCD backlight control signal (high level lighting, if you do not need control, please connect 3.3V)」と定めており、電源ラインではなく制御信号線である。**`akizukidenshi.com`配布の`msp2807.pdf`（`Power Consumption`欄が`TBD`）とは別文書であり、2026-09-05に新たに参照した。**2026-09-05に現物の非通電導通測定を行った。**`LED` pinと`VCC` pinの間は導通なし（オーバーレンジ）であることを確認した。`U1`（3.3V LDO、刻印`662K`／`UMW S4T`）の3本足を識別した: 1本は`VCC`と導通（入力側）、1本は`GND`と導通（安定した再測定で確認）、残る1本はどちらとも非導通（出力側、消去法）。`LED` pinは、`VCC`および`U1`の3本の足のいずれとも導通しない（オーバーレンジ）ことを確認した。**これは`LED` pinが上記の通り制御信号線であることと整合する。****実際の物理LEDへ電力を供給しているライン（`R5`の先）が`VCC`側か`U1`出力側かの特定は試みたが、決められなかった。**`R5`（6.8Ω、部品が小さく確実にプローブを当てられない）の両脚は`VCC`・`U1`出力側のいずれとも非導通と読めたが、部品サイズから読み取りの信頼性が低い。代わりに`Q1`（SOT-23、刻印`J3Y`）の3本足で`VCC`・`U1`出力側との導通を試みたが、3本とも両方に非導通だった。**`Q1`はトランジスタであり、非通電・単純な抵抗測定では接合の向き次第で導通を示さないため、この方法では判定できないと判断した。**したがって、backlightのLED給電経路が`VCC`側か`U1`出力側かは、非通電の導通測定では確定できないと結論する。LED個数と、`R5`／`R6`／`Q1`／`J1`を経た正確な電流経路はパネル端のカプトンテープに隠れており未確認のまま（分解を要するため確認しない）。**配線方法はここに書かない。**配線規則の正本は[power-budget.md](power-budget.md)であり、ここへ再掲しない。**2026-09-23追記（[#461](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/461)、ユーザー承認）。**`HW-TBD-024`（このmoduleが耐えられる電流の上限）は今も未公開のまま未解決だが、`LED` pinを3.3Vへ直結する構成（backlight点灯を含む）自体を一律に妨げる扱いは終了した。判断根拠・残る限定は[tbd-register HW-TBD-024](tbd-register.md)を参照し、ここへ再掲しない |
...
| pin列 | 上段 `CS` `Vs` `GND` `VDD`、下段 `INT1` `INT2` `SDO` `SDA` `SCL`（2026-08-13確認）。**`Vs`と`VDD`が別pinとして表記されている。**ただし**これはheaderのsilkラベルであって、board上の配線ではない。****2026-09-05の非通電導通測定で、`Vs`ピンと`VDD`ピンの間が導通している（同一net）ことを確認した。**2系統を独立に給電できる設計ではない。**ただし、この共通netがICの`VS`／`VDD I/O`それぞれへ直結しているか、直列抵抗が入るかは、IC足が小さくプローブを直接当てられずパターンを追っていないため未解決のまま残る**（接写ではheader〜IC間に`C1`が`Vs`側・`VDD`側それぞれ1個見えるが断定しない） |
...
| 測定量 | 温度、湿度、気圧 |
| Interface | I2C（最大3.4 MHz）または SPI 4線式／3線式（最大10 MHz）、選択式。**現物の`J1`／`J2`／`J3`で確定する**（[tbd-register HW-TBD-005](tbd-register.md)）。**2026-09-22追記: この行の「現物の`J1`／`J2`／`J3`で確定する」は、interfaceがI2CとSPIのどちらになるかを指す。****`J3`が2026-09-07にはんだ付けされたことでI2C側になったが、その根拠はユーザーの申告であり実測ではない。**`J3`の現在の状態と根拠の水準は下の`jumper（AE-BME280）`節が持つ。**ここへ再掲しない。****同節の記述はユーザーの申告に基づき、測定値を持たない。**したがって**この行の「確定する」は、実測による確定としては未達のままである** |
...
| jumper | 意味 | **現物の実装状態（2026-08-22に実測）** | **現在の状態（2026-09-22時点。根拠はユーザーの申告であり、実測ではない）** |
|---|---|---|---|
| `J1` | I2C時の SDA 用プルアップ（R1 4.7 kΩ）の選択 | **開放** | **開放のまま**（2026-09-22にユーザーが申告。**この時点での再測定の記録は無い**） |
| `J2` | I2C時の SCL 用プルアップ（R2 4.7 kΩ）の選択 | **開放** | **開放のまま**（同上） |
| `J3` | **I2C時にはんだでジャンパする**（`CSB`→VDD） | **開放** | **はんだ付け済み**（実施2026-09-07。**2026-09-22のユーザーの申告。この記録を書いたAIセッションは確認しておらず、測定値は無い**。記録は下の`J3をはんだ付けした（2026-09-07）`節と[experiment-log.md](experiment-log.md)の`EXP-014`） |

**SPI 4線式／3線式で使う場合は`J1`〜`J3`をすべてオープンにする。**

**2026-09-22追記。****2つの列は根拠の水準が違う。**左は測定表を持つ実測（下の2026-08-22の節）、
右は**ユーザーの申告**（同じく下の2026-09-07の節）である。**同格の確定事実として並べて読まない。**
**`J3`にはんだが乗っているため、この現物の構成はI2C向けになった**（根拠はユーザーの申告。実測ではない）。SPIで使うには`J3`を外す作業が要る。**「構成がI2C向け」は`J3`にはんだが乗っているという申告までを指す。****`CSB`と`VDD`が電気的に繋がっているかは判定していない**（下の`J3をはんだ付けした（2026-09-07）`節の`この記録が主張しないこと`）。

#### 現物の実装状態を実測で確定させた（2026-08-22）

> **2026-09-22追記。****この節は2026-08-22時点の記録である。既存の文言は1つも書き換えていない**（測定表を含む。**日付つきの追記は本文中にも入れた**。下記の状態列の変更だけが例外である）**。****`closeまでに要ること`表は状態列を更新した。**#2は`未実施`→`済`（旧記述は同じ欄に残した）、#4と#5は`未実施`→`行われた記録が無い`である。**表の前文にも時点の追記を入れた。****`J3`はその後2026-09-07にはんだ付けされた**（下の`J3をはんだ付けした（2026-09-07）`節）。**`J1`／`J2`は2026-09-22時点でも開放のままである**（ユーザーの申告。再測定の記録は無い）。
...
> **2026-09-22追記。****この実装作業は2026-09-07に実施された**（下の`J3をはんだ付けした（2026-09-07）`節）。**上の「現状のままではI2Cで動かない」は2026-08-22時点の記述である。**
...
##### closeまでに要ること（実装と確認を含む）

**この実測で済んだのは(1)だけである。**追跡は[tbd-register HW-TBD-005](tbd-register.md)が行う。（**2026-09-22追記: これは2026-08-22時点の記述である。**下の表では#2も`済`になっている。）

| # | 要ること | 状態 |
|---|---|---|
| 1 | `J1`／`J2`／`J3`の実装状態の確定 | **済**（2026-08-22。上の実測） |
| 2 | **I2Cで使う場合の`J3`のはんだ付け** | **済（根拠はユーザーの申告。#1の`済`と水準が違う）**（2026-09-07に実施。**#1は測定表を持つ実測だが、この行は測定値を持たない。**実施の記録は下の`J3をはんだ付けした（2026-09-07）`節と[experiment-log.md](experiment-log.md)の`EXP-014`）。**2026-09-22までは「未実施。開放では`CSB`が`VDD`へ繋がらないため、説明書が定めるとおりはんだジャンパが要る」としていた。**その記述は2026-08-22時点では正しく、**実施から記録までの15日間は記録の欠落であって、状態の記述の誤りではない** |
| 3 | bus全体の実効pull-upの計算と、その結果に基づく`J1`／`J2`の判断 | **まだできない。**[gpio-assignment.md](gpio-assignment.md)の`I2C sensor bus`。**ADXL345側のpin接続の確認が先に要る**（2026-09-24追記（[#472](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/472)）: ADXL345側のpin接続は2026-08-27に確定している。残る入力は`Cb`である。同文書の`I2C busの実効pull-up`節） |
| 4 | **I2Cアドレスの配線**（`SDO`→GNDで`0x76`、`SDO`→VDDで`0x77`） | **行われた記録が無い。**基板側に既定値が無いため、配線しないとアドレスが定まらない。**（2026-09-22追記: 2026-08-22時点では「未実施」と書いていた。`J3`の件で、記録の不在を未実施と書く型が実際に外れたため、言い方を改めた。現況をユーザーへ確認したわけではない。）**（2026-09-24追記（[#472](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/472)）: これは`EXP-015`より前の記述である。[experiment-log.md](experiment-log.md)の`EXP-015`（2026-09-22）の配線は`SDO`→`GND`である。**この行の状態は変えていない**） |
| 5 | 実装後の動作確認（Device ID register `0xD0`のreset値`0x60`の読み出し） | **行われた記録が無い**（2026-09-22追記: #4と同じ理由で「未実施」から改めた）（2026-09-24追記（[#472](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/472)）: これは`EXP-015`より前の記述である。[experiment-log.md](experiment-log.md)の`EXP-015`（2026-09-22）が、読み出しで生byte `0x60`を得たと記録している。一致判定はしていない。**この行の状態は変えていない**） |

**(2)と(4)は実装作業であり`TBD`ではない。それでもcloseの条件に含める。**
`HW-TBD-032`が選定・購入・実装・確認を`必要な根拠`に含めているのと同じ扱いにする。
**実装を含めないと、状態が確定した時点でcloseできてしまい、動くことを確かめないまま次へ進む。**

**(2)を行う前に決めることが1つある。**`J3`をはんだ付けするとI2C専用の構成になる。
**SPIで使う選択肢を残すなら、後戻りに手間がかかる。**
[gpio-assignment.md](gpio-assignment.md)の`I2C sensor bus`はGPIO25／GPIO26で確定しており
**I2Cを採る前提は立っているが、この文書にその決定は記録していない。**
**はんだ付けはその決定を確認してから行う。**

> **2026-09-22追記。****この段落が求めた「I2Cを採るという決定の確認」を、はんだ付け（2026-09-07）の前に行った記録は無い。**`J3`は既にはんだ付けされており、**この現物の構成はI2C向けになった。**この段落は2026-08-22時点の記述として残す。**SPIで使う選択肢を残すかは、以後この文書では未決として扱わない。****これが変えるのはこの節の扱いだけである。**[tbd-register.md](tbd-register.md)の`HW-TBD-005`と[hardware-bom.md](hardware-bom.md)のSPI記述には手を付けていない。**追跡をやめるのはこの記録の判断である。****AIセッションの判断であり、人間の承認ではない。**現物がI2C側になったことの帰結だが、**`J3`を外せばSPIへ戻せるため不可逆ではなく、自動的に決まることではない。****設計判断としてSPIを捨てた記録はどこにも無い。****この状態（追跡はしないが決定は無い）を誰が確定または再開させるかも決まっていない。**
理由は`J3`が既にはんだ付けされていることであり、**その根拠はユーザーの申告である**（この記録を書いたAIセッションは確認していない）。
**「現物で確定した」とは書かない。**測定値を持っていない。
**残る欠落が2つある。**(1) I2Cを採るという決定そのものが、どの正本にも記録されていない。
(2) **この段落が定めた事前条件（決定を確認してからはんだ付けする）が満たされないまま作業が行われたことを、
誰が受け入れたのかが記録されていない。**事前条件が撤回されたのか、破られたまま追認されたのかは、
**この文書では判定しない。**

> **2026-09-24追記（[#472](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/472)）。**上の欠落(1)を埋めた。**2026-09-24、ユーザーが`ACCEL-01`／`ENV-01`をI2Cで使うことを承認した（[#472](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/472)の本文の`範囲`節）。**記録先は[hardware-bom.md](hardware-bom.md)の両行の`Interface`列と、[tbd-register.md](tbd-register.md)の`HW-TBD-005`である。**`J3`のはんだ付けは引き続きユーザーの申告であり、測定値を持たない**（上の限定は変わらない）。**欠落(2)はこの追記でも判定しない。**

##### この記録が主張しないこと

- **`J1`／`J2`をはんだ付けするかを決めていない。**bus全体の実効pull-upの計算が先である。
- **`J3`のはんだ付けをまだ行っていない。**（**2026-09-22追記: これは2026-08-22時点の記述である。`J3`は2026-09-07にはんだ付けされた。**下の`J3をはんだ付けした（2026-09-07）`節）
- **通電していない。**moduleへの給電は一度も行っていない。（**2026-09-22追記: これは2026-08-22時点の記述である。**2026-09-07のはんだ付け時に通電したかどうかは、下の`J3をはんだ付けした（2026-09-07）`節のとおり判定しない）
...
#### J3をはんだ付けした（2026-09-07）

**`J3`ははんだ付け済みである。**上の`現物の実装状態を実測で確定させた（2026-08-22）`節は
2026-08-22時点の記録であり、**その後にこの変更が入った。**同節の本文は書き換えていない。

| 項目 | 内容 |
|---|---|
| 対象 | `ENV-01`（BME280モジュール、秋月 K-09421 / AE-BME280）の`J3`。**I2C時にはんだジャンパする** jumper であり、閉じると`CSB`(pin3)が`VDD`(pin1)へ繋がる |
| 実施日 | **2026-09-07**（ユーザーの申告、2026-09-22） |
| 実施者 | **訊いていないため記録しない。****この記録を書いたAIセッションは実施にも確認にも立ち会っていない。**他のセッションについては記録が無く、判定しない |
| 確認方法 | **写真とテスター**（ユーザーの申告）。**測定点・レンジ・読みの記録は残っていない** |
| `J1`／`J2` | **2026-09-22時点で両方とも開放のまま**（ユーザーの申告）。**この時点での再測定の記録は無い** |

##### この記録の水準

**この節はユーザーの申告に基づく。**2026-08-22の節が持つような測定表（測定点・レンジ・読み・判定）を
**この節は持たない。****持っていない測定値を、同じ体裁で並べない。**

ユーザーは2026-09-22に「`J3`ははんだ付け済み。写真とテスターで確認した」と言明した。
測定点を訊いたところ「覚えていないが、提示したすべてのケースで問題なしという結果になった」との回答であり、
**どの端子間をどのレンジで測り、何と読んだかは記録に残っていない。**

##### この記録が主張しないこと

- **この記録を書いたAIセッションは`J3`の閉を確認していない**（他のセッションについては記録が無く、判定しない）**。**写真も測定値も本セッションでは見ていない。根拠はユーザーの申告だけである。
- **`VDD`↔`CSB`間の導通を測定値として持っていない。**2026-08-22の`開放`と対になる`導通`の読みは記録に無い。
- **通電の有無を主張しない。**[experiment-log.md](experiment-log.md)の`EXP-001`〜`EXP-013`を`ENV-01`と`BME280`で検索して0件だった（2026-09-22）が、**2026-09-07の作業時に通電したかどうかは判定しない**（この記録を書いたAIセッションは立ち会っておらず、ユーザーの申告にも給電の有無は含まれていない）。**記録として確かなのは、この記録を書いたAIセッションがChip ID（register `0xD0`、reset値`0x60`）の読み出しを行っていないことだけである。**
- **`J1`／`J2`を2026-09-22に測り直していない。**開放であることはユーザーの申告であり、2026-08-22の実測が変わっていないという申告として扱う。
- **はんだ付けの品質（ブリッジ・未接合・隣接pinへの流れ）を判定していない。**

##### 記録が15日遅れた経緯

**実施は2026-09-07、記録は2026-09-22である。**この間、**正本のどこにも、`J3`がはんだ付け済みであるという記録が無かった。**

**状態は文書ごとに違う。**この文書・[gpio-assignment.md](gpio-assignment.md)・[tbd-register.md](tbd-register.md)・
Issue `#16`は`未実施`／`開放`と書いていた。**[experiment-log.md](experiment-log.md)は`J3`に関する記述を1件も持っていなかった**
（誤った記述があったのではなく、**記録が存在しなかった**）。

**2026-09-07の時点でAI側がこの作業を知らされていたかどうかは、記録が無く、ここでは判定しない。**
確かなのは、15日間どの正本にも記録が無かったことだけである。

**上の列挙は、`docs/`と`firmware/`を`J3`で全数走査した結果のうちMarkdownの正本と、
走査では出てこないIssue `#16`を並べたものである（2026-09-22）。**
**走査にはこれ以外に`firmware/esp32/src/main.rs`の`run_i2c_bringup`のdoc commentが掛かり、同じ変更で更新した。**
**走査で当たった他のfile**（`firmware/esp32/README.md`、`docs/toolchains/esp32-rust-toolchain.md`、`docs/decisions/0016-*.md`）**は、ESP32-DevKitCのheader `J2`／`J3`と逆極性保護MOSFET`2SJ334`であり、この`J3`とは別物である。**
**ただし網羅を主張しない。**走査の対象外に置いた既知の古い記述が4つある。

1. [power-budget.md](power-budget.md)。`J3`の状態により読み直しが要る記述が12行ある。別の変更と並行するため今回触らない。
2. [hardware-bom.md](hardware-bom.md)の`ENV-01`行`選択jumperの実装状態はTBD（現物確認要）`。
   **2026-08-22の実測時点で既に古く、今回の変更が生んだものではない。**
   **引き受け先は決めていない。**
3. [hardware-bom.md](hardware-bom.md)の`TOOL-01`行の用途列（`ENV-01`の`J3`のはんだジャンパを、I2Cで使う場合の必須作業として挙げている）。
   工具を持つ理由の記述であり状態の主張ではないが、**未了の作業として読める。引き受け先は決めていない。**
4. `firmware/esp32/src/main.rs`の`run_i2c_bringup`のdoc commentに残る行番号参照2件（`134行目`と`140行目`）。
   **どちらもこの文書の171行目が該当する。**同commentの`182行目`→`213行目`は同じ変更で訂正したが、
   **この2件は触っていない行にあるため残した。**
**再発を防ぐ運用は[experiment-log.md](experiment-log.md)の`EXP-014`に提案として置いた。まだどの正本にも置いていない。**
...
| 2026-08-22 | 11 | **`HW-TBD-005`のcloseまでに要ることを、実装と確認まで含めて表にした。****先の実測で済んだのは実装状態の確定だけである。**残るのは`J3`のはんだ付け、bus全体の実効pull-upの計算と`J1`／`J2`の判断、I2Cアドレスの配線、実装後の動作確認（Device IDの読み出し）である。**はんだ付けと配線は`TBD`ではなく実装作業だが、closeの条件に含める。**`HW-TBD-032`が選定・購入・実装・確認を`必要な根拠`に含めているのと同じ扱いにする。**実装を含めないと、状態が確定した時点でcloseできてしまい、動くことを確かめないまま次へ進む。****`J3`のはんだ付けはI2C専用の構成にする方向の変更であり、SPIで使う選択肢を残すなら後戻りに手間がかかる。**I2Cを採る前提は[gpio-assignment.md](gpio-assignment.md)側に立っているが、**この文書にその決定は記録していない。はんだ付けはその決定を確認してから行う**（[#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)） |
...
| 2026-09-22 | 16 | [#453](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/453)（[#16](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/16)の残件1件の記録。**`#16`自体はcloseしない**）。**`ENV-01`（AE-BME280）の`J3`がはんだ付け済みであることを記録した。**実施は**2026-09-07**、記録は2026-09-22であり、**15日間、正本のどこにも`J3`がはんだ付け済みであるという記録が無かった。****この文書・[gpio-assignment.md](gpio-assignment.md)・[tbd-register.md](tbd-register.md)・Issue `#16`は`未実施`／`開放`と書いており、[experiment-log.md](experiment-log.md)は`J3`に関する記述を1件も持っていなかった**（誤りではなく不在である）。**2026-09-07の時点でAI側がこの作業を知らされていたかどうかは記録が無く、判定しない。**確かなのは、15日間どの正本にも記録が無かったことである。変更した箇所は次のとおりである。**件数は書かない**（数え直すたびに外したため）。(a) `Environmental sensor`表の`Interface`行へ、interfaceがI2C側になったこととその根拠が実測ではないことを追記した。(b) `jumper（AE-BME280）`節の表へ`現在の状態（2026-09-22時点）`の列を追加し、表の下へ2つの列の根拠の水準が違うことと、この現物が構成がI2C向けになったことを書いた（**`CSB`と`VDD`の電気的な接続は主張しない**）。**`現物の実装状態（2026-08-22に実測）`の列は1欄も変更していない。**`J3`＝**はんだ付け済み**、`J1`／`J2`＝**開放のまま**である。(c) `closeまでに要ること（実装と確認を含む）`表の前文へ時点の追記を入れ、#4と#5の状態列を`未実施`から`行われた記録が無い`へ改めた。**`J3`の件で、記録の不在を未実施と書く型が実際に外れたためである。****現況をユーザーへ確認したわけではない。**(d) `現物の実装状態を実測で確定させた（2026-08-22）`節へ、2026-08-22時点の記録である旨と後続の変更への案内を追記した。**同節の本文・測定表・`この状態から出る帰結`・`この記録が主張しないこと`は書き換えていない。**帰結(1)、`(2)を行う前に決めることが1つある`段落、`主張しないこと`の`J3`の行と通電の行へ日付つきの追記を入れた。**既存の文言は1つも書き換えていない。****`closeまでに要ること`表の#2は`未実施`から`済`へ改め、旧記述を同じ欄に残した。**(e) `J3をはんだ付けした（2026-09-07）`節を新設した。**この節はユーザーの申告に基づく記録であり、測定表を持たない。**確認方法は写真とテスターだが、**測定点・レンジ・読みの記録は残っていない**（ユーザーは覚えていないと回答）。**持っていない測定値を2026-08-22の表と同じ体裁で並べないため、水準が違うことを明示した。**`主張しないこと`として、この記録を書いたAIセッションが閉を確認していないこと、`VDD`↔`CSB`の導通を測定値として持たないこと、通電の有無を主張しないこと（Chip ID読み出しをAIが行っていないことだけは言える）、`J1`／`J2`を2026-09-22に測り直していないこと、はんだ付けの品質を判定していないことを挙げた。**あわせて、`(2)を行う前に決めることが1つある`段落が求めた「I2Cを採るという決定の確認」を、はんだ付けの前に行った記録が無いことを明記した。**この現物の構成はI2C向けになった（**根拠はユーザーの申告であり実測ではない。`CSB`と`VDD`の電気的な接続は判定していない**） |
| 2026-09-23 | 17 | [#461](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/461)。**`Backlight回路／電流／polarity`行の末尾が、`HW-TBD-024`が未解決の間は`LED` pinへの3.3V直結を判定できないとしていたが、この記述は2026-09-23（ユーザー承認）に偽になった。**`HW-TBD-024`は今も未公開のまま未解決だが、`LED` pinを3.3Vへ直結する構成（backlight点灯を含む）自体を一律に妨げる扱いは終了した。判断根拠・残る限定は[tbd-register.md](tbd-register.md)の`HW-TBD-024`行（[#461](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/461)）を参照し、ここへ再掲しない。値・配線規則・他の記述は変えていない |
| 2026-09-24 | 18 | [#472](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/472)（[PR #471](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/471)のCodeRabbit review指摘）。`closeまでに要ること（実装と確認を含む）`の後の段落への2026-09-22追記が挙げた欠落(1)（I2Cを採る決定がどの正本にも無い）を埋めた。**2026-09-24にユーザーがI2Cの採用を承認した**ことを日付つきの追記で残し、[hardware-bom.md](hardware-bom.md)と[tbd-register.md](tbd-register.md)の`HW-TBD-005`にも同じ決定を記録した。**欠落(2)は判定していない。**あわせて、`closeまでに要ること`表の#3〜#5へ、`EXP-015`とADXL345側のpin接続の確定（2026-08-27）より前の記述であることを日付つきで追記した。**表の状態列の判定は変えていない。既存の文言は書き換えていない** |
