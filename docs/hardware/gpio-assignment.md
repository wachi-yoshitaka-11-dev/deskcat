...
| 2026-09-23 | 39 | [#13](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/13)。**`電源pinの短絡・誤配線の確認（非通電）`表の項目4（給電経路の重複確認）を更新した。**旧記述は「採用済みの経路はB-2b」と、`DISP-01`の給電経路をB-2b（外部の3.3 V電源）だけに固定して書いていたが、[#461](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/461)（2026-09-23）でESP32自身の`3V3` pinからの通常接続も認められたため、この記述は経路を1つに限定できなくなった。**判定基準（単一経路であること）自体は変えず**、給電元がB-2bか`3V3` pinかで参照先の節が変わることを明記し、[power-budget.md](power-budget.md)の新設節（`DISP-01`単体bring-upの手順、`#461`承認範囲）へ相互参照を足した。**項目2・3（一覧照合、逆極性確認）はいずれの経路でも判定内容が変わらないため触っていない。****GPIO割り当て・他項目の判定基準は変えていない** | [#461](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/461)、[power-budget.md](power-budget.md)の`DISP-01`単体bring-upの手順（ESP32`3V3` pin給電、`#461`承認範囲） |
