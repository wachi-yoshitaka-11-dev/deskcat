...
| 2026-09-25 | 42 | [#446](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/446)のPR B。`ESP32の使用制限pin`表の`UART0`行と`信号inventory`の`UART-RX`行が、「このfirmwareは受信loopを実装していない」「RXを読まない」と書いていたが、`pi-protocol-mode`が`boot`のACK受信にRXを読む実装（`crate::boot_session`）が入ったため偽になっていた。既定buildはRXを読まないままであることは変えず、`pi-protocol-mode`だけの挙動として書き分けた。**GPIO割り当てそのものは変えていない。実機での受信確認（software ring buffer溢れの有無、`boot`→ACK成立）はまだ無い** | [#446](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/446)、`firmware/esp32/src/boot_session.rs` |
