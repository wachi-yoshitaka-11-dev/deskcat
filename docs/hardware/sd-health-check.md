...
| 項目 | 状態 |
|---|---|
| `manfid`＝`0x1b`＝Samsung | **一次資料では解けない。**仕様書はMIDを「SD-3C, LLCが管理・定義・割り当てる」8 bitの番号と規定するのみで、**どの番号がどのメーカーかという登録簿を収録していない**（Section 5.1）。**2026-08-15に確認したところ、SD-3Cも登録簿を公開していない**（[Licensees](https://www.sd-3c.com/Licensees.aspx)はライセンシー名の一覧であり、MIDとの対応表を持たない）。**したがって「別の一次資料を当たる」という解決経路は存在しない。**詳細は下記 |

**MIDの対応表は公開されていない**（2026-08-15確認）。次の2点を当たった。
...
**したがって`manfid`から社名を引く一次資料は存在しない。**「別の一次資料を当たれば解ける」
という性質の未確定ではないため、**この経路の追跡を打ち切る。**
...
| `manfid`＝`0x1b`がSamsungであることの確認 | **一次資料では解けない** | 仕様書もSD-3Cも**MIDと社名の対応表を公開していない**（2026-08-15確認）。**「別の一次資料を当たれば解ける」ものではない。****`SD-01`のメーカー表示の正は引き続き現物printである** |
...
| 2026-08-15 | 12 | **`manfid`の追跡を打ち切った。**Revision 3以来「別の一次資料が要る」としていたが、**2026-08-15に[SD-3C Licensees](https://www.sd-3c.com/Licensees.aspx)を確認したところ、同社もMIDと社名の対応表を公開していない**（ライセンシーの社名一覧のみ）。仕様書Ver 9.10も対応表を収録していない。**したがって`manfid`から社名を引く一次資料は存在せず、「別の一次資料を当たれば解ける」という性質の未確定ではない。**状態を`未照合`から`一次資料では解けない`へ改めた。**非公式の実測集計が`0x1b`＝Samsung・OEM ID `534d`とし本個体の値と一致するが、著者自身が非公式と明記しているため根拠に採らない。****`SD-01`のメーカー表示の正は引き続き現物printである** | [SD-3C Licensees](https://www.sd-3c.com/Licensees.aspx)、[Part 1 Physical Layer Simplified Specification Version 9.10](https://www.sdcard.org/downloads/pls/) Section 5.1 |
