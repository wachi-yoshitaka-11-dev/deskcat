...
| 定格電圧範囲 | 4.8–6 V | **Digital側ページのQ&A欄にある`admin`（メーカー側）の回答（2021-11-07）にのみ由来する。**仕様表の記載は`Operating voltage: 4.8v`だけで、Analog側ページは`Reviews (0)`である。**どちらの品かが未確定であり、variantに依存しない定格として扱わない**（2026-08-24。正はBOM。`TBD`へ戻すかは[tbd-register HW-TBD-026](tbd-register.md)経由で人間の判断） |
