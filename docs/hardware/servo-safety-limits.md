...
| 定格電圧範囲 | 4.8–6 V | **Digital側ページのレビュー欄にのみ由来する。**仕様表の記載は`Operating voltage: 4.8v`だけで、Analog側ページのreview欄は`There are no reviews yet.`である。**どちらの品かが未確定であり、variantに依存しない定格として扱わない**（2026-08-24。正はBOM。`TBD`へ戻すかは[tbd-register HW-TBD-026](tbd-register.md)経由で人間の判断） |
| 無負荷電流 | **TBD** | 測定（**一次資料に記載が無い**。正はBOM） |
| 動作電流 | **TBD**（0.5–2 Aと負荷依存で幅が広い。**`データシート値`という表記を2026-08-24に外した。**この値もDigital側ページのレビュー欄にのみ由来する（Analog側にreviewは無い）。**値は変えていない。**確定は`HW-TBD-010`／`011`の範囲である） | 測定（**一次資料に記載が無い**。正はBOM） |
...
- [ ] **（Blocked）**Pi入力の電圧droopが許容範囲内であることを確認した。**現状はこの項目を合格にできない。**Raspberry Pi Zero Wには低電圧検出回路が無く（[power-budget.md](power-budget.md)の`Pi Zero Wには低電圧検出が無い`）、判定は`ADC-5V`の実測に依るが、**(i) 判定に使う最低入力電圧が`HW-TBD-028`(a)として未確定であり、(ii) `ADC-5V`の測定系が未実装である（**分圧抵抗は入手済み。実装と検証が残る**。2026-08-12に購入履歴と照合して訂正した）。****両方が揃うまでこの項目はBlockedであり、「確認した」と記入しない**（[tbd-register HW-TBD-028](tbd-register.md)、[hardware-bom.md](hardware-bom.md)の`MEAS-01`）
