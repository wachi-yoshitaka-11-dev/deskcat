...
- [ ] **peak時**の5 V／3.3 Vの電圧droopを測定し、**ESP32の**brownoutとresetが起きないことを確認した（peakはこの確認にのみ使う）。**Piについては同じ確認ができない。**Raspberry Pi Zero Wには低電圧検出回路が無いため、Pi側の判定は`ADC-5V`で測ったdroopそのもので行う（[power-budget.md](power-budget.md)の`Pi Zero Wには低電圧検出が無い`）
...
| 2026-08-12 | 7 | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3)。受け入れchecklistの電圧droop項目が、**Piのbrownoutをsoftwareで検出できることを暗に前提としていた。**[Raspberry Pi公式documentation](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html)の`Power supply warnings`は、低電圧検出回路が`On all models of Raspberry Pi since the Raspberry Pi B+ (2014) except the Zero range`にあると明記しており、**`SBC-01`のRaspberry Pi Zero Wはこの`Zero range`に該当する。**同項目を「**ESP32の**brownoutとresetが起きないこと」に限定し、Pi側は`ADC-5V`で測ったdroopそのもので判定する旨を追記した。**ESP32は自身でbrownoutを検出するため、ESP32についての確認は変えていない。**判定に使うPi入力の最低電圧そのものは`HW-TBD-028`(a)として未確定である（一次資料に無いことを2026-08-12に確認した）。**サーボの動作制限も出力有効化のgateも変えていない** |
