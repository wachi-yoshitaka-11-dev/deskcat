...
| 定格電圧範囲 | 4.8–6 V | **Digital側ページのQ&A欄にある`admin`（メーカー側）の回答（2021-11-07）にのみ由来する。**仕様表の記載は`Operating voltage: 4.8v`だけで、Analog側ページは`Reviews (0)`である。**どちらの品かが未確定であり、variantに依存しない定格として扱わない。****そして`PSU-SERVO-01`が供給する5 Vは、仕様表の`4.8v`より上である。****5 Vを入れてよいかは[tbd-register HW-TBD-035](tbd-register.md)で追跡する。この表では決めない**（2026-08-24。正はBOM） |
