...
3. **PMがこの3点についてユーザーへ再確認を求めており、2026-09-22時点で結果は
   未着である。**結果が「進めてよい」でない限り、`gate確認`は実行の承認を出さない。
   結果が「進めてよい」であれば、その旨と日付をこの節へ追記し、以後は1〜3を
   実行可能な承認として扱う。
...
gate表のいずれの行も解決していない。承認は本手順1回分（単発command、繰り返しなし）
に限る。動かなかった場合、再試行する場合は、そこで止めて改めてこの節の状態を
更新する。
...
- **手順3のうち「安全な電流制限」:** 満たさない。詳細は下記残余riskの
  「§6が要求する『電流制限を設定でき十分な定格を持つ電源』は、この試験にも無い」を参照する。
...
- [Hardware Safety Policy §6](../governance/hardware-safety-policy.md#6-サーボ)は、
  ...
  width要件の確認、hornまたは機械負荷の除去、電流制限を設定でき十分な定格を持つ
  電源の使用、狭いpulse範囲と可動域の設定、直ちに操作できる電源遮断手段、手や
...
- **§6が要求する「電流制限を設定でき十分な定格を持つ電源」は、この試験にも無い。**
  ...
  形は完全に同一ではないが、**どちらも「電流制限を確立していない」という点で
  §6が要求する水準に届いていない。**）**LCDはこの理由で段階B-2bを止めているのに
...
6. **`起動とarm delay`（人間）** ESP32を起動する（flash直後）。`espflash`のmonitorで
   ...
   定義された上限は無い**（`run_i2c_bringup`のI2C呼び出しが無期限timeoutを使うため。
   `main::run_i2c_bringup`のdoc参照）。**明らかに出ない（他のbring-up logより
   大幅に遅い、または出ないまま止まって見える）場合は、異常（I2C bus固着の疑い）
   として扱い、servoの電源を入れずに報告する。**
