...
| `HW-TBD-027` | `SERVO-PWM`の外部pull-down（`RES-PULL-01`）の抵抗値と本数 | GPIO27はreset時にhigh-Zであり、pull-downが実装されるまでPWM driver初期化前のservoの動きを止められない。`HW-TBD-019`の起動時状態は、この部品が決まらないと確定できない |
