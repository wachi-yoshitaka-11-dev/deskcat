...
| ACCEL-01 | 軽打／持ち上げ／姿勢 | ADXL345モジュール（秋月 M-06724） | Selected | Analog Devices ADXL345。I2C／SPI選択式、tap／double-tap／free-fall検出をhardware内蔵 | I2CまたはSPI（3線式／4線式）、選択式 | Address等はTBD（現物のjumper／pin設定確認要） | VDD 2.0–3.6V（別途VDD_IO）。**M-06724はregulator非搭載のため3.3V直結必須、Logic 5V railへは直結しない**（最終構成ではESP32 board上の3V3 pinから給電。**段階B-2の測定中に限り外部3.3 V電源から給電する経路（B-2b）がある**。配線規則の正本は`power-budget.md`の`段階B-2の測定`であり、ここには再掲しない） | [Analog Devices ADXL345製品ページ](https://www.analog.com/en/products/adxl345.html)、[ADXL345 Data Sheet（メーカー公式PDF）](https://www.analog.com/media/en/technical-documentation/data-sheets/adxl345.pdf) | DeskCatの軽打／持ち上げ判定にtap／free-fall検出hardwareが適合するため採用決定。KXR94-2050（同時購入、秋月）は不採用・spare保管。Interface選択jumperと実装済みaddressは現物確認が必要。**公式文書欄を訂正した**（2026-08-10）。従来はDigi-Keyの解説記事だけを載せており、メーカーのdatasheetをリンクしていなかった。**ただし`analog.com`へは本作業環境から接続できず（WebFetch／curl／PowerShellのいずれもECONNRESETまたはtimeout）、datasheetのrevisionと内容は未検証である。**また**秋月 M-06724の商品ページは現在404**（`gM-06724`／`g106724`とも）であり、「regulator非搭載」の根拠資料へアクセスできない。moduleにregulatorが載っていないことは現物で確認する |
