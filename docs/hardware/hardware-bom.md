...
- [ ] 供給電圧とlogic電圧が明確である
- [ ] **Board上regulatorの種別（LDO／switching）と、`3V3` pinが外部へ供給できる電流の定格を確認した**（`MCU-01`のみ。**定格の確認は[power-budget.md](power-budget.md)の段階B-2を実施してよいかのgateである**。PC hostのOCPはUSB入力を制限するだけで`3V3` branchを守らず、先に壊れうるのはboard上のregulatorである。種別は3.3 V側の実測値を5 V側へ換算する根拠にも使う。switchingなら`Iin ≒ Iout + Iq`が成り立たない）
