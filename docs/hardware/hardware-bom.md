...
| SD-01 | Piのbootとstorage | Samsung EVO Plus microSDHC 32GB | Selected | Samsung。Speed Class: UHS Speed Class 1（`U1`表示） | **現行lineupに該当なし。**Samsungが公開する[EVO Plus microSD data sheet](https://download.semiconductor.samsung.com/resources/data-sheet/2023_samsung_data_sheet_microsd_card_evo_plus_rev.1.0.pdf)（2023年版 Rev 1.0）は**64GB／128GB／256GB／512GBのmicroSDXCのみ**を対象とし、32GB microSDHCを含まない。**この資料を現物の根拠に使わない** | SBCから供給 | TBD | カード本体のprint（`SAMSUNG EVO Plus 32 microSDHC U1`）で確認済み。Piのslotは空だったため、この手持ちcardを新たに使用する。使用前の健全性（health）check未実施。**32GB microSDHC版は旧モデルであり、現行の公式資料から仕様を引けない。**識別は現物printを正とする（2026-08-10確認） |
...
- [ ] 供給電圧とlogic電圧が明確である
- [ ] **周辺module3点（MSP2807、ADXL345、BME280）がmoduleとして受け入れてよい電圧の積集合とESP32のlogic levelから、3.3 V railの許容電圧範囲を確定した**（[power-budget.md](power-budget.md)の段階B-2の**共通**条件。**給電経路に関わらず必要**であり、B-2bを選んでも省略できない。追跡は[tbd-register HW-TBD-025](tbd-register.md)）。**積集合を取る対象はIC単体の動作範囲ではない。ADXL345分は`HW-TBD-004`が解けるまで確定しない**（`moduleへ入れてよい電圧`が未確定であるため）
