...
- [ ] 供給電圧とlogic電圧が明確である
- [ ] **周辺module3点（MSP2807、ADXL345、BME280）の動作電圧範囲の積集合とESP32のlogic levelから、3.3 V railの許容電圧範囲を確定した**（[power-budget.md](power-budget.md)の段階B-2の**共通**条件。**給電経路に関わらず必要**であり、B-2bを選んでも省略できない）
- [ ] **周辺module3点それぞれについて、耐えられる電流の上限を持っている**（同じく**共通**条件。**MSP2807は未公開のため現状は無い。**現物のbacklight回路の確認か一次資料が要る）
