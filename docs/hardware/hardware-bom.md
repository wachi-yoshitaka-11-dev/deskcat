...
| ACCEL-01 | 軽打／持ち上げ／姿勢 | ADXL345モジュール（秋月 M-06724） | Selected | Analog Devices ADXL345。I2C／SPI選択式、tap／double-tap／free-fall検出をhardware内蔵 | I2CまたはSPI（3線式／4線式）、選択式 | Address等はTBD（現物のjumper／pin設定確認要） | `VS` 2.0–3.6V（interface電源は別系統の`VDD I/O`）。**Logic 5V railへは直結しない。これは確定した規則である。ただし根拠はIC定格からの導出ではなく、確認前の安全規則である**（moduleがregulatorを持たない場合、5VはICへ直接掛かって定格上限3.6Vを超える。その可能性を否定できないため禁止する。**IC定格からmodule boardの許容入力電圧は決まらない**）。**給電は3.3Vを唯一の候補とするが、まだ確定していない。**IC定格と整合する候補はこれだけだが、**moduleがその電圧を受け入れることの根拠は別に要る**（module boardの資料が入手できず、`moduleへ入れてよい電圧`は未確定である。[tbd-register HW-TBD-004](tbd-register.md)、[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`module boardの値`）。**「M-06724はregulator非搭載」という前提の根拠資料は現在アクセスできない**（秋月 商品ページが404。2026-08-10確認）ため、**regulatorの有無は現物で確認する**（[tbd-register HW-TBD-004](tbd-register.md)）。仮に搭載されていても5V直結の判断は変わらないが、moduleへ入れてよい電圧の上限はIC定格と同一とは限らない（**3.3Vで確定した場合の**最終構成ではESP32 board上の3V3 pinから給電する。**段階B-2の測定中に限り外部3.3 V電源から給電する経路（B-2b）がある**。配線規則の正本は`power-budget.md`の`段階B-2の測定`であり、ここには再掲しない） | [Analog Devices ADXL345製品ページ](https://www.analog.com/en/products/adxl345.html)、[ADXL345 Data Sheet（メーカー公式PDF）](https://www.analog.com/media/en/technical-documentation/data-sheets/adxl345.pdf) | DeskCatの軽打／持ち上げ判定にtap／free-fall検出hardwareが適合するため採用決定。KXR94-2050（同時購入、秋月）は不採用・spare保管。Interface選択jumperと実装済みaddressは現物確認が必要。**公式文書欄を訂正した**（2026-08-10）。従来はDigi-Keyの解説記事だけを載せており、メーカーのdatasheetをリンクしていなかった。**ただし`analog.com`へは本作業環境から接続できない**（WebFetch／curl／PowerShellのいずれもECONNRESETまたはtimeout。2026-08-12の再試行でもtimeout）。**内容は[SparkFunがhostするRev. 0版](https://www.sparkfun.com/datasheets/Sensors/Accelerometer/ADXL345.pdf)を2026-08-12に開いて確認した**（読み取った範囲は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`ICの値`）。**mirrorであるため公式配布物との同一性は確認しておらず、Rev. 0が現行revisionである保証もない。**また**秋月 M-06724の商品ページは現在404**（`gM-06724`／`g106724`とも）であり、「regulator非搭載」の根拠資料へアクセスできない。**regulatorの有無は現物で確認する**（載っていないことを前提にしない） |
...
- [ ] **周辺module3点それぞれについて、耐えられる電流の上限を持っている**（同じく**共通**条件。追跡は[tbd-register HW-TBD-025](tbd-register.md)。**MSP2807は未公開のため現状は無く、その分は`HW-TBD-024`**。現物のbacklight回路の確認か一次資料が要る。**ADXL345とBME280についても、datasheetに公開値があるかは未確認である**）
