...
| HW-TBD-015 | P1 | **残: 耐久性の確認と、書込み速度がClass表示に達しない原因の切り分け。****使用前のhealth checkは2026-08-12に実施済みである**（判定`Partial`。[sd-health-check.md](sd-health-check.md)）。29.80 GiB全域へ位置依存dataを書いて読み戻し`Data LOST`は0 sectorであった（容量詐称なし、検査範囲に読み書き不良なし）ため、**`妨げる対象`のうちDeploy側は解けた。**識別情報はSamsung EVO Plus microSDHC 32GBと現物print確認済みで、card内部register（`manfid`／`oemid`／`name`／`csd`／`scr`／`ssr`）を読み、2026-08-13に読み方をPhysical Layer Simplified Specification Ver 9.10と照合した。**`32`／`microSDHC`／`U1`はregisterがprintを裏付けたが、`Samsung`だけは裏付けられない**（仕様書がMIDの登録簿を収録していないため）。**メーカー表示の正は引き続き現物printである。****残る2点**: (a) 耐久性は1回のcheckでは判定できない。(b) 書込み速度6.11 MB/sが`U1`の10 MB/sに達しておらず、**cardとhost controllerのどちらが効いているかを分離していない**（測定は`sd high-speed`で行われ、`U1`の規定が適用されるUHS modeではない）。**file systemは主因でないと確定した**（raw deviceへの直接書込みでも7.4 MB/s） | (a) 耐久性: 使用中のlog。(b) 速度: **別のhost controllerで同じcardを測る。**file system分の切り分けは実施済み。実施済み分の根拠: [sd-health-check.md](sd-health-check.md)、識別の根拠: [hardware-bom.md](hardware-bom.md) SD-01 | 耐久性（**Deployは解決済み**） | [#114](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/114) | Human | Open（範囲をさらに縮小） |
