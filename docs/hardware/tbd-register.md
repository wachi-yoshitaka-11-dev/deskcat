...
| HW-TBD-021 | P0 | **5 V ingressの過電流保護部品（`PROT-OC-01`）の選定とtrip値。**M-12001は3 Aを供給でき、候補構成で定格が判明している部品のうち最小は1.5 Aである（HW-TBD-022が解決するまで、これが真の最小値である保証はない）。現状は「上限を超えたらテスターの読みで人が電源を落とす」だけであり、connectorと線材が発熱する前に電流を止める手段が無い | メーカーの時間-電流特性（一次資料）に基づく選定。選定基準・挿入位置・上限との関係は[power-budget.md](power-budget.md#過電流保護段階cのgate) **2026-08-22に、部品が満たすべき数値の窓と、窓が閉じる境界を計算した。選定はしていない。**手順1〜3の入力は既存の表から揃うため計算できる。**MSP2807を除いた定常電流の合計から下限が出て、判明している最弱部品の定格が上限になる。****MSP2807の定常電流が一定値を超えると窓が閉じ、両方を満たす保護部品が存在しなくなる。**そのときは保護部品の選定の問題ではなく**経路の定格が足りないという結論**である（値と限定の正は[power-budget.md](power-budget.md)の`窓が閉じる境界（2026-08-22の計算）`。**ここへ再掲しない**）。**方式（PTC／ヒューズ）は未決定であり、これは人間の判断である。****上限も確定値ではない**（`経路部品と定格`に未確定の行が残っており、より低い定格が現れれば窓は狭くなる） | **合成給電（段階C）**。保護部品の選定・実装まで実施しない | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**2026-08-22に窓と境界を計算した。選定も方式の決定もしていない。****この境界は段階B-2でのMSP2807の測定に対する合否の境界になる。**超えた場合は保護部品の選定へ進む前に、経路の変更か`servo-safety-limits.md`の制限の締め直しを決める。**方式の決定は人間の判断である。**残るのは(1) 方式の決定、(2) `HW-TBD-022`（経路の最小定格の確定）、(3) 品番と数値の一次資料（時間-電流特性）である） |
| HW-TBD-022 | P0 | **大電流経路の線材・接続部材（`WIRE-PWR-01`）と`CABLE-PI-PWR-01`の選定と許容電流。**（`CABLE-PI-PWR-01`は[power-budget.md](power-budget.md#経路部品と定格)で同じ規則の適用対象であり、2026-08-11の照合でこの行に含めることを明示した。）手持ちのbreadboardとジャンパー線は個別の許容電流がメーカー資料で確認できず、経路の最小定格を出せない。最小定格が出ないとingressの上限（経路部品の定格の最小値の80%）が確定しない | 公称許容電流が公開されている線材・接続部材の選定。決定の根拠は[power-budget.md](power-budget.md#経路部品と定格) **2026-08-22に選定規則を評価可能な形にした。選定はしていない。**販売元が公開しているのは住友電工の`許容電流計算式`（式と材料別の係数表）であって数値ではない。**式の入力のうち`r0`（20 ℃における導体抵抗の規格値）だけが電線側の仕様から来るため、`r0`が公開されていない製品では式を評価できない。**候補として見た秋月の`UL1015 AWG18`は`導体抵抗`も`許容電流`も公開していない。**第三者の公開値は18 Aと19.2 Aで食い違い、条件の記載も揃っていない。****したがって定格として採らない。**詳細は[power-budget.md](power-budget.md)の`線材の許容電流をどう確定させるか（2026-08-22の調査）`。**ここへ再掲しない** | ingressの上限の確定、**合成給電（段階C）** | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**2026-08-22に選定規則を評価可能な形にした。選定はしていない。**残るのは(1) 許容電流か`導体抵抗`を**製造者が公開している**線材の選定、(2) 分岐点の接続部材（**未調査**）、(3) `CABLE-PI-PWR-01`（**未調査**）、(4) GND戻り経路である。**AWG18級なら経路の既知の最小値1.5 Aより一桁大きいことは分かったが、これは非メーカー値との比較であり定格の確定ではない。**`経路部品と定格`の表の`TBD`は埋めていない） |
