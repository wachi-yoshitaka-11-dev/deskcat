...
|---|---|---|---|---|---|---|---|
| HW-TBD-004 | P0 | **2026-08-13の現物確認で次が確定した。**(1) **IC刻印は`345B` / `#727` / `750B` / `PHIL`**であり、`345B`はAnalog DevicesのADXL345の刻印である。**これによりICの根拠が、silkと購入履歴からIC自身の刻印になった。**(2) **regulatorは非搭載**（IC＋`C1`×2＋抵抗のみで、3端子部品が無い）。(3) **裏面の半田ジャンパ2箇所はいずれもオープン。**(4) **搭載pull-upは`01C`（EIA-96で10 kΩ、1%）が4個**、ほかに`R2`＝`0`（0 Ωジャンパ）1個。**どのlineへ何個付くかは2026-08-27に現物写真でパターンを追って確定した。**値と判定方法の正は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`Module搭載pull-up`。**ここへ再掲しない。**(5) pin列は上`CS Vs GND VDD`、下`INT1 INT2 SDO SDA SCL`で、**`Vs`と`VDD`が別pinに出ている。****ただしこれはheaderのラベルであって、board上の配線ではない。****2026-09-05の非通電導通測定で、header `Vs`ピンと`VDD`ピンの間が導通している（同一net）ことを確認した。**したがって「2系統電源を個別に受ける設計」ではない。**ただし、この共通netがICの`VS`／`VDD I/O`それぞれへ直結しているか、個別に直列抵抗が入るかは、IC足が小さくプローブを直接当てられずパターンを追っていないため未解決のまま残る。**接写写真では、header〜IC間に`Vs`側・`VDD`側それぞれ1個ずつ、コンデンサ（`C1`）が見える位置にあり、目視ではIC方向へ直接パターンが伸びているように見えるが、断定はしない。(6) local decouplingは`C1`が2個実装されているが、**容量表記が無く値は不明**。**残: 実装済みI2C addressと`moduleへ入れてよい電圧`。**addressは`SDO`の接続で決まるが**基板上で固定されておらず、配線時に決まる**。`moduleへ入れてよい電圧`はregulator非搭載が確定したため**ICの`VS`／`VDD I/O`がheaderへ直結している可能性が高いが、直列抵抗やlevel shiftの有無をパターンで追っていないため断定しない**。module／ICはADXL345（秋月 M-06724）と特定済み。**module boardの資料が現在入手できない**（秋月 M-06724の商品ページは`gM-06724`／`g106724`とも404。2026-08-10確認）ため、jumper構成をIC datasheetから推定せず現物で読む。**`moduleへ入れてよい電圧`は2026-08-12に明示追加した。**[hardware-bom.md](hardware-bom.md)と[power-budget.md](power-budget.md)が3.3V給電を確定形で書いていたが、**ICの定格3.6Vはboardの許容値ではない**（regulatorやlevel shiftの有無で変わる）ため、3.3Vを唯一の候補としたうえで確定はこの行に依存させた。**5V直結の禁止は別の主張として維持する。**これは「moduleがregulatorを持たなければ5VがICへ直接掛かる」ことを否定できないための**確認前の安全規則**であって、IC定格からmoduleの許容入力電圧を導いたものではない | 現物のjumper／address pin設定の確認、基板上のregulatorの有無の確認、moduleへ入れてよい電圧の確定、および**board上に実装済みのdecoupling capacitorの確認**（`HW-TBD-029`から2026-08-12に移した。ICへの指定値は同行のcloseで確定しているが、**外付けが要るかはboardの実装でしか決まらない**。値の正は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`Local decoupling`節）。ICの根拠は[Analog Devices ADXL345 datasheet](https://www.analog.com/media/en/technical-documentation/data-sheets/adxl345.pdf)（**Rev. G。2026-08-12にブラウザで取得して確認した**。**「`analog.com`へは接続できない」という旧記載は誤りであったため同日訂正した**。到達できないのはCLI clientだけである。読み取った内容は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`ICの値`）。**ただしdatasheetを得てもこの行は解決しない。**jumper構成、実装済みaddress、regulatorの有無、moduleへ入れてよい電圧は、いずれもICの資料では決まらない。特定の根拠: [hardware-bom.md](hardware-bom.md) ACCEL-01 | Accelerometer driverとしきい値、**ADXL345への給電電圧の確定**。**残る「moduleへ入れてよい電圧」は`HW-TBD-025`(a)（#3の電源architecture）が待つ値でもあるため、2026-09-06に`#3`を対応Issueへ加えた**（`HW-TBD-023`／`HW-TBD-024`と同じ理由。判定は必要な根拠列が求める作業の範囲で行い、題名や領域の近さでは判定しない）。**I2Cアドレスは配線時に決まる項目であり、現時点でIssueの担い手を要さない** | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)、[#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（範囲を縮小） |
| HW-TBD-005 | P0 | **実装状態の確定（jumperの開放・アドレス未固定）は解決したが、closeに要る残りの実装・判断・動作確認は解決していない。**（2026-09-06判定。「残件は解決した」という記述はこの列の先頭に残っていたが、`状態`列が示す通り`J3`のはんだ付け・`J1`／`J2`の判断・アドレス配線・動作確認が未実施であり、実態と矛盾していたため訂正した）。`J1`／`J2`／`J3`は**3つとも開放**である（2026-08-22に導通確認で実測。**電源を接続しない状態でpin間の抵抗を測った。**測定値と判定方法の正は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`現物の実装状態を実測で確定させた（2026-08-22）`。**ここへ再掲しない**）。**説明書が示す工場出荷状態と一致する。**2026-08-13の接写では光学的に判別できず（パッドが錫めっきで鏡面）、**「全てオープンである可能性が高いが断定しない」としていた点が実測で決まった。****`SDO`が基板上でどこにも固定されていないことも実測で裏付けた。**したがってI2Cアドレスは配線時に決まり、基板側に既定値は無い。module／ICはBosch BME280（秋月 K-09421）と特定済み。**jumperの意味とaddressの決まり方は一次資料で確定している** | **[AE-BME280製品説明書 v1.1](https://akizukidenshi.com/goodsaffix/AE-BME280_manu_v1.1.pdf)（2015-06-02）で確定**（2026-08-10）: `J1`＝I2C時のSDA用プルアップ(4.7kΩ)選択、`J2`＝同SCL用、`J3`＝**I2C時にはんだジャンパする**（SPI 4W／3W時は`J1`〜`J3`すべてオープン）。I2Cアドレスは`0x76`（`SDO`→GND、既定）／`0x77`（`SDO`→VDD）。6pin SIPは1=VDD, 2=GND, 3=CSB, 4=SDI, 5=SDO, 6=SCK。VDDとVDDIOは基板上で接続済み。Chip ID register `0xD0`、reset値`0x60`。ICの根拠は[Bosch BME280 datasheet Rev 1.24](https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bme280-ds002.pdf)。特定の根拠: [hardware-bom.md](hardware-bom.md) ENV-01 **判定は2026-08-22に完了した。**導通確認はDT830Bの`Ω`の`20k`レンジで行い、**ゼロ点を確認したうえで測った。** **2026-08-22に、closeまでに要ることを実装と確認まで含めて確定した。****この行はそれまで一次資料の内容しか持たず、実装作業を含んでいなかった。**`HW-TBD-032`が選定・購入・実装・確認を`必要な根拠`に含めているのと構造を揃える。**closeには5項目が要る**（実装状態の確定、`J3`のはんだ付け、実効pull-upの計算と`J1`／`J2`の判断、I2Cアドレスの配線、実装後の動作確認）。**内容と状態の正は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`closeまでに要ること（実装と確認を含む）`であり、ここへ再掲しない。****済んでいるのは実装状態の確定だけである。** | Environment driverとbus計画。**残る`J1`／`J2`のpull-up判断とI2Cアドレスの配線はGPIO割り当ての決定であり、`HW-TBD-027`／`HW-TBD-032`と同じ理由で2026-09-06に`#2`を対応Issueへ加えた**（`#1`のcloseで担い手が居なくなることを防ぐため） | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)、[#2](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/2) | Human | Open（**残件の`J1`／`J2`／`J3`の半田の有無は2026-08-22に実測で確定した。3つとも開放である。****この状態から実装作業と未決の判断が出る。****(1) I2Cで使うには`J3`のはんだ付けが要る**（説明書が定める。開放では`CSB`が`VDD`へ繋がらない）。**(2) module搭載の4.7 kΩプルアップは繋がっていないため、busのpull-upをこのmoduleから供給しない。**`J1`／`J2`をはんだ付けするかは、[gpio-assignment.md](gpio-assignment.md)の`I2C sensor bus`で**bus全体の実効pull-upを計算してから決める。まだ決めていない。****(3) アドレスは配線で決める**（`SDO`→GNDで`0x76`、`SDO`→VDDで`0x77`）。**通電はしていない。****closeの手順（正本文書への反映と関連項目の解除）を通すまで行は残す**。**2026-08-22にcloseまでに要る5項目を確定した**（正は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)の`closeまでに要ること（実装と確認を含む）`）。**済んでいるのは実装状態の確定だけである。****`J3`のはんだ付けがI2Cで動かすための必須作業であり未実施。****`J1`／`J2`の判断はADXL345側のpin接続の確認が先に要るためまだできない。****2026-08-25に、計算の式と前提を一次資料（I2C-bus specification UM10204 Rev. 7.0 §7.1）から確定させ、[gpio-assignment.md](gpio-assignment.md)の`I2C busの実効pull-up`節へ置いた。****式が無いことは、もう理由ではない。****さらに2026-08-26に、I2Cのpull-up値とaddressは一般値で開始してよい側と定まった**（[hardware-safety-policy.md](../governance/hardware-safety-policy.md)の対応表。ADR-0014／0016）。**したがって一次資料からの導出も、値の確定も、着手の前提ではない。**外れてもbusが応答しないだけで、安全要件5項目のどれにも当たらない。**残る前提は`J1`／`J2`をはんだ付けするかの判断だけであり、それは正本が「ADXL345側のpin接続の確認が先に要る」と定めている。**参考として、値が決まらない入力は3つある（ADXL345側のpin接続、bus容量`Cb`、採るmode。**`検証済み最大bus速度`は`TBD`である**）。**同節は`Rp`の候補値ごとに許される`Cb`の上限も示しており、確定した時点でそのまま判断に使える。****アドレスは同日に判断材料を揃えた**（ADXL345の候補`0x1D`／`0x53`とBME280の候補`0x76`／`0x77`は**全4通りで衝突しない**ため、**選択は衝突回避では決まらない。**正は同文書の`I2C addressの選択`節）。**どちらを採るかの決定と配線は未実施。****動作確認も未実施**） |
| HW-TBD-007 | P0 | **残: 5 V ingressの変換部品の購入と、connector定格の検証。**電源modelはM-12001（5 V／3 A）と確定し、現物も2026-08-08に着荷済み。ただしMicro-Bオスplugをbreadboardへ引き込む変換部品が未購入である（候補: 秋月 g110972、定格1ピン1.5 A）。上限は経路上の全部品の定格の最小値の80%（**値の正は[power-budget.md](power-budget.md)の`判明している最小定格が1.25 Aへ下がった（2026-08-25）`であり、ここへ再掲しない。**未確定の経路要素が残るため確定値ではない。HW-TBD-021／022）であり、**判定は定常電流で行う**（connector定格は熱の制限のため）。**段階A（Pi単体起動）と段階B-1（ESP32をPCのUSBから給電）はこの行に依存せず進められる。ただし段階Aは探索的な通電であって、Piの電源経路の受け入れではない。**M-12001の5 VがRaspberry Pi公式の要求する5.1 Vを満たすかはこの行で未判断であり、判定に使う最低電圧も`HW-TBD-028`(a)として未確定である。**段階Aが正常に起動したことをもって合格としない。****段階B-2（周辺module3点から3.3 V側の定常電流を測る）も同様だが、B-2自体が共通条件のHW-TBD-025と、経路別のHW-TBD-023（B-2a）またはHW-TBD-024（B-2b）にBlockedされている。**なお**合成給電（段階C）はこの部品が要る。****2026-08-12に電源modelそのものについて1件判明した。**Raspberry Pi公式documentationは全modelが**5.1 V**供給を要求すると述べているが、選定済みのM-12001は**5 V**である。**この差の可否はまだ判断していない。**判定に使う最低電圧が`HW-TBD-028`(a)として未確定であり（一次資料に無いことを確認済み）、**電圧の余裕を評価する根拠が無い**ためである。**部品の選定は変更していない**。**2026-08-27の追記。**`PROT-OC-01`をPTCへ変更した経緯で、`HW-TBD-021`の詰まりの本体は保護部品の性能ではなく、この行が持つ`Micro-Bオスplugとreceptacleの嵌合接点`（変換基板側1ピンあたり1.5 A）であると判明した（正は[power-budget.md](power-budget.md)の`3 Aでのトリップ時間を読み、線材と嵌合接点の判定で止まった（2026-08-27）`。**ここへ再掲しない**）。**確認項目を1つ足す。**変換基板（秋月`110972`、`AE-MRUSB-DIP`。DIP化キット）は`VBUS`をブレイクアウトする複数pinを持ちうる。**`VBUS`を複数pinへ渡して並列に受ければ、実効的な定格が1ピンあたり1.5 Aより上がる可能性がある。****着荷後、現物のpin配列とパターン（`VBUS`が何本のpinへ出ているか、pin間がボード上で導通しているか）を確認する。**非通電で行える。この確認は`HW-TBD-021`の詰まりを解く経路の1つであり、確認結果は`power-budget.md`の`経路部品と定格`へ反映する | 変換部品の選定＋実測。確定済み部分の根拠: [hardware-bom.md](hardware-bom.md) PSU-PI-01／PSU-SERVO-01、[power-budget.md](power-budget.md#5-v-ingress物理的な引き込み経路)。**5.1 Vと5 Vの差についての根拠**: [Raspberry Pi公式documentation](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html)の`Power supply`（`All models require a 5.1V supply`。2026-08-12取得）。**同documentationは許容範囲を示さないため、5 Vが範囲内かどうかもこの資料からは決まらない** | 初回統合通電 | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（範囲を縮小。**2026-08-12に電源modelの供給電圧について未判断の論点が1件加わった**）**2026-09-08に`AE-MRUSB-DIP`のpin配列を現物確認した（非通電、テスターの導通チェック）。**引き出しpinは4本あり、うち2本（シルク印刷`+`側）が`VBUS`として導通で1グループ、残り2本（シルク印刷`-`側）が`GND`として導通で1グループ、receptacle金属シェルは`GND`側グループに含まれることを確認した。`VBUS`グループと`GND`グループの間には導通が無い（短絡していない）。**したがって`VBUS`は2本のpinへ出ている。**ただし、**この2本が基板上で導通していることと、外部配線を2本とも使えば実効的な電流定格が1ピンあたり1.5 Aより上がることは別である。**receptacle内部の配線（内部traceの幅・銅箔厚）はメーカーが公開しておらず、receptacleの内部端子そのものがボトルネックである可能性を排除できないため（[AGENTS.md](../../AGENTS.md) 推測禁止）。**したがってこの確認だけでは`HW-TBD-021`の詰まりは解けない。**確認結果は[power-budget.md](power-budget.md)の`経路部品と定格`へ反映する **2026-09-09に人間が決定した案B（`WORK-INSTRUCTIONS-INGRESS-PLAN-B.md`）により、この行の「connector定格の検証」の対象が変わる。**`PSU-INGRESS-01`（`AE-MRUSB-DIP`）は5 V ingressの経路から外れるため、**同品のconnector定格（1ピンあたり1.5 A）はもう経路の判定に効かない。****ただし`CABLE-PI-PWR-01`のPi側`PWR IN` receptacle定格（`経路部品と定格`表の該当行、定格`TBD`）はPi branch内にあり、ingressの経路変更の影響を受けずそのまま残る。**この行の追跡対象は今後こちらが中心になる。**cableはまだ切っていない（机上の反映のみ）。**あわせて、この行冒頭の「残: 5 V ingressの変換部品の購入」は案Bにより対象を失った。**案Bは端子台（`114217`、着荷済み）へcableを直接ネジ止めする構成であり、新たな変換部品の購入は不要である。**残るのは「connector定格の検証」（上記のとおり対象が`CABLE-PI-PWR-01`のPi側receptacleへ移った）だけである** |
...
| HW-TBD-021 | P0 | **5 V ingressの過電流保護部品（`PROT-OC-01`）の選定とtrip値。**M-12001は3 Aを供給でき、候補構成で定格が判明している部品のうち最小は**`PROT-OC-01`の定格電流である**（**2026-08-25に変換基板の嵌合接点から入れ替わった。****2026-08-26に方式がヒューズからPTC（Bourns `MF-R135`、`Ihold` 1.35 A）へ変更されている**（この行の状態欄の`2026-08-26の更新`が正。**ここへ再掲しない**）。値の正は[power-budget.md](power-budget.md)の`最小定格は1.35 Aへ上がった（2026-08-26）`。**ここへ再掲しない**。HW-TBD-022が解決するまで、これが真の最小値である保証はない）。現状は「上限を超えたらテスターの読みで人が電源を落とす」だけであり、connectorと線材が発熱する前に電流を止める手段が無い | メーカーの時間-電流特性（一次資料）に基づく選定。選定基準・挿入位置・上限との関係は[power-budget.md](power-budget.md#過電流保護段階cのgate) **2026-08-22に、部品が満たすべき数値の窓と、窓が閉じる境界を計算した。選定はしていない。**手順1〜3の入力は既存の表から揃うため計算できる。**MSP2807を除いた定常電流の合計から下限が出て、判明している最弱部品の定格が上限になる。****MSP2807の定常電流が一定値を超えると窓が閉じ、両方を満たす保護部品が存在しなくなる。**そのときは保護部品の選定の問題ではなく**経路の定格が足りないという結論**である（値と限定の正は[power-budget.md](power-budget.md)の`窓が閉じる境界（2026-08-22の計算）`。**ここへ再掲しない**）。**方式は2026-08-25にヒューズで決定し、品番も選定した**（[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)。**PTCを採らなかった理由と、同じ理由で2 A品も採れなかった経緯は同Issueの本文が持つ。ここへ再掲しない。**選定内容と一次資料値の正は[power-budget.md](power-budget.md)の`方式と品番を決定した（2026-08-25）`。**未購入である**）。**2026-08-26にこの決定は覆り、方式はPTCへ変更されている**（この行の状態欄の`2026-08-26の更新`が正。**ここへ再掲しない**）。**その結果、保護部品自身が経路の最小定格になり、窓とgate値が下がった**（正は同節と`判明している最小定格が1.25 Aへ下がった（2026-08-25）`。**値をここへ再掲しない**）。**上限も確定値ではない**（`経路部品と定格`に未確定の行が残っており、より低い定格が現れれば窓は狭くなる） **2026-08-22に境界の不等号を厳密不等号へ揃えた。**`方式によらない基準`は`その電流の80%が想定する定常電流の合計を上回ること`と定めており**等号を含まない。****したがって下限は`I > 合計/0.8`であり、境界の値そのものは窓に入らない。****MSP2807の定常電流が境界値以上になると窓が閉じる**（境界そのものが閉じる側である。値の正は[power-budget.md](power-budget.md)の`窓が閉じる境界（2026-08-22の計算）`。**ここへ再掲しない**） | **合成給電（段階C）**。保護部品の選定・実装まで実施しない | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**2026-08-22に窓と境界を計算した。その時点では選定も方式の決定もしていなかった**（方式と品番は2026-08-25に決まった。下記）**。****この境界は段階B-2でのMSP2807の測定に対する合否の境界になる。境界値以上で窓が閉じる。**その場合は保護部品の選定へ進む前に、経路の変更か`servo-safety-limits.md`の制限の締め直しを決める。**2026-08-25に方式と品番が決まった**（[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)。ヒューズ。**PTCは採らない**）。**それでもcloseしない。****残るのは(0) 実際に買える品番の確定**（**2026-08-25時点の品番は一次資料で特性を確認した候補であり、購入は確定していない**（`digikey.jp`が取扱終了を表示している。**他の販売店は一次で確認できていない**）**。**入手性は[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)が持つ。**方式と定格電流は品が替わっても変わらないが、関門・`I²t`・冷抵抗は品に依存するため取り直しになる。**正は[power-budget.md](power-budget.md)の`入手性はこの節で確認していない（2026-08-26）`）、**(1) 購入と実装、および`記録するtrip動作`のうち実装後にしか埋まらない欄、(2) ヒューズホルダの定格**（**商品ページにもdatasheetにも記載が無いことを確認済み。**ヒューズ側のdatasheetはホルダによる減額の参照を求めているが、その資料が存在しない）、**(3) `HW-TBD-022`**（経路の最小定格の確定）、**(4) `遮断までの時間`の合否判定**（判定条件の「最弱部品の熱時定数」がどの経路部品についても未確定である。溶断時間の側は公開値が揃った）、**(5) DCでの遮断能力**（**メーカーが与えるのは交流の定格だけであり、この経路は5 V DCである。**交流の遮断定格から直流を導かない。正は[power-budget.md](power-budget.md)の`遮断能力はDCで未確定である`。**ここへ再掲しない**）、**(6) trip値の締め直しの判断**である。**(6)は段階B-2に従属する。**段階B-2は`HW-TBD-025`の共通条件が未確定で実施できず、**とくにMSP2807の安全な電流上限が未公開である限りB-2aもB-2bも実施できない。****2026-08-26に人間が方針を決めた。****servo予算の見直しで窓を広げる**（正は[power-budget.md](power-budget.md)の`servo予算の見直しで窓を広げる（2026-08-26、人間の判断）`。**関係式も値もここへ再掲しない**）。**測定を待たずに動かせる量がservo予算だけだからである。****ただしまだ実行していない。**新しい予算値が未定であり、決めるには(1)いくつにするかの設計判断と、(2)その値が`servo-safety-limits.md`の可動域・速度・duty cycleで実際に強制されること、の2つが要る。**実行するときは`ingressに必要な定格の見積もり`（**2026-09-09、PM承認の節見出し改名により`変換基板に必要な定格の見積もり`から差し替え**）の正式改訂の手順を踏み、電源側とservo側の片方だけを変えない。**窓が閉じた場合の結論（保護部品の選定の問題ではなく経路の定格が足りないという結論）は[power-budget.md](power-budget.md)が持つ。**ここへ再掲しない**）。**2026-08-26の更新**: **方式をPTCへ変更した**（人間の判断。[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)）。Bourns `MF-R135`（秋月 通販113013）。**却下の前提が誤っていた。**訂正③は「在庫のあるPTCに条件を満たす品が無い」としていたが、**当時見たのは3品番だけで、秋月のPTCカテゴリには41件ある。条件を満たす品が実在した**（値と理由の正は[power-budget.md](power-budget.md)の`方式をPTCへ変更した（2026-08-26、人間の判断）`。**ここへ再掲しない**）。**判明している最小定格は1.25 Aから1.35 Aへ上がり、上限は1.0 Aから1.08 Aへ、MSP2807の境界は160 mAから240 mAへ広がった。****ヒューズホルダは経路要素から外れた**（PTCは基板へ直付けするため）。**2026-08-27の更新**: **`M-12001の供給上限3 Aでのtrip時間をdatasheetのTime to Tripグラフから読んだ。**確定値としては読めなかった。曲線がグラフの描画上限（1000秒）へ入るのが電流にして約2.97〜3.06 Aであり、3.00 Aはこの範囲をまたぐ。**読めたのは下界（少なくとも数百秒、1000秒に達するか超える可能性がある）だけである**（読み取り方法と根拠は[power-budget.md](power-budget.md)の`3 Aでのトリップ時間を読み、線材と嵌合接点の判定で止まった（2026-08-27）`。**ここへ再掲しない**）。**線材（`111641`）は連続許容電流（約6.0〜6.9 A）が3 Aを上回るため、継続時間によらず持つ。****嵌合接点（定格1.5 A）は3 Aがその200%であり、この下界の時間で持つかどうかを判定する一次資料が無い。****それでもcloseしない。**`記録するtrip動作`の`遮断までの時間`と`通過エネルギー`は、最弱部品側の熱時定数と`I²t`耐量が非公開のため依然として判定できない。**この判定はPMへ上げた。****`(0) 実際に買える品番の確定`は解消した**（秋月で買える）。**発注は次に秋月で買う機会にまとめる**。**2026-08-27、PMの判断。**「持つ」として扱わない（一次資料が無い。ADR-0016の`A`＝判定に効く数であり、桁の余裕や推定では埋められない）。**方式の再検討はしない。PTCを維持する。**ヒューズへ戻すと`DCでの遮断能力が未確定`という別の未検証の穴と、日本での入手性の問題へ戻るだけである。**詰まりの本体はPTCの性能ではない。**`Ihold`を定常840 mAより上に置く必要がある限り、`Itrip`とM-12001の供給上限3 Aの間には必ず遅い帯が残る（方式によらない）。**詰まりの本体は嵌合接点が1.5 Aのままであることである。**変換基板（秋月`110972`）の`VBUS`を複数pinへ渡せば実効定格が上がる可能性があり、`HW-TBD-007`へ着荷後の現物確認項目として追加した。**したがってHW-TBD-021の詰まりを解く経路は保護部品の再選定ではなく、HW-TBD-007の現物確認である** **2026-09-08にその`HW-TBD-007`の現物確認（`VBUS`引き出しpinの導通チェック、非通電）を実施した。**`VBUS`は引き出しpin4本のうち2本へ出て導通していることを確認したが、receptacle内部の配線（trace幅・銅箔厚）が非公開であるため実効定格は上げられなかった（正は[power-budget.md](power-budget.md)の`経路部品と定格`の該当行と本台帳`HW-TBD-007`。**ここへ再掲しない**）。**したがってこの行が名指ししていた「`HW-TBD-007`の現物確認」という経路は尽きた。**嵌合接点1.5 Aという詰まりは残っている。**次の経路は未決である（PMへ上げた、2026-09-08）** **2026-09-09に人間が設計判断を決定した（案B、`WORK-INSTRUCTIONS-INGRESS-PLAN-B.md`）。**M-12001の出力cableをMicro-Bプラグの手前で切り、端子台（`114217`、6 A）へ直接ネジ止めする。**この決定により、この行が詰まりとして名指ししていた嵌合接点（Micro-Bオスplugとreceptacleの嵌合接点、1.5 A）そのものが経路から外れる。**したがって「1.5 A定格の接点が200%の電流を数百秒以上通したときに持つか」という未解決の問いは、ingress側については対象を失った。**最小定格を自分で確かめた。**`経路部品と定格`表を全部足し合わせた結果、`PROT-OC-01`（PTC）の`Ihold` 1.35 Aが引き続き経路の最小値であり、外れた1.5 Aの2行はもともと最小値ではなかったため、最小定格・gate値（1.08 A以下）とも変わらない。**ただしこの行はcloseしない。**残る条件（(1)実際に買える品番の確定、(4)遮断までの時間の合否判定、(6)trip値の締め直しの判断が段階B-2に従属すること等）は、この設計判断の対象外であり、Plan Bとは独立に残る。**cableはまだ切っていない（机上の反映のみ）。****「A1が解けた」とは書かない**（Pi側の嵌合接点は`TBD`のまま残り、追跡は`HW-TBD-007`） |
| HW-TBD-022 | P0 | **大電流経路の線材・接続部材（`WIRE-PWR-01`）と`CABLE-PI-PWR-01`の選定と許容電流。**（`CABLE-PI-PWR-01`は[power-budget.md](power-budget.md#経路部品と定格)で同じ規則の適用対象であり、2026-08-11の照合でこの行に含めることを明示した。）手持ちのbreadboardとジャンパー線は個別の許容電流がメーカー資料で確認できず、経路の最小定格を出せない。最小定格が出ないとingressの上限（経路部品の定格の最小値の80%）が確定しない。**2026-08-25に線材と端子台を選定した。**線材は秋月 通販111641（住友電気工業、絶縁体イラックスA、0.3054 mm²＝0.3 sq）で、**この品は許容電流が公開されている**（同社`許容電流計算式`PDFの**2ページ目**の`イラックス®A電線（定格温度90℃）`の`0.3sq`曲線。**取得物のsha256は2026-08-22に記録した値と一致し、同一文書の当時見ていなかったページである**）。端子台は秋月 通販114217（**製造者が公開する電流定格6 A**、適合AWG18〜26） | 公称許容電流が公開されている線材・接続部材の選定。決定の根拠は[power-budget.md](power-budget.md#経路部品と定格) **2026-08-22に選定規則を評価可能な形にした。選定はしていない。**販売元が公開しているのは住友電工の`許容電流計算式`（式と材料別の係数表）であって数値ではない。**式の入力のうち`r0`（20 ℃における導体抵抗の規格値）だけが電線側の仕様から来るため、`r0`が公開されていない製品では式を評価できない。**候補として見た秋月の`UL1015 AWG18`は`導体抵抗`も`許容電流`も公開していない。**第三者の公開値は18 Aと19.2 Aで食い違い、条件の記載も揃っていない。****したがって定格として採らない。**詳細は[power-budget.md](power-budget.md)の`線材の許容電流をどう確定させるか（2026-08-22の調査）`。**ここへ再掲しない** **2026-08-22に、線材・cableと接続部材で受け入れ条件が違うことを分けた。****`導体抵抗`は導体の証拠であって接点の定格を証明しない。**線材・cableの導体は「許容電流そのものか製造者が公開する`導体抵抗`のいずれか」、**接続部材は「製造者が公開する電流定格」**である（正は[power-budget.md](power-budget.md)の`線材と接続部材で受け入れ条件が違う`。**ここへ再掲しない**）。**cableは導体と接点の両方を持つため、両方の条件を満たす必要がある。** | ingressの上限の確定、**合成給電（段階C）** | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**2026-08-22に選定規則を評価可能な形にした。選定はしていない。**残るのは(1) **線材・cableの条件**を満たす品の選定、(2) 分岐点の接続部材（**未調査**）、(3) `CABLE-PI-PWR-01`（**未調査**）、(4) GND戻り経路である。**AWG18級なら経路の既知の最小値1.5 Aより一桁大きいことは分かったが、これは非メーカー値との比較であり定格の確定ではない。**`経路部品と定格`の表の`TBD`は埋めていない。****2026-08-24に選定条件が2つ増えた。**(1) `CABLE-PI-PWR-01`をrail側で受ける端子台がこの行の接続部材に入る（rail側の受け方が②（直付け）で確定したため）。(2) **手配線区間の接続部材は極性が固定される形であること**（`HW-TBD-030`の承認内容の2から渡された条件である）。**どちらも正は[power-budget.md](power-budget.md)であり、ここへ再掲しない**）。**2026-08-25の更新**: 選定が2件済み、**3件が残る。**(1) **線材の許容電流の数値がまだ読めていない。**グラフからの読み取りであり、**周囲温度`T`と条数と`η0`を先に決める必要がある**（`η0`は2条・中心間隔`S=d`で0.85。同PDFの1ページ目）。読んだ点を添えて`経路部品と定格`の3行（往路2本とGND戻り）へ入れる。(2) **分岐点の接続部材は未選定であり、2026-08-25の発注に含めていない。**端子台114217は`CABLE-PI-PWR-01`をrail側で受ける分であって分岐点の部材ではなく、**1極あたり3本（入力1・出力2）を受ける形が決まっていない。**(3) `CABLE-PI-PWR-01`は未選定のまま（導体と接点の両方の条件が要る）。**この行はcloseしない**。**2026-08-25の追記**: **分岐点の構成と`CABLE-PI-PWR-01`が決まり、残るのは線材の許容電流の読み取りだけになった。**(1) 分岐の節点は`PROTO-02`（ユニバーサル基板、秋月 通販100517）上のはんだ接続で作り、端子台`114217`を6個（3分岐＋予備3）で各branchを終端する。**はんだ接続に定格は無く、節点の導体は`111641`が決める。**(2) `CABLE-PI-PWR-01`は秋月 通販109314で確定したが、**同品は許容電流もAWGも公開していない。****同日、人間がこの行へ許容電流の公開を求めないと決め、同日その扱いが正本にも入った**（[Hardware Safety Policy](../governance/hardware-safety-policy.md)の`安全要件の5項目`と`5項目以外の扱い`。根拠は[ADR-0014](../decisions/0014-safety-requirements-and-general-values.md)、**`Accepted`**。**ここへ再掲しない**）。**ただし[ADR-0016](../decisions/0016-evidence-bar-inside-the-safety-requirements.md)の`B-2`であり、比較する実在値が無いため`Bで済ませるときの条件`の1を満たしておらず、この行はまだ通っていない**（負荷側のPi branch上界350 mAは実在値だが、導体側が無い。満たす道は[power-budget.md](power-budget.md)の`経路部品と定格`の同行。**ここへ再掲しない**）。(3) **残るのは線材の許容電流の数値の読み取りである**。**2026-08-26の追記**: **その読み取りを実施した。**`0.3sq`曲線の両端をpixel座標から読み、式が`I`を`T1 - T`の平方根に比例させることを使って換算した。基準点は`I(T1 - T = 100 ℃) = 10.0 A`で、**周囲30℃・2条密着（`η0=0.85`）なら約6.6 A**である（周囲25℃で約6.9 A、40℃で約6.0 A）。**数値と読んだ点の正は[power-budget.md](power-budget.md)であり、ここへ再掲しない。****これで`ADR-0016`の`B-2`が求める「比較する実在値」が導体側にも揃った。**`PROT-OC-01`は2026-08-26にヒューズからPTC(`Ihold` 1.35 A)へ変更されている(HW-TBD-021の状態欄が正。ここへ再掲しない)ため、比較の基準は1.25 Aではなく1.35 Aである。**現行の基準(1.35 A)に対しては約4.4〜4.9倍(周囲30℃で約4.9倍、40℃の最悪ケースで約4.4倍)であり、桁ではない。****2026-08-29に、人間がこの倍率を「桁で余裕がある」の基準として足りるとみなし、この点を承認した。**この承認は`WIRE-PWR-01`・`CABLE-PI-PWR-01`双方の導体側許容電流の評価にのみ及ぶ。**周囲温度`T`と条数・配列は決めていない**（`T`は`MEAS-04`で実測する量である。Owner: Human）。**残るのは(4) GND戻り経路である。****ただしこれは線材・接続部材の選定作業における残件であって、closeの条件のすべてではない。****`CABLE-PI-PWR-01`の短絡安全の受け入れは未了である。**同品を`B-2`として扱うことは**証拠区分と選定上の評価**であり、**短絡時の故障電流・保護動作・温度上昇に対する安全受け入れを表さない。両者は独立している。****したがって`B-2`が通っても、短絡安全の受け入れが済むまで段階Cのgateは開かない。**条件と判定の正本は[power-budget.md](power-budget.md)であり、**ここへ再掲しない。****この行はcloseしない** **2026-09-08に(4) GND戻り経路を検討し、「確定した」と一度書いたがPM（`deskcat-f2`）のレビューで訂正した。**`GND戻り経路`は往路と同じ電流を通すため線材は往路と同じ`111641`であり、読み取り値（周囲25〜40℃・2条密着で約6.0〜6.9 A）も往路と同一である。**この行自身が「この承認は`WIRE-PWR-01`・`CABLE-PI-PWR-01`双方の導体側許容電流の評価にのみ及ぶ」と明言しており、往路・戻り路の使用箇所を区別していない。同じ導体（`111641`）を戻り経路にも使う以上、この評価はここにも及ぶと判定する。**したがって導体の評価分については新規の人間判断を要さない（この最後の一文はAI判断としてここに明記する）。**ただし残る2点があり、(4)は解けていない。**(a) 周囲温度`T`と条数・配列は未確定のまま残る（`T`は`MEAS-04`で実測する量である。Owner: Human）。(b) 同経路は`GND topology`節の構成に従うが、star point構成そのものの確認は変換基板が揃うまで完了しない（`power-budget.md`の`方式1で残っている未解決3点`の3。段階B-1での通電確認）。**正は[power-budget.md](power-budget.md)の`GND戻り経路（各railからstar pointまで）`行。ここへ再掲しない。****`CABLE-PI-PWR-01`の短絡安全の受け入れも別に残る。**この項目は2026-09-08にPM（`deskcat-f2`）が`HW-TBD-021`の詰まり（変換基板側の嵌合接点1.5 Aが`PROT-OC-01`のItrip直上での遅いtripに耐えるか）と同根と判定し、**`HW-TBD-007`の着荷後の現物確認項目（変換基板`VBUS`を複数pinへ渡せるかの現物確認）待ちとした。**ただし2026-09-08のCodeRabbit `full review`指摘（[PR #369](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/369)）を受け訂正する。**この現物確認が解くのは変換基板側の嵌合接点（ingress側）の実効定格だけである。**`CABLE-PI-PWR-01`自身（downstream側のcableとPi側嵌合接点）が`PROT-OC-01`のトリップまでの故障電流・保護動作・温度上昇に耐えるかは、この現物確認では証明されない別の証拠が要る**（`power-budget.md`の`HW-TBD-022`を閉じるために残っていることの3が「Pi側の嵌合接点の定格は`TBD`のまま残る」と明記するとおり）。**したがってこの項目は、変換基板`VBUS`の現物確認・`GND topology`のstar point構成確認に加え、`CABLE-PI-PWR-01`downstream側の独立した安全受け入れの3点すべてが揃うまでこの行はcloseしない** **2026-09-09に人間が設計判断を決定した（案B、`WORK-INSTRUCTIONS-INGRESS-PLAN-B.md`）。**M-12001の出力cableをMicro-Bプラグの手前で切り、端子台（`114217`）へ直接ネジ止めする。これにより`変換基板側の嵌合接点`という要素自体が経路から外れ、上記3点のうち1点目（変換基板`VBUS`の現物確認）は対象を失う。**残る2点（`GND topology`のstar point構成確認、`CABLE-PI-PWR-01`downstream側の独立した安全受け入れ）はこの決定の影響を受けず、そのまま残る。****`CABLE-PI-PWR-01`の短絡安全の受け入れ（A1）はこの決定では解けない**（Pi側の嵌合接点はingressの経路変更と無関係にTBDのまま残る。追跡は`HW-TBD-007`）。**cableはまだ切っていない（机上の反映のみ）** |
| HW-TBD-023 | P0 | **ESP32 boardの`3V3` pinから外部負荷を取ってよい条件。**(a) 外部供給可能電流の定格、(b) board上regulatorの種別（LDO／switching）、(c) **過電流保護／短絡保護の有無と動作**（制限電流、折り返し特性、熱shutdownの有無）、(d) **`U2`周辺のdecoupling実装**（参照設計では入出力とも22 µF。`HW-TBD-029`から2026-08-12に移した）。**根拠は2段階に分けて扱う。参照設計の回路図で分かったことは、現物の事実ではない。**現物が公式V4リファレンス設計どおりに実装されている保証は文書だけでは得られない。**`HW-TBD-001`（pin配列の照合）は2026-08-15にcloseしたが、それはpin配列が一致したことを示すだけであり、実装部品が回路図どおりであることまでは保証しない。実際に`U2`は回路図と違う部品であった**（下記）。**参照設計で判明**: `U2 = AMS1117-3.3`（LDO）であり、回路図上にヒューズ・ポリスイッチは無い（`LESD5D5.0CT1G`×3はESD保護であって過電流保護ではない）。参照設計どおりなら、短絡・配線ミス時に電流を制限しうるのは`AMS1117-3.3` IC自身の内蔵保護だけになる。**2026-08-13の現物確認で、参照設計と現物が違うことが判明した。****現物の`U2`の刻印は`LD1117AG` / `33AQVCUV`であり、`AMS1117-3.3`ではない。**どちらもSOT-223の3.3V LDOでpin互換の二次ソースだが、部品が違う。`33`は3.3V版を示すと解される。**これは「参照設計由来の事実を現物の事実として書かない」という扱いが実際に効いた例である。**回路図を信じて`AMS1117`のdatasheetを当てていれば、別部品の仕様で段階B-2aの可否を判定していた。**2026-08-15に`U2`のメーカーを特定した。**現物の刻印は**UMW（UTD Semiconductor Co., Limited）の`LD1117-3.3`（SOT-223）**の刻印規則と一致する（根拠欄）。**これにより(b)が確定し、(c)は一部だけ埋まった。****(b)はLDOである。****(c)のうち確定したのは制限電流と熱shutdownの有無だけである。**内蔵の電流制限（`Ilimit` min 1.25 A／max 1.6 A）と熱shutdown（`TSD` typ 150 ℃）を持つ。**短絡保護の有無と折り返し特性はdatasheetに記載が無い。これは「無い」ことの確認ではない**（[AGENTS.md](../../AGENTS.md) 推測禁止）。**したがって(c)が求める「短絡・配線ミス時の動作」は未確定であり、(c)はcloseしない。****それでも段階B-2aに不利な事実が1つ確定した。**電流制限のtrip点1.25–1.6 Aは**この品の1 Aという定格より上にある。**したがって**`IOUT` 1.0–1.25 Aの区間は電流制限が動作しない範囲である。**この区間で熱shutdownが働くかは放熱条件次第でありdatasheetからは決まらない。**「熱shutdownが代わりに効く」と読み替えない。****残**: (a)、(c)の短絡・折り返し、(d)。**(a)はメーカーdatasheetでは埋まらない。**ICの供給能力は`3V3` pinが外部へ出せる電流の**上限を与えるだけ**であり、**Espressifは`3V3` pinの外部供給可能電流をboard levelで公開していない。**熱からの導出（datasheetのNote 1が与える`PD(max) = (TJ(max) - Tamb) / Rθja`に`TJ(max)` 150 ℃と`Rθja` 88 ℃/Wを入れる）も**この基板の実力ではない。**同Note 1が**`Rθja`は実装方法で変わると明記している**ためであり、[ESP32-DevKitC V4公式回路図](https://dl.espressif.com/dl/schematics/esp32_devkitc_v4-sch.pdf)は基板の銅箔面積を与えない。**したがって(a)は現物の実測でしか埋まらない。**(d)は2026-09-05の接写で個数まで確認した。`U2`の上に1個、下に横並びで2個、合計最低3個のコンデンサらしき部品（積層セラミック）が実装されている。**容量値は読めていない**（表記が無い）。**2026-09-06に、UMW LD1117 datasheetの`4.Pinning information`図（SOT-223）で足の並びを確認した。**刻印面を見て足が下に出る向きで、左から`ADJ/GND`（pin1）・`VOUT`（pin2）・`VIN`（pin3）である。**`AG`は固定電圧版であり、block diagramの`Fixed: F1 and F2 connect, disconnect A`によりpin1は実質`GND`として働く。**ただし、この並びが分かっても、写真の3個のコンデンサのうちどれがどの足の近くにあるかは別軸（コンデンサは部品の上下、足は部品の左右）であり、対応づけていないため**依然としてどれが入力側でどれが出力側かは未確定である**（推測しない）。回路図に無い保護部品は現物にも見当たらない | 参照設計の根拠: [ESP32-DevKitC V4公式回路図](https://dl.espressif.com/dl/schematics/esp32_devkitc_v4-sch.pdf)。経路は`VBUS → D3 BAT760-7（Schottky）→ EXT_5V → U2 AMS1117-3.3 → VDD33 → J2 pin1`、入出力とも22µF（2026-08-10確認）。**現物の根拠**: 2026-08-13に斜光＋接写で`U2`の刻印を読み取り、`LD1117AG` / `33AQVCUV`を得た。**メーカーは2026-08-15に特定した。**出典は[UMW LD1117 datasheet](https://www.umw-ic.com/static/pdf/a5e0c99cefdefaf03cfa7777b369e45b.pdf)（発行元`UTD Semiconductor Co., Limited`、Mar.2025、14page、sha256は`5dec5c09027316c375c27108cbac9d088ba94060bebacd614a04a3b8bb18a3d3`。2026-08-15取得）である。**同datasheetの`14.Ordering information`はSOT-223の刻印を1行目`LD1117AG`、2行目`xxAQwwT S`（`xx`はVoltage、`ww`はWeek Code）と図示し、発注表に`Order Code: UMW LD1117-xx`／`Marking: LD1117AG xx`／`Package: SOT-223`を載せている。****現物の`LD1117AG` / `33AQVCUV`はこのtemplateと一致する。**1行目はliteralが一致し、2行目は`33`が`xx`（3.3 V）、続く`AQ`がtemplateのliteralと一致し、残る`VCUV`がdate／lotに当たる。**8文字の構成が一致する。****UTC（Unisonic Technologies）は候補から外した。**UTCにも`LD1117AG`系の型番があり、[UTC LD1117/A-K datasheet](http://www.utc-ic.com/uploadfile/2011/0915/20110915031520257.pdf)（文書自身が`UNISONIC TECHNOLOGIES CO., LTD`、文書番号`QW-R125-113.F`、footer `www.unisonic.com.tw`、Copyright 2024を記載する。sha256は`2af2ec5e4cf07cabcf2b76eb66cf24257554bb019d228d43a520460a7b1430b7`。2026-08-15取得。**`www.unisonic.com.tw`は同じPDFを直接返さず、`utc-ic.com`はhttpsの証明書が期限切れのためhttpで取得した。この1点だけはUMW側の資料より出所が弱い**）の`MARKING INFORMATION`は`A`をCurrent Code（1 A）、`G`をHalogen Free、`33`をVoltage Codeと定義するため**`LD1117AG`と`33`を意味の上では説明できる。しかしUTCの刻印templateは`UTC`の社名行を含み、`AQ`というliteralを生まない。**現物の読み取りは2行で社名行が無く、`AQ`を含むため、**UMWの方が一致度が高い。****決め手は刻印単体ではなく、メーカーが公開したtemplateとの一致である。**要件は[power-budget.md](power-budget.md#段階b-2の測定)の`B-2a: 3V3 pinから給電する経路`、確認項目は[hardware-bom.md](hardware-bom.md) `MCU-01`と部品受け入れchecklist。**配線規則の正本は[power-budget.md](power-budget.md)であり、この行では定めない** | **段階B-2a**。**(a)が未確定である限りB-2aを許可しない。**B-2bへ移れば回避できるが、`HW-TBD-024`／`025`は経路によらず必要なため、それらが未解決ならどちらの経路も進まない。**なお(c)が一部埋まったことでB-2aの前提が一つ悪くなった。**B-2aは`故障電流を制限するもの`をregulator自身の保護に置く経路だが（[power-budget.md](power-budget.md#段階b-2の測定)の経路表）、**電流制限は1.25 Aまで動かず、短絡時の挙動はdatasheetから決まらない。**採否の判断材料としてこの事実を`HW-TBD-009`と`HW-TBD-030`へ渡す | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)、[#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**(b)は2026-08-15に確定した。(c)は制限電流と熱shutdownだけが埋まり、短絡保護と折り返し特性はdatasheetに記載が無いため未確定のまま残る。**(a)と(d)も残るためcloseしない。(a)はEspressifがboard levelで公開しておらず、**メーカーdatasheetでは埋まらないことが確定した**ため、残る手段は現物の実測だけになった。**そしてその実測は段階B-2そのものであり、この行がB-2aをBlockedしているため循環する**（`HW-TBD-024`と同じ構造である）。**B-2bを採れば(a)は要らなくなるため、循環を解く必要は無い。経路の選択で回避できる。**(d)は2026-09-05に個数（最低3個）まで確認したが、入力側・出力側の内訳と容量値は未確定のまま残る。**`HW-TBD-001`は2026-08-15にcloseしたが、この行は独立して残る**） |
| HW-TBD-024 | P0 | **MSP2807が耐えられる電流の上限。**メーカー未公開である。外部の3.3 V電源から給電する経路（段階B-2b）は**設定した電流制限が唯一の保護**であり、その設定値の上限を決めるにはmodule側の安全な上限が要る。**上限が無いままではMSP2807へ給電できない。**これは`HW-TBD-025`の共通条件(b)の一部であり、**B-2bだけでなくB-2aにも掛かる**（B-2aでもregulatorの保護のtrip点がmoduleにとって安全かを判定できない）。ADXL345とBME280だけ測っても未知数は埋まらないため、B-2自体が成立しない | **一次資料には無いことが確定した。**メーカーdatasheet（[msp2807.pdf](https://akizukidenshi.com/goodsaffix/msp2807.pdf)）の`Power Consumption`欄は`TBD`と印字されている（2026-08-10確認）。[LCD Wiki](http://www.lcdwiki.com/2.8inch_SPI_Module_ILI9341_SKU:MSP2807)にもbacklight駆動回路の記載は無い。**したがって現物のbacklight回路を見る以外に埋める手段が無い。****2026-08-13に現物を確認した結果は次のとおりである。**`R5`＝`6R8`（6.8 Ω）、`R6`＝`102`（1 kΩ）、`Q1`＝SOT-23で刻印`J3Y`、**`U1`＝SOT-23で刻印`662K` / `UMW S4T`**、`C1`＝積層セラミック（容量表記なし）、`J1`＝半田ジャンパで**オープン**。**`662K`はSOT-23の3.3V LDOの標準刻印であり、moduleが「VCC 3.3–5 V」を受けられる理由がboard上のLDOであることを示す。したがってこのLDOの電流能力がmoduleの上限を決める要素になる。****`U1`のメーカーと品番は2026-08-15に特定した。****UMW（UTD Semiconductor Co., Limited）の`XC6206P332MR`（SOT-23、3.3 V）である。**出典は[UMW XC6206 datasheet](https://www.umw-ic.com/static/pdf/e442437afc9daa185da86486e2bfe8c5.pdf)（発行元`UTD Semiconductor Co., Limited`、Mar.2025、13page、sha256は`8d8e3e30dbf48a39e0b4e11122ac242c2849b68ee8225ba0d672d83decf30504`。2026-08-15取得）である。**同datasheetの`13.Ordering information`は発注表に`Order Code: UMW XC6206P332MR`／`Marking: 662K`／`Package: SOT-23`を載せ、SOT-23の刻印図を1行目がmarking code、2行目が`UMW yww`（`yy`はYear Code、`ww`はWeek Code）と示している。****現物の`662K` / `UMW S4T`はこれと一致する。**1行目が3.3 V版のmarking codeと一致し、2行目の`UMW`が社名の位置に来る。**残る`S4T`は復号できない。**datasheetがyear／week codeの対応表を出していないためである。**`662K`は複数社が二次ソースする刻印であり、刻印単体では特定できない。決め手は2行目の`UMW`と、UMWが`662K`を自社の発注表に載せていることの組み合わせである。****供給能力**は次のとおりである。`IOUT(max)`は`VIN=VOUT(T)+1V`で**min 100 mA**、過電流保護`Ilimt`（**datasheetの表記のまま。`HW-TBD-023`の`LD1117`側は`Ilimit`と綴っており、資料が違う**）は**typ 380 mA**、短絡電流`Ishort`は**typ 30 mA**、絶対最大定格は`IOUT` **500 mA**と`PD`（SOT-23）**300 mW**である。**なお同datasheetは`1.Description`と`3.Features`で「`Vin=4.3V`／`Vout=3.3V`で`Iout=250mA`」と述べるが、`8.Electrical characteristics`は同じ条件で`IOUT(max)` min 100 mAしか規定していない。****保証値は100 mAであり、250 mAは本文の記述であって表の保証ではない。**矛盾を平均も統合もせず、両方を記録する（[AI Agent Policy](../governance/ai-agent-policy.md)）。**ただしLED個数と接続は確認できない。**パネル端がカプトンテープで覆われており、剥がすのは分解にあたるため目視では不能である。**`J1`がLDOのbypass選択かどうかもパターンを追っていないため断定しない。****`U1`の供給能力が判明しても、それだけではmoduleの上限は決まらない。**`U1`の3本足（入力／`GND`／出力）は2026-09-05の導通測定で識別できた。**`LED` pinは公式User Manual（LCDWIKI）が「backlight control signal」と定める制御信号線であり、`U1`のいずれの足とも非導通（測定結果と整合）である。**したがって`LED` pinの導通確認だけでは「backlightのLED列が`U1`の出力側にあるのか、`VCC`側（`U1`の上流）にあるのか」は決まらない。**実際にLEDへ給電しているライン(`R5`の先)を`VCC`・`U1`出力それぞれと比較する測定を試みたが、`R5`は部品が小さくプローブを確実に当てられず、`Q1`（トランジスタ）は接合の性質上、非通電の単純な抵抗測定では判定できないと判った**（詳細は[sensor-datasheet-notes.md](sensor-datasheet-notes.md)のBacklight行、Revision 12）。**したがって、この経路は非通電の導通測定では確定できないと結論する。**`U1`のdatasheetで押さえられる上限（供給能力）がbacklight電流に適用できるかも、判定できないまま残る。`R5`／`R6`／`Q1`／`J1`を経た正確な経路（電流値そのもの）もパターンを追っておらず未確定のままである。なおLCD Wikiに現れる「0.31 W」はtypicalの消費であって**耐えられる上限ではないため、この行の答えにしない**。要件は[power-budget.md](power-budget.md#段階b-2の測定)の`B-2b: 外部の3.3 V電源から給電する経路`の`実施前に満たす条件` | **段階B-2（B-2a／B-2bとも。`HW-TBD-025`の共通条件(b)の一部であるため）**。MSP2807の定常電流の実測と、そこから決まる`PSU-INGRESS-01`の選定が段階Cまで進まない | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)、[#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**`U1`のメーカーと供給能力は2026-08-15に確定したが、moduleの上限は出ていない。**LDO経路の上限は資料で押さえた（絶対最大`IOUT` 500 mA、`PD` 300 mW、保証100 mA、過電流保護typ 380 mA）。**`U1`の3本足の識別は2026-09-05に完了した**（[sensor-datasheet-notes.md](sensor-datasheet-notes.md)のBacklight行、Revision 12）。**backlightがこの経路を通るかは、`R5`・`Q1`を使った追加測定を試みたが、非通電の導通測定では判定できないと結論した。**LED個数は依然としてテープで隠れており、非分解では確認できない。****循環の扱いは2026-08-15に決めた。**datasheetと実測の二択のうち**datasheet側は尽きた。実測（非通電の導通測定）も、この経路については尽きた。**残るのはLED個数（分解を要するためこのIssueの範囲では埋まらない）と、この経路（通電を伴う測定でしか決まらない可能性がある）である。**したがってこの行は段階B-2を引き続きBlockedする**） |
| HW-TBD-025 | P0 | **段階B-2の共通条件。給電経路によらず必要であり、B-2a／B-2bのどちらを選んでも省略できない。**(a) `3.3 V rail`の許容電圧範囲。周辺module3点が**moduleとして受け入れてよい電圧**の積集合とESP32のlogic levelから決める（**IC単体の動作範囲の積集合ではない**）。(b) 周辺module3点それぞれが耐えられる電流の上限。B-2bでは電源に設定する電流制限値の上限を決めるために、B-2aでは**board上regulatorの保護のtrip点がmoduleにとって安全かを判定する**ために要る。**datasheet調査は2026-08-12に完了した。3点とも絶対最大定格に電流の上限は記載が無く、公開値からは得られない。**残るのは現物回路の確認であり、それも得られないなら当該moduleへB-2で給電しない。**`HW-TBD-023`（B-2a）や`HW-TBD-024`（B-2b）が解決しても、この行が未解決ならB-2を実施できない** | (a)の材料は公開spec（[hardware-bom.md](hardware-bom.md)の`DISP-01`／`ACCEL-01`／`ENV-01`の電源欄）と[gpio-assignment.md](gpio-assignment.md#電圧domainすべての外部pull-upに適用)である。**ただし積集合を取る対象は各moduleが受け入れてよい電圧であって、IC単体の動作範囲ではない。****`ACCEL-01`分はこれが未確定である**（`moduleへ入れてよい電圧`は`HW-TBD-004`。ADXL345 ICの`VS` 2.0–3.6 Vからmodule boardの許容入力電圧は決まらない）。**したがって(a)は`HW-TBD-004`が解けるまで確定せず、「現物確認を待つ必要はない」とした旧記載は2026-08-12に撤回した。**`DISP-01`と`ENV-01`分はmodule boardの資料があるため先に置ける。**2026-08-12に、その2点とESP-WROOM-32D moduleから暫定値を置いた**（値の正は[power-budget.md](power-budget.md#段階b-2の測定)の共通条件表の`許容電圧範囲`行であり、導出は同節の`3.3 V railの許容電圧範囲`。**ここへ再掲しない**）。**同時に、その暫定値の下限がそのままでは成立しないことが判明した。**下限がrailの公称値と同一で下振れの余裕が原理的に無く、さらに参照設計の`U2 = AMS1117-3.3`の出力がdatasheetの与える2行のどちらを採っても下限を割りうる（`HW-TBD-023`。**参照設計由来であり現物の事実ではない**。**2行の条件の違いはdatasheetに明記されていないため、広い方を採る**）。**`msp2807.pdf`はVCCについて`3.3V~5V`の一行しか持たず、下限が公称値か絶対最小値かを決められない。解釈で埋めない**（[AGENTS.md](../../AGENTS.md) 推測禁止）。**これは(a)そのものの中身であるため、別行を立てず(a)で追跡する。****両立させる経路は3つあり、いずれも人間の判断か現物確認を要する。****(1) MSP2807のmodule level資料に下限の性質を求める → 不可であることを2026-08-12に確認した。****(2) MSP2807のVCCを5 V railへ移す**（module specは3.3–5 Vを許す）**→ `DISP-01`の現物確認待ち**（logic IOが3.3 V TTLであり、5 V給電時に出力が5 VになればESP32のGPIOを壊す。level shiftの有無が未確認。`HW-TBD-002`）。**(3) 周辺module用に別途3.3 V regulatorを置き、設定点とtoleranceで下限を成立させる → 設計判断**（採る場合は[hardware-bom.md](hardware-bom.md)の購入待ちリストへ部品が増える）。**経路(2)を採る場合、`module VCC 3.3–5 V`の範囲を確認しただけでこの行をcloseしない。**(2)は段階B-2の前提そのものを引き直す。[power-budget.md](power-budget.md)は現在B-2を**周辺module3点を3.3 Vで測る**測定と定義しており、MSP2807だけ5 V railへ移すと**測定するrail、電流予算の割り付け、許容電圧範囲の積集合、logic levelの保護、decouplingの要否、B-2a／B-2bの依存関係のすべてが変わる。****closeの条件にはこれらの更新を含める**（詳細は[power-budget.md](power-budget.md#段階b-2の測定)の`暫定値の下限は、そのままでは成立しない`節の経路表）。**(b)** は各moduleのdatasheetの絶対最大定格を当たる。**2026-08-12に当たった結果、ADXL345とBME280はいずれも絶対最大定格を公開しているが、電流の上限は記載が無い**（[ADXL345 Data Sheet](https://www.analog.com/media/en/technical-documentation/data-sheets/adxl345.pdf) Rev. G Table 2 page 5は電圧・加速度・温度・短絡持続時間のみ、[BME280 Data Sheet](https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bme280-ds002.pdf) Revision 1.24 Table 5 page 13は電圧・温度・圧力・ESDのみである）。**「あるはず」と仮定せずに確認した結果であり、無いことの確認も成果である。**したがって**MSP2807と同じ扱いが3点すべてに及ぶ**（現物回路の確認か、上限を得られないなら当該moduleへB-2で給電しない）。**MSP2807分は`HW-TBD-024`**。要件は[power-budget.md](power-budget.md#段階b-2の測定)の共通条件表、確認項目は[hardware-bom.md](hardware-bom.md)の部品受け入れchecklist | **段階B-2（B-2a／B-2bとも）** | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**(a)はBlocked。理由は2つあったが、2026-09-06に両方とも、確定したrail値（3.234–3.366 V）に関する限り解決した。****1つ目（`ACCEL-01`のmoduleとしての許容入力電圧が未確定で積集合を取れないこと）は、`HW-TBD-004`が一般論としては未解決のままだが、このrail値についてはICの`VS`動作範囲2.0–3.6 Vに両端とも余裕を持って入り、下側余裕は直列抵抗8.8 kΩ相当まで許容する（ADXL345のsupply current 140 µAに基づく評価であり、**typ値に基づく評価であってworst-caseの境界ではない**）ことを暫定の根拠に基づく設計判断として確定した**（値・根拠の正は[power-budget.md](power-budget.md#段階b-2の測定)の`3.3 V railの許容電圧範囲`節`ACCEL-01`行の注記とRevision 89。**ここへ再掲しない。****`HW-TBD-004`自体はcloseしていない**。module一般としての受け入れ電圧範囲の確定はこの議論の範囲外である）。**2つ目（`DISP-01`と`ENV-01`から出る暫定値の下限そのものが成立しないこと）は、`HW-TBD-002`（2026-09-06close）が経路(2)を解かないことを確認したうえで、経路(1)〜(3)のいずれでもなく下限を3.234 V（`U2`のdatasheet値。**条件付きのdatasheet値であり無条件の保証値ではない**）で受け入れる設計判断により解決した**（値・根拠・caveatの正は[power-budget.md](power-budget.md#段階b-2の測定)の`3.3 V rail下限の設計判断`節。**ここへ再掲しない**）。**この判断はILI9341のロジック単体負荷を前提としており、backlight電流が`U1`を経由すると`HW-TBD-024`で確定した場合は再導出を要する。****したがって(a)は、このrail値（3.234–3.366 V）に関する限り確定した。** **2026-09-08に訂正した。**この行はそれ以前、`U2`の3.234 Vを「保証最小出力」と呼び、ADXL345側の直列抵抗の余裕を「直列抵抗の値によらず結論が変わらないことを個別に確認した」と無条件の結論として書いていた（[PR #366](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/366)のCodeRabbit `full review`の指摘、[#367](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/367)）。**どちらも誤りだった。**`U2`の3.234 Vはdatasheetの条件（`IOUT = 10 mA`、`VIN = 5 V`、`TJ = 25 ℃`）付きの値であり無条件の保証値ではなく、ADXL345のsupply current 140 µAはtyp値でありworst-caseの境界ではない（datasheetにmax欄が無く、絶対最大定格にも電流の上限が無いことは(b)が既に確認している）。**上記の記述はこの訂正後の表現である。**結論（(a)はこのrail値に関する限り確定した）は変わらないが、根拠の性質（暫定の設計判断であり、worst-caseの確認済み境界ではない）を明示した。(b)のdatasheet調査は2026-08-12に完了し、**3点とも電流上限の公開値が無いことを確認した**。残るのは3点それぞれの現物回路の確認である。MSP2807分は`HW-TBD-024`） |
| HW-TBD-026 | P0 | **SG90の電気的駆動条件。**(a) 制御logic要件（ESP32のGPIOは3.3 Vだが、SG90のlogic閾値が未確定であり、3.3 V driveで確実に動作するかを判定できない。[servo-safety-limits.md](servo-safety-limits.md#サーボ識別情報)は現物確認まで確定しないとしている）、(b) PWM周期／rate（50 Hzは一般値であり、確定値として採らない）、(c) 許容最小／最大pulse幅（データシート由来の電気的な駆動範囲。**機械的可動域は`HW-TBD-010`**）。**Stall／peak電流と無負荷／動作電流の実測は`HW-TBD-010`／`HW-TBD-011`の範囲であり、この行では重複させない**。**2026-08-24に一次資料を確認した。結論は「(a)(b)(c)のいずれも一次資料に記載が無い」である。****まず公式のdatasheet PDFが存在しないことが確定した。**[TowerPro公式のDownload](https://towerpro.com.tw/download/)にファイルの掲載がなく、製品ページにも`.pdf` linkがない。**したがって一次資料は製品ページの仕様表だけである。**仕様表が載せているのは`Operating voltage: 4.8v`、`Stall torque: 1.8kg/cm(4.8v)`、`Operating speed`、`Dead band width: 1us`、`Temperature range: 0℃-55℃`、重量・寸法・gear種別・plug種別・wire長であり、**logic閾値（V_IH／V_IL）、PWM周期／rate、許容pulse幅の3つはどこにも無い。****項目別**では次のとおりである。**(a) 記載が無い。**したがって**3.3 V driveの可否をこの資料では判定できず、level shifterの要否も決まらない。****「一般に3.3 Vで動く」という情報は一次資料ではないため採らない**（[AGENTS.md](../../AGENTS.md) 推測禁止）。判断は人間へ渡す。**部品を先に決めない。****(b) 記載が無い。**`50 Hz`は出所を持たない一般値のままである。**(c) 記載が無い。****しかも2026-08-24の再検証で、メーカーが直接聞かれても答えていないことが分かった。**`sg90-7`のQ&A欄で客が2022-03-02に「What is the range for the pulse width to control the angle? Online it says 1ms-2ms but that doesn't seem to cover the full 180 degree range.」と質問しているが、**同日の`admin`の回答はpulse幅に触れず、角度の話に替わっている。**得られたのは`Dead band width: 1 us`の1点だけであり、**これは許容pulse幅範囲ではなく範囲の内側の分解能に当たる量である。この値からpulse幅範囲を導かない。****さらに、適用する一次資料のページ自体が特定できないことが判明した。**公式サイトには同名で**2品**ある。`SG90 Digital`（`Operating speed: 0.1sec/60degree(4.8v)`）と`SG90 Analog`（同`0.12sec/60degree(4.8v)`）で、torque・電圧・dead band・寸法・温度範囲は同一である。**現物ラベル`TOWER PRO Micro servo 9g SG90`はこの2品を区別しない。**`HW-TBD-006`（正確なservo model）の解決記録はDigital側のページだけを引いている。[hardware-bom.md](hardware-bom.md) `SERVO-01`は2026-08-24に両ページを併記したが、**どちらの品かは未確定のままである。****`HW-TBD-006`は解決済みであり、この行では再openしない。**どちらの品かの判断と、`HW-TBD-006`を再openするかの判断は人間へ渡す。**なお(a)(b)(c)は2品で差の出る項目ではなく、どちらのページにも記載が無い。****したがってこの判別が付いても(a)(b)(c)は埋まらない。****2026-08-24の生HTML再検証で、variantについて2件追加で判明した。**(1) **同名の製品は2つではなく3つある。**`SG90 360 degree`（連続回転、SKU`SG90 360 degree`）が別に存在する。寸法が`23×12.5x22mm`で他2品の`23×12.2x29mm`と異なり、`Operating Angle: 360 degree`・`Direction: CCW`を持つため**現物とは別物と見られるが、断定していない**（現物の寸法を測っていない）。(2) **`Operating speed`でDigital／Analogを判別する根拠は弱い。**Analog側ページは箇条書きが`0.12 sec/60degree(4.8v)`だが、**同一ページの`PRODUCT CONFIGURE TABLE`は`Speed(sec/60deg) 0.1`と書いており、ページ内で矛盾している。****一次資料が自己矛盾しているため、速度の実測で品を判別できるとは言えない。****さらに、`4.8–6 V`と`0.5–2 A`の出所はDigital側ページに限られる。****Analog側ページは`Reviews (0)`であり、この2つの値はどちらもDigital側にしか無い**（2026-08-24確認）。**出所の格は2026-08-24の生HTML再検証で1段上がった。****匿名のreviewではなく、`admin`（メーカー側）の回答である**（2021-11-07。`The operation current for our SG90 is about 0.5A - 2A. Voltages from 4.8V - 6V are fine.`）。**それでも仕様表の値ではなくQ&A欄の記述であり、`Operating voltage`の仕様表記載は`4.8v`だけである。****したがってこの2値は、どちらの品かが未確定な状態で、片方の品のページのreview欄だけを出所とする値である。****variantに依存しない定格として扱わない。**`定格電圧範囲`を`TBD`へ戻すか、`Peak／stall電流`を実測まで`TBD`とするかは、**closeした`HW-TBD-006`の扱いと`HW-TBD-010`／`011`の範囲に掛かるため、この行では判断しない。人間へ渡す。****あわせて、この行の範囲外だが`HW-TBD-010`（機械的可動域）に関わる記述をQ&A欄で見つけた。**`admin`が2022-03-02に**「all of our normal servos are operating 0-150 degrees unless specify 180 or 360 degrees」**と述べている。**メーカーが可動域を0〜150度と言っており、hobby servoで一般に前提される180度を否定している。****この行では値として記録しない**（機械的可動域は`HW-TBD-010`であり、`HW-TBD-026`と重複させない）。**`HW-TBD-010`のcalibrationは監視下の実測で決めるものであり、この記述を確定値に採らない。**事実の存在だけを人間へ渡す。**なお、同じ`データシート値`という誤帰属が`HW-TBD-006`の解決記録にも残っていた**（`定格電圧4.8–6 V、stall電流はデータシート値0.5–2 A`）。**2026-08-26に訂正した（[#193](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/193)）。値は変えておらず、close済みの行を再openしたのでもない。**この行で記録したのは値ではなく、一次資料に何が有って何が無いかである**（[servo-safety-limits.md](servo-safety-limits.md#サーボ識別情報)が「datasheetの代表値を確定値として採らない」と定める） | **メーカー資料では(a)(b)(c)は埋まらないことが2026-08-24に確定した。**公式のdatasheet PDFが存在せず、一次資料である製品ページの仕様表にこの3項目の記載が無いためである（`HW-TBD-023`(a)と同じ構造。**メーカーが公開していないものを推定で埋めない**）。**残る手段は無負荷試験と実測だけであり、それは007／009によりBlockedである。****「datasheetが無い」の検証方法を明記する。**要約に依らず生HTMLで確認した。`robots.txt`が指す`wp-sitemap.xml`からsite全体の**203 URLを列挙し、`.pdf`は0件**。SG90の3ページと`/download/`の生HTMLに**`.pdf`へのlinkが0件**、**`datasheet`／`data sheet`／`spec sheet`の語も0件**。**いずれもHTTP 200で実体を取得しており、403や空応答ではない**（2026-08-24）。根拠とした一次資料は[TowerPro公式 SG90 Digital](https://towerpro.com.tw/product/sg90-7/)、[TowerPro公式 SG90 Analog](https://towerpro.com.tw/product/sg90-analog/)、[TowerPro Download](https://towerpro.com.tw/download/)（いずれも2026-08-24取得）である。**第三者掲載の`Soldered_101246.pdf`を一次資料に採らない。**TowerPro発行と確認できず、取得もできないためである（詳細と扱いは[hardware-bom.md](hardware-bom.md) `SERVO-01`が正）。**(a)については、閾値が確定しないままlevel shifterの要否を決めない。**要件は[servo-safety-limits.md](servo-safety-limits.md#サーボ識別情報)の識別情報表 | `SERVO-PWM`の駆動条件の確定、servoの実機動作全般 | [#17](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/17) | Joint | Open（**データシートから確定できる部分は2026-08-24に着手し、完了した。結果は「(a)(b)(c)のいずれも一次資料に記載が無い」であり、値は1つも確定していない。**この行に残るのは無負荷試験を要する範囲であり、**007／009によりBlocked**である。**あわせて人間へ渡す判断が残る**（level shifterの要否、どちらの品か、review欄由来の値の扱いなど。**全数はこの行の`不足している情報／判断`欄が正であり、ここで数え直さない**）。**2026-08-25に、3品目`SG90 360 degree`の除外／該当を決める判定基準を確定した。判定は寸法で行う。****これは「現物の品の特定」の全体を解く基準ではない。**寸法で決まるのは3品目かどうかだけであり、**`Digital`と`Analog`の分離は寸法では不可能である**（両者は`23×12.2x29mm`で同一）。仕様表の`Dimension`は`23×12.2x29mm`（`Digital`／`Analog`）に対し**`23×12.5x22mm`（`360 degree`）**であり、**高さが29 mmと22 mmで7 mm違う。定規で判別できる。**幅と奥行の差は0.3 mm（`12.2`対`12.5`）しかないため**判定に使わない。****3ページの仕様表を一次資料から直接再確認した**（2026-08-25。生HTMLを取得して`Dimension`行を読み、`Digital`＝`23×12.2x29mm`／`Analog`＝`23×12.2x29mm`／`360 degree`＝`23×12.5x22mm`を確認した）。**同時に、3品目の仕様表だけが持つ項目も確認した**（`Operating Angle: 360 degree`、`Direction: CCW`、`Control system: Digital`、`Stall torque: 1.3kg/cm(4.8v), 1.5kg/cm(6V)`、`Operating speed: 110RPM(4.8V);130RPM(6V)`、`Operating voltage: 4.8v-6V`、SKU`SG90 360 degree`）。**他2品は`Stall torque: 1.8kg/cm(4.8v)`で`Operating speed`は`sec/60degree`表記である。****ただしこれらはいずれも通電または治具を要するため、判定には使わない。判定は寸法だけで行う。****SKU名だけで断定しない。**現物ラベルは`TOWER PRO Micro servo 9g SG90`であり`360 degree`の文字が無いが、**ラベルがSKU全体を載せるとは限らないため、状況証拠にとどめる。寸法を主根拠、ラベルを補強とする。****2026-08-28に高さを実測した（[Issue #257](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/257)。27 mm）。**[PR #258](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/258)がIssue本文の記述をこの行へ反映し漏らしていたため、2026-08-29に反映する。**測定条件（2026-08-29に追記で判明）: 器具はメジャー（巻尺）、端点は筐体の底面から出力軸側の最上部まで。**メジャーは目盛りの読み取りと先端フックの遊びにより、この程度の寸法では定規より誤差が乗るが、**具体的な誤差値は校正されていないため確定値として書かない。**それでも`SG90 360 degree`の仕様値22 mmとの差5 mmは、想定しうる読み取り誤差を超える。**`23×12.2x29mm`（Digital／Analog）との差2 mmについては、仕様表がどの端点を基準に29 mmとしているかが不明であり、この差はその不明に由来しうる。断定しない。****`23×12.2x29mm`（Digital／Analog）側に近く、`23×12.5x22mm`（360 degree）との差5 mmより有意に近いため、`SG90 360 degree`ではないと判定した。****`Digital`と`Analog`の分離は試みない。**両者は寸法が同一で外形では分けられず、分離できる量（`Operating speed`など）は通電を要して`007`／`009`によりBlockedであり、**かつ`HW-TBD-035`の判断には影響しない**（どちらも仕様表は`4.8v`単独）。**`Digital`／`Analog`のどちらかは未確定のまま残る。この行はcloseしない**） |
| HW-TBD-027 | P0 | **`RES-PULL-01`（`SERVO-PWM`の外部pull-down）の抵抗値と本数。**GPIO27はreset時にhigh-Zであり、外部pull-downはPWM driver初期化前にservoが不定pulseを受けないための**必須**部品である。それにもかかわらず抵抗値が未選定である。**部品は10 kΩと4.7 kΩが各1袋2026-08-08に着荷しており、未購入ではない**（[hardware-bom.md](hardware-bom.md) `RES-PULL-01`）。**残るのは必要な本数と抵抗値の選定と実装であり、手元の2種で足りるとは限らない。****この行は`TBD`セルではなく、同じ行の登録時の記述「未購入・未選定」から登録した**（**同記述は2026-08-15に「一部入手済み・未選定」へ訂正済みである。登録根拠として引いた文言は現在のBOMには無い**）（[区別の基準](#区別の基準)） | 抵抗値の選定（ESP32のGPIO駆動能力とservo側入力インピーダンスから決める）。要件は[gpio-assignment.md](gpio-assignment.md#信号inventory)の`SERVO-PWM`行、部品は[hardware-bom.md](hardware-bom.md) `RES-PULL-01`（**同部品は`LCD-RST`／`LCD-CS`のpull-upもまかなう。そちらは`HW-TBD-032`**）。**選定だけではcloseしない。**必要な根拠は[HW-TBD-027の証拠契約](#hw-tbd-027の証拠契約)に定める。**値を決めた文書だけでcloseすると、pull-downが実装されていないのに`HW-TBD-019`のBlockedが外れる** | 初回統合通電、`HW-TBD-019`の起動時状態の確定 | [#2](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/2) | Human | Open（**2026-08-25に証拠契約の項目1（抵抗値と本数の選定）を満たした。`4.7 kΩ`×`1本`である。****2026-08-26に人間が、この抵抗値を一般値で開始してよい側に置くと決めた**（正本は[hardware-safety-policy.md](../governance/hardware-safety-policy.md)の対応表。「pull-upとdecouplingの値」と「サーボPWM、可動域、速度、加速度」の両方に読めたため判断を要した）。**pull-down自体が必須であることは変わらない。**導出は[gpio-assignment.md](gpio-assignment.md)の`起動時状態を確定させる外部pull`節にあり、**ここへ再掲しない。**reset時のGPIO27が`oe=0, ie=0`＝**内部pullが無い真のhigh-Z**であることをESP32 Datasheet v5.3の`IO_MUX`で確認した。**上限は計算できない。**SG90の`logic閾値`が一次資料に無いため（`HW-TBD-026`(a)）「Lowと解釈される上限電圧」が得られない。**そのため、駆動側の下限に余裕がある範囲で未知に強い側（低い値）を採った**（4.7 kΩでの駆動負荷は`IOH` typ 40 mAの1.8 %、既知漏れによる持ち上がりは0.235 mV）。**残るのは証拠契約の項目2〜4である**（購入は不要。10 kΩと4.7 kΩが各1袋100本入で着荷済みのため、**項目2は`残作業`の`未購入`を消す作業に相当する**）。**2026-08-29に、初回通電後のGPIO27を実測した（[gpio-assignment.md](gpio-assignment.md)のRevision 23）。0 Vであり、pull-down正常。USB抜き差しによる再現性も確認した。**ただし**証拠契約の項目4が求める測定条件（ESP32をresetに保持した状態で測る）を厳密には満たしていない。**測定はUSB電源投入直後の値であり、この board にfirmwareが書き込まれているかを確認していない。**firmwareが書き込まれておらずGPIO27を能動的に駆動していないなら条件を満たすが、確認できていないため、**項目4は「満たした」とは言わず、この制約つきの実測として記録するにとどめる。**同日改めて、**証拠契約の項目4が求める条件を満たす実測を行った。**servoは接続せず、servo電源も投入していない。**人間がESP32のENピンを押し続け、resetに保持した状態**（reset保持中はGPIO27が`oe=0, ie=0`のhigh-Zになることが保証されるため、firmwareの書き込み状態は条件に影響しない）で、`SERVO-PWM`（GPIO27）の信号線（緑線）とGND（オレンジ線）の間をテスターDT830B（**DC電圧20Vレンジ**）で測定し、**0.00 Vだった。**測定条件（servo未接続・servo電源未投入・ESP32をresetに保持）を満たした実測である。**ただし判定閾値は`HW-TBD-026`が決まるまで確定しないため、この値だけで「Lowである」とは結論しない。**証拠契約の項目4はこれで満たした。**項目2・項目3の状態はこの追記では確認・変更していない。****判定閾値は`HW-TBD-026`が決まるまで確定しないため、この行は026より先にcloseできない**） |
| HW-TBD-028 | P1 | **電源品質の数値制限。**(a) Pi入力で許容する最低電圧、(b) ESP32入力／3.3 Vで許容する最低電圧、(c) 最大定常ripple、(d) 最大transient droopと継続時間、(e) connector／wireで許容する最大温度上昇。**許容するbrownout／reset回数は0回で確定済みであり、この行に含めない。****2026-08-12に5項目とも規則を定義した。数値が確定したのは(d)の継続時間0だけである**（値の正は[power-budget.md](power-budget.md#受け入れ条件)の`電源品質の数値制限`節。**ここへ再掲しない**）。残りは下記のとおり他の行に従属するか、一次資料が存在しない | **実測ではなく定義が先である。**各deviceのdatasheetの動作電圧範囲と絶対最大定格、および線材・connectorの温度定格から上限・下限を定める。実測はその後の受け入れ試験で照合する。要件は[power-budget.md](power-budget.md#受け入れ条件)が挙げている一覧。**2026-08-12時点の項目別の状況は次のとおり。****(a) 一次資料が存在しないことを確認した。**Raspberry Pi公式documentationは`All models require a 5.1V supply`と述べるのみで許容範囲も最低電圧も示さず、Zero W向けのproduct briefとreduced schematicsも公開されていない。**低電圧検出のしきい値4.63 Vは`except the Zero range`と明記されており、このboardに適用されない。二次情報でも代用しない**（[AGENTS.md](../../AGENTS.md) 推測禁止）。**帰結として段階AでPiの低電圧を判定する手段が二重に無い**（判定値が無く、Pi自身の検出回路も無い。[power-budget.md](power-budget.md)の`Pi Zero Wには低電圧検出が無い`）。**(b)** ESP32 boardの5 V入力は参照設計のdropout最大1.3 Vから4.6 Vと導いたが、**参照設計由来であり現物の`U2`は未確認である**（`HW-TBD-023`）。3.3 V rail側は`HW-TBD-025`(a)に従属する。**(c)(d)** 推奨動作条件が瞬時値の制約であることから導出規則を定めた。深さは(a)(b)の下限に従属する。**(e)** 規則だけを定めた。経路部品が未選定のため数値は`HW-TBD-021`／`HW-TBD-022`／`HW-TBD-007`に従属し、**一次資料に無い温度上昇値を代わりに置いていない**。**照合できる段階の対応表も同節に置いた。****測れる段階・測定のgate・合否判定を別の列に分けている**（(a)は段階A、(b)は5 V入力側が段階B-1で3.3 V rail側が段階B-2、(c)は段階A／B-1と段階Cを分ける、(d)(e)は段階C。**段階AとB-1はgateも追加購入も要らない**）。**ただし合否判定は5項目とも`Blocked`である。**測定はgateさえ満たせば進められるが、判定には確定した閾値と検証済みの測定系の両方が要り、現状はどの項目も揃っていない。**測定値を記録してよいが、そこから受け入れの合否を導かない。****2026-08-16に、測定方式の独立性と合格主張を張ってよい範囲を`HW-TBD-034`として分離した。**この行が扱うのは**数値**であり、034が扱うのは**測定方式と主張の範囲**である。**ここへ034の内容を再掲しない。****2026-08-25に、5項目それぞれが「確定済み」「どの行に従属するか」「一次資料が存在しないのか」のどれなのかを、正本の表の列として揃えた**（[power-budget.md](power-budget.md)の`電源品質の数値制限`。**数値は1つも追加していない。**既に本文と状態列にあった従属関係を項目ごとに書き出したものである。**列の内容をここへ再掲しない**）。**あわせて(e)について、一次資料が存在しない要素が1件確定した。**`HW-TBD-021`で選定したヒューズホルダである（**確認の内容と根拠は[power-budget.md](power-budget.md)の`経路部品と定格`のヒューズホルダ行が正。ここへ再掲しない**）。**この要素は従属先が解けても数値が出ない** | 電源系の受け入れ承認、合成給電（段階C）の合否判定 | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Joint | Open（**規則は5項目とも定義済み。数値は(d)の継続時間0のみ確定。**(a)は一次資料が無いため確定せず、(b)〜(e)は`HW-TBD-004`／`023`／`021`／`022`／`007`と、`HW-TBD-025`(a)の下限の両立に従属する。**照合（実測）は未着手である。段階A／B-1は他のgateを要さずに測れるが、測れることと合否を判定できることは別である。**(a)は判定値も測定系も無いため、段階Aで得た値から受け入れの合否を導かない。**(c)(d)は閾値が確定しても`HW-TBD-034`が解けるまで合否を判定できない。****2026-08-25に5項目の内訳を正本の表へ列として明示した**（確定は(d)の継続時間0と(b)の5 V入力側、一次資料無しは(a)と(e)のヒューズホルダぶん、残りは従属。**正は[power-budget.md](power-budget.md)であり、ここへ再掲しない**）。**Ownerは`Joint`のままである。**2026-08-25時点ではこの行に「数値をAIが決めない」と書いていたが、**2026-09-06にPM（`deskcat-f2`）が(a)を受け入れ試験の閾値として要求しない（要件から外す）と決定した。**根拠・範囲（Pi側の(c)(d)も対象外、ESP32側の(b)(c)(d)と`HW-TBD-007`／`HW-TBD-009`と`#247`の診断は対象外にしない、(d)の継続時間0は確定のまま）・復帰条件の正は[power-budget.md](power-budget.md#受け入れ条件)の`(a)を要件から外す（2026-09-06、PM決定）`節。**ここへ再掲しない。**この決定に至る過程で一度(a)へ暫定値4.63 Vを置き取り下げた経緯（PMが`power-budget.md`の`(a)の一次資料は存在しない`節を読まずに決定したことが原因。PM自身が発見しrevertした）も同節に記録した。**(a)以外の数値・判定は、AIが提案し人間が承認する形で決める**） |
| HW-TBD-030 | P1 | **逆極性保護の要否と方式。**登録時点は[power-budget.md](power-budget.md)の`配線・保護表`で`TBD`／`Blocked`のまま残っていた（**同行は2026-08-25に`一部確定`へ変えた。**下記）。配線ミス時にPi、ESP32、周辺module3点が同時に破損しうる。**2026-08-12にdesign reviewの材料と推奨案を用意した**（[power-budget.md](power-budget.md)の`逆極性保護のdesign review`節）。**登録時点では決定していない。**採否はAIが提案し人間が承認する。その承認を経るまで確定として扱わない。**要否と方式は2026-08-24に承認された**（状態欄。**行はcloseしていない**）。**2026-08-25に承認内容の1の品番が確定した**（東芝`2SJ334`。[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)。**同日に一度保留され、同日のうちに解除された。**経緯と品の定格の正は[power-budget.md](power-budget.md)の`逆極性保護のdesign review`節内の記録である。**ここへ再掲しない**）。**同日に承認内容の2の手順も作成した**（同文書の`通電前の配線確認手順`）。**1と2は択一ではなく、守る区間が違う** | Design review。**2026-08-12に用意した材料**: (1) 逆接が起こりうる区間の洗い出し。**極性付きconnectorで固定される区間（M-12001→変換基板、rail→Piの`PWR IN`）では起こらず、起こりうるのは手配線区間だけである。**したがって**ingress 1箇所に保護部品を入れても下流の手配線区間は守れない。****この区間の分類は2026-08-24に変わった。**rail側の受け方が②（直付け）で確定してrail側が手配線になったため、**`rail→Piの PWR IN`は`可`である**（正は[power-budget.md](power-budget.md)の`逆極性保護のdesign review`の逆接可否表。**ここへ再掲しない**）。**2026-08-12の材料としての記述は残し、分類の訂正をここに置く。**(2) 方式ごとの電圧降下の代償。**直列Schottkyは5 V予算を食う。ただし採否は確定しない。**比較対象の4.6 Vは`IOUT ≤ 0.8 A`かつ参照設計の`U2`という条件付きの導出値であって確定した合格下限ではなく、**現物の`U2`と実負荷を確認するまで判断できない**（`HW-TBD-023`、`HW-TBD-028`(b)）。P-ch MOSFETによるideal diodeは降下が小さいが品番を選ばないと具体値が出ない。逆並列diode＋fuseは溶断までの間、負荷に逆電圧が掛かる。(3) 推奨案はingressへP-ch MOSFETを置き、**下流の手配線区間は保護部品ではなく極性が固定される接続方法と通電前の配線確認手順で守る**というもので、**2026-08-24に人間がこれを承認した**（状態欄）。**品番と定格は`PROT-OC-01`と同じく発注直前に一次資料で確定する。**(4) **2026-08-25に品番が確定した**（東芝`2SJ334`。[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)。**同日に保留され同日のうちに解除された。**定格と経緯の正は[power-budget.md](power-budget.md)の`逆極性保護のdesign review`節内の記録。**ここへ再掲しない**）。(5) **承認内容の2の実体である通電前の配線確認手順を2026-08-25に作成した。**確認箇所、方法（目視と導通）、記録先、通電前であることの明記、**守る区間と守らない区間の書き分け**を含む（正は[power-budget.md](power-budget.md)の`通電前の配線確認手順`。**ここへ再掲しない**） | 合成給電（段階C）の配線承認 | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**要否と方式は2026-08-24に人間が承認した。closeはしない。**承認内容の正は[power-budget.md](power-budget.md)の`逆極性保護のdesign review`の`決定（2026-08-24）`である（**ここへ再掲しない**）。**2026-08-24時点で残っていたのは(1) 品番と定格の一次資料での確定、(2) 通電前の配線確認手順の作成、(3) 極性が固定される接続部材の選定**（`HW-TBD-022`の`WIRE-PWR-01`側で選ぶ）**である。****2026-08-24のreview指摘で、(2)(3)の対象にrail → Piの`PWR IN`区間が加わった。**②（直付け）を採ったことでrail側が手配線になり、端子台でVBUSとGNDを入れ替えるとPiへ逆電圧が掛かる。**ingressのMOSFETより下流であり、承認内容の1では守れない。****方式が決まっただけでは段階Cの配線承認は与えられない。****2026-08-25に(1)の品番が確定し、(2)を作成した**（[#205](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/205)。品番は東芝`2SJ334`。**同日に保留され同日のうちに解除された。**正は[power-budget.md](power-budget.md)の`逆極性保護のdesign review`節内の記録と`通電前の配線確認手順`）。**2026-08-26に人間がcloseしないと判断した。**判断の理由は下の(ii)〜(iv)であり、**いずれも「記録が足りない」ではなく「実際に守られていない」ものである。****(i) 承認内容を満たしたと見るかは、AIが提案し人間が承認する。2026-08-26時点では「まだ満たしていない」である。****(ii) MOSFETが未実装である。****実装方法（向き、ゲート抵抗の値、基板上の配置）は2026-09-03に決定した**（正は[power-budget.md](power-budget.md)の`実装方法を決定した（2026-09-03）`。**決定しただけであり、実装そのものはまだ済んでいない。**）。**実装されるまで経路に逆極性の保護部品は1つも無い。****(iii) 色分けを成立させる接続部材が未選定である**（`HW-TBD-022`の`WIRE-PWR-01`）。**(iv) `3V3` pin → 周辺module3点の区間について、通電前の配線確認手順を実施した記録が無い。****2026-08-26に同手順の対象へ加えたが**（[#236](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/236)。正は[power-budget.md](power-budget.md)の`通電前の配線確認手順`の確認箇所表）、**対象に入れたことと実施したことは別である。**実施と記録は段階B-2の実施時であり、まだ行っていない。**なお、この区間を承認内容の1（ingressのP-ch MOSFET）が守らないことは、closeの条件ではない。****MOSFETはingressに1個入るだけであり、下流の手配線区間を守らないのは設計上の限定である。****恒久的に真であるため、close条件に置くとこの行は永久にcloseできない**（2026-08-26に(iv)の書き方を改めた。[PR #238](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/238)のreview指摘）。**守る手段は2の側であり、2の実施記録を(iv)の条件とする。****この3つが解けたら再度Humanが判断する。**closeの条件は「方式が決まったこと」ではなく「守れる状態になったこと」だと2026-08-26に定めた。**2026-09-04追記: [PR #342](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/342)のCodeRabbit `full review`の指摘を受け、(ii)の向き（Source/Drain）を訂正した**（内蔵diodeがchannelと並列であるため、旧い向きでは逆接時にchannelがOFFでもdiodeが導通していた。正と検証は[power-budget.md](power-budget.md)のRevision 87）。**(ii)（MOSFET未実装）の状態は変わらない。この行はcloseしない** **2026-09-09訂正: 上の`2026-08-12にdesign reviewの材料と推奨案を用意した`欄が「極性付きconnectorで固定される区間（M-12001→変換基板、rail→Piの`PWR IN`）では起こらず」としていたが、2026-09-09にPM（`deskcat-f2`）の設計判断（案B）により`M-12001 → 変換基板`（現`端子台`）は手配線へ変わり、この前提は成立しない。**同区間の逆接は、PMが基板上の配置を実測で確認したうえでMOSFET（`PROT-RP-01`）が守ると判定した（正は[power-budget.md](power-budget.md)の`逆極性保護のdesign review`の`逆接可否表`。**ここへ再掲しない**）。**過去の記録（2026-08-12時点の材料）は書き換えない。ここに訂正を記す** |
...
| HW-TBD-035 | P0 | **servoへ入れてよい電圧。**`PSU-SERVO-01`はM-12001（5 V）から分岐したservo railであり、**この5 Vをservoへ入れてよいかを確認した文書が無い。**[hardware-bom.md](hardware-bom.md) `SERVO-01`の`定格電圧範囲`は`4.8–6 V`だが、**2026-08-24の一次資料確認で、この範囲の出所が仕様表ではなく`SG90 Digital`ページのQ&A欄にある`admin`（メーカー側）の回答だと判明した**（2021-11-07。`Voltages from 4.8V - 6V are fine.`）。**製品ページの仕様表が載せる電圧は`Operating voltage: 4.8v`だけであり、供給予定の5 Vはこれより上である。**つまり**5 Vの可否は、仕様表ではなくQ&A欄の1文だけに依っている。**しかも**`SG90 Analog`ページは`Reviews (0)`でこの記述が無く、現物がどちらの品かも未確定である**（`HW-TBD-026`）。**そして同名の3品目`SG90 360 degree`は、仕様表そのものが`Operating voltage: 4.8v-6V`を載せている。****つまりどの品かによって、`4.8–6 V`に仕様表の裏付けがあるかどうかが変わる。**3品目は`Operating Angle: 360 degree`の連続回転で寸法も`23×12.5x22mm`（他2品は`23×12.2x29mm`）と異なるため、**現物とは別物である可能性が高い。**2026-08-28の実測（27 mm、[Issue #257](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/257)）で`SG90 360 degree`ではないと判定した（下の証拠契約の条件1。**Digitalと Analogの分離は未達のまま**）。**[power-budget.md](power-budget.md)の`許容電圧範囲`の積集合は3.3 V railの周辺module3点とESP32の`VDD33`だけを対象としており、servoを含まない。**servo railは5 Vで別系統のため積集合の対象外であるが、**代わりに確認する項目も存在しない。****したがってservoの入力電圧は、どの文書でも適合を確認されていない。**扱いは`HW-TBD-004`の`moduleへ入れてよい電圧`と同じ型である（IC・部品の定格から基板・製品の許容入力を導かない） | **メーカーの一次資料では埋まらない見込みである。**公式のdatasheetが存在せず（`HW-TBD-026`）、仕様表は`4.8v`単独である。**Q&A欄の回答を根拠として採ってよいかは2026-08-26に決まった**（[ADR-0017](../decisions/0017-what-counts-as-a-primary-source.md)。メーカー自身による仕様書以外の記述は3条件を満たすときだけ使える）。**この行の`4.8–6 V`は、その乙の条件2（どの品に対する記述かが特定できる）を満たさないため、現状では採れない。**採れないあいだに残る手段は、(1) メーカーへの問い合わせ、(2) 4.8 Vを供給する構成へ変える、(3) 監視下で5 Vを入れて挙動を観察する、のいずれかである。**(3)は通電を伴うため`007`／`009`のgateに従う。****この行では5 Vを入れてよいと決めない。****closeに必要な記録は[HW-TBD-035の証拠契約](#hw-tbd-035の証拠契約)に定める。**関連は[servo-safety-limits.md](servo-safety-limits.md#サーボ識別情報)の識別情報表と[power-budget.md](power-budget.md)の`許容電圧範囲` | servoへの給電、`SERVO-PWM`の駆動条件の確定、servoの実機動作全般 | [#17](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/17) | Human | Open（**2026-08-24に登録した。**`HW-TBD-026`の調査で判明した、追跡されていなかった項目である。**2026-08-26に、Q&A欄の回答を根拠として採ってよいかの規則が決まった**（[ADR-0017](../decisions/0017-what-counts-as-a-primary-source.md)。乙は3条件を満たすときだけ使える）。**この行の`4.8–6 V`は、その乙の条件2（どの品に対する記述かが特定できる）を満たしていない**（**下の証拠契約の条件番号とは別の体系である。**理由と追跡先は[HW-TBD-035の証拠契約](#hw-tbd-035の証拠契約)が持つ）。**したがって残るのは規則の判断ではなく品の特定であり、`対応Issue`列を[#17](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/17)単独へ改めた**（2026-08-26。従来は[#193](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/193)を併記していた））。**2026-08-25に、証拠契約の項目1のうち「3品目`SG90 360 degree`の除外／該当」を決める判定基準を確定した。****項目1の全体を解く基準ではない。**寸法で分離できるのは3品目だけであり、**`Digital`と`Analog`の判別には使えない**（両者は寸法が同一である）。**その分離は試みない**（通電を要して`007`／`009`によりBlockedであり、かつこの行の判断には影響しない。どちらも仕様表は`4.8v`単独である）。判定は寸法で行い、**高さが29 mm前後なら`Digital`または`Analog`、22 mm前後なら`360 degree`である**（正は`HW-TBD-026`の状態欄。**ここへ再掲しない**）。**3ページの仕様表は2026-08-25に一次資料から直接再確認した。****この判定が[ADR-0017](../decisions/0017-what-counts-as-a-primary-source.md)の段を決める。**`360 degree`なら仕様表そのものが`Operating voltage: 4.8v-6V`を載せているため**甲（メーカーの仕様書）**になり、他2品なら**仕様表は`4.8v`単独で、`4.8–6 V`の出所はQ&A欄の`admin`回答だけ＝乙（メーカー自身による仕様書以外の記述）**にとどまる。**そして寸法の実測は、どちらに転んでも要る。**同policyは**甲についても「どの品に対する仕様書かが特定できること」を要求し、「これは乙の条件2と同じ問題であり、甲だから免除されるのではない」と明記している。****つまりこの実測は、乙の条件2を解くためだけでなく、甲を使う場合の前提としても必要である。****したがって項目1を解かないまま条件4（合否判定の基準）を決めると、根拠の強さを取り違える。****2026-08-28に高さを実測した（[Issue #257](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/257)。27 mm。正は`HW-TBD-026`の状態欄。ここへ再掲しない）。**[PR #258](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/258)がIssue本文の記述をこの行へ反映し漏らしていたため、2026-08-29に反映する。**`SG90 360 degree`ではないと判定できたため、[ADR-0017](../decisions/0017-what-counts-as-a-primary-source.md)の段は乙（メーカー自身による仕様書以外の記述）に確定した。****ただし`Digital`と`Analog`の分離はこの実測ではできず、項目1が求める「現物の品の特定」は`Digital`か`Analog`のどちらであるかの1点が未達のまま残る。この行はcloseしない**）。**2026-08-30、人間が袋に`TH270`と印字されていることを報告した。**現物識別（証拠契約の項目1）の新しい手掛かりになりうるため調査した。**結論は「不明」である。**TowerPro公式site（SG90 Digital／Analog／360 degreeの3ページ、Downloadページ）、servodatabase等のSG90関連の実在ページ、主要な販売代理店ページのいずれにも`TH270`という文字列は見つからなかった。**実在が確認できた`TH270`は4件あり、いずれもservoとも本リポジトリの部品とも無関係である**（DMC／Daniels製crimp tool用turret head `M22520/1-12`、Standard Motor Products製throttle position sensor、Sencor製weather station用sensor、Tommy Hilfiger腕時計の型番）。**Web検索toolの要約が「Tower Pro TH270 SG90 Micro Servo」という趣旨の記述を返したことが複数回あったが、実際の検索結果リンクの中にその表題を持つものは1件も無く、要約段階での誤生成と判断した。この記述を採らない**（[AGENTS.md](../../AGENTS.md) 推測禁止。丙にも該当しない出所不明の記述）。**したがって`TH270`は現物識別にも`HW-TBD-035`の根拠にも使えない。**袋の表示自体（`TH270`のみ、他に記載なし）は[hardware-bom.md](hardware-bom.md) `SERVO-01`へ記録した。**項目1の未達（`Digital`／`Analog`の判別）はこの調査によっても変わらない** |
...
`Owner`列は、**その行に残っている行為を誰が行うかを示す。****「誰が中身を決めるか」を示す列ではない。**

| 値 | 意味 |
|---|---|
| `Human` | 人間しかできない行為だけが残っている（現物を見る、はんだ付けする、買う、監視下で通電する） |
| `Joint` | 人間しかできない行為と、机上で閉じる行為の両方が残っている |
| `AI` | 机上で閉じる行為だけが残っている（一次資料の読み取り、導出、値・方式・要否の提案） |

**この列は、提案を誰が出すかを示さない。****提案はAIが出し、承認は人間が行う。**
これはどの行でも同じであるため、列では区別しない。

`Owner: Human`は「人間が値を決める」ではなく、「**人間が手を動かさないと先へ進まない**」を意味する。
**`Owner`を、AIが提案を出さない理由として引用しない。**AIが提案を出さない理由になるのは、
一次資料が無いこと（[AGENTS.md](../../AGENTS.md) 推測禁止）と、実測が要ることだけである。

**この定義は2026-09-07に置いた。****それ以前に各行へ付けた値は、この定義に照らして付けたものではない。**
各行の値は、その行の`対応Issue`を進めるときに、残っている行為を読み直して合わせる。
...
**条件ごとの追跡先。**条件1〜3は[#17](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/17)（サーボ電気動作のcharacterization）の範囲である。同Issueの受け入れ条件は`正確なservo dataを記録した`・`外部電源と電流制限を記録した`・`5 Vと3.3 Vのtransientを取得した`を持つ。**ただし`servoの入力端での電圧測定`を名指ししてはいない。**#17を追跡先に置くのはservoの電気動作の検証がそこへ集約されているためであり、**条件2・3の文言そのものはこの台帳が持つ。****品の特定そのものは`HW-TBD-026`（同じく#17）が持つ。****条件4が待っていた規則は、2026-08-26に[ADR-0017](../decisions/0017-what-counts-as-a-primary-source.md)として決まった。**メーカー自身による仕様書以外の記述（乙）は3条件を満たすときだけ根拠にできる。**この行の`4.8–6 V`は、その乙の条件2（どの品に対する記述かが特定できる）を満たさない**（同名3品があり現物がどれか未確定）。**ADR-0017の乙の条件番号であり、この節の条件1〜4とは別の体系である。****したがって条件4に残るのは規則の判断ではなく、条件1が解けた後の適用であり、それは条件1〜3と同じ#17の範囲である。****`対応Issue`列を#17単独へ改めた**（2026-08-26。従来は#193を併記していた）。**条件そのものも判定基準もここで変えていない。**
...
`検証済み`ではなく`build検証済み`にとどまっていた理由の一方であった。
**2026-08-20に満たされ、同文書は`検証済み`へ昇格した**（`esptool` 4.12.0が`ESP32-D0WD`を報告した）。
**この行は解決手順を通して2026-08-25にcloseした。**登録対象としての性質（識別項目）は変わらない。
...
| 発注状態のみ | 21 | 部品は決まっており購入していないだけ。**調達はIssueで追う**（[購入待ちリスト](hardware-bom.md#購入待ちリスト)節が方針を持つ） |
...
`HW-TBD-004`以下の未解決項目により`Fail／TBD`のままである（**`HW-TBD-001`は2026-08-15に、`HW-TBD-002`は2026-09-06にcloseしたため、
この代表例を差し替えた**）。**分類の完了は、
...
| HW-TBD-006 | 正確なservo modelを`TowerPro Micro servo 9g SG90`と確定した。**`SG90 Digital`のQ&A欄でメーカーが許容と回答した電圧範囲は4.8–6 V**、動作電流は0.5–2 A（負荷依存）。**2026-08-26に、出所と欄名の記載を訂正した。****旧記載は`定格電圧4.8–6 V、stall電流はデータシート値0.5–2 A`だった。****この2値はデータシート値ではない。**公式siteの`SG90 Digital`ページのQ&A欄にある`admin`（メーカー側）の回答（2021-11-07）にのみ由来し、**仕様表の記載は`Operating voltage: 4.8v`だけである。**また`0.5–2 A`はメーカーが`operation current`として述べた値であり、**stall電流ではない**（`The operation current for our SG90 is about 0.5A - 2A.`）。**`定格`という語を外したのは、出所の格を決めたからではない**（その判断は#193に残る）。**値は1つも変えておらず、この行を再openしたのでもない**（[PR #194](https://github.com/wachi-yoshitaka-11-dev/deskcat/pull/194)が`TBD`へ戻さない判断を下している）。**同名の品が複数あり現物ラベルで判別できない点は`HW-TBD-026`が正である**（ここへ再掲しない）。**Peak／stall電流の実測は本行の範囲外**であり、`power-budget.md`の測定計画と`HW-TBD-010`／`HW-TBD-011`で追跡する | 現物ラベルの写真（`TOWER PRO Micro servo 9g SG90`）、[TowerPro公式 SG90 Digital](https://towerpro.com.tw/product/sg90-7/)（仕様表と、`admin`回答のあるQ&A欄）。**[Soldered_101246.pdf](https://www.mouser.com/catalog/specsheets/Soldered_101246.pdf)を`datasheet`として引いていた記載は、2026-08-26に外した。**TowerPro発行と確認できておらず、[hardware-bom.md](hardware-bom.md) `SERVO-01`は同じPDFを**「一次資料の扱いにしない」**としている。**2箇所が同じ資料へ別の格を与えていた。**反映先: [hardware-bom.md](hardware-bom.md) SERVO-01、[power-budget.md](power-budget.md)負荷表、[gpio-assignment.md](gpio-assignment.md) SERVO-PWM | 2026-08-05 |
...
| HW-TBD-031 | **搭載moduleの識別と中核chipの識別の両方を満たした。**module刻印は2026-08-13に読了した（`ESPRESSIF` / `ESP32-WROOM-32D` / `FCC ID : 2AC7Z-ESPWROOM32D`。斜光＋接写）。**中核chipの識別は、要件を2026-08-15に書き換えた。**旧要件「chip刻印の読み取り」は**非破壊では成立しない**（chipは半田付けされた金属シールドの内側にあり、シールドの除去は分解であるため範囲外）。同日に満たし方を`esptool`が報告するchip名へ再定義し、**2026-08-20に[#6](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/6)のflash時に実施した。**`esptool` 4.12.0が`ESP32-D0WD`を報告し、期待値に一致した。**`espflash`の出力では満たせない**（family名しか返さない）。**主張するのはchip名が期待値に一致したことまでであり、周辺回路・servo・Protocol・電源品質は含まない。**満たし方の正本は[esp32-rust-toolchain.md](../toolchains/esp32-rust-toolchain.md)の`chip 識別の満たし方`であり、**ここへ再掲しない。**<br>**解決手順6（影響文書の更新）**: [esp32-rust-toolchain.md](../toolchains/esp32-rust-toolchain.md)の状態を`build検証済み`から`検証済み`へ昇格し、同文書内に残っていた「この項目は未達のままである」を訂正した。[implementation-readiness-review.md](../runbooks/implementation-readiness-review.md)の「未達なのは次の1項目である」も訂正した。本台帳の`登録範囲`節にあった「`build検証済み`にとどまる理由」も過去形へ直した。<br>**解決手順7（関連Issueのblockedからreadyへの変更）は対象が無い。**031に言及するIssueは[#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)（open）、[#6](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/6)（closed）、[#133](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/133)（closed）の3件だけであり、**openな#1は`status:blocked`を持たない**（2026-08-25にlabelを確認）。**031だけにblockされていた行も無い。**この行の`妨げる対象`はIssueではなく正本文書の状態であった | module刻印は現物の斜光＋接写（2026-08-13）。chip識別は[Version Record 2026-08-20-esp32-flash-boot-native](../toolchains/version-records/2026-08-20-esp32-flash-boot-native.md)の`chip 識別（HW-TBD-031）`節（`esptool`の未加工出力を収録）。満たし方の正本は[esp32-rust-toolchain.md](../toolchains/esp32-rust-toolchain.md)の`chip 識別の満たし方`。chipの品番の出所は[ESP-WROOM-32D datasheet v2.7](https://documentation.espressif.com/esp32-wroom-32d_esp32-wroom-32u_datasheet_en.pdf)。反映先: [hardware-bom.md](hardware-bom.md) `MCU-01` | 2026-08-25 |
| HW-TBD-002 | **残っていた3点（pin配列の現物照合、電源pinの現物照合、board上のdecoupling実装の確認）をすべて満たした。**pin配列は2026-08-13の現物確認で既に照合済みだった（裏面ラベルが左→右に`T_IRQ`〜`VCC`、datasheetの1=VCC…14=T_IRQとは逆順であることを確認済み）。**電源pinの現物照合は2026-09-05に、非通電導通測定で行った。**`VCC`ピンが`U1`（3.3V LDO、刻印`662K`／`UMW S4T`）の入力側の脚と導通し、`GND`ピンが`U1`のGND側の脚と導通することを確認し、両ピンがラベル通りの電源ネットであることを裏付けた。**decoupling実装は`C1`（積層セラミック、`U1`近傍）1個の存在を接写で確認した。容量表記は無く値は不明のままだが、これはメーカーdatasheetに記載が無いことが`HW-TBD-029`で既に確定しているため、現物確認で得られる情報の上限である。**backlight回路のLED接続経路（`VCC`側か`U1`出力側か）は非通電測定では決定できないと判明したが、これは`HW-TBD-024`（LED個数・電流上限）の範囲であり、この行（pin配列・電源pin・decoupling）には含まれない | pin配列: 現物写真（2026-08-13、斜光＋接写）と[メーカーdatasheet](https://akizukidenshi.com/goodsaffix/msp2807.pdf)＋[LCD Wiki](http://www.lcdwiki.com/2.8inch_SPI_Module_ILI9341_SKU:MSP2807)の14pin定義。電源pin: 2026-09-05の非通電導通測定（DT830B、`2000Ω`レンジ）。decoupling: 2026-09-05の接写。反映先: [hardware-bom.md](hardware-bom.md) `DISP-01`、[sensor-datasheet-notes.md](sensor-datasheet-notes.md)のBacklight行 | 2026-09-06 |
| HW-TBD-032 | **`LCD-RST`／`LCD-CS`／`LCD-BL`の外部pull（`RES-PULL-01`）の抵抗値・本数・極性の選定、購入、実装、起動時の状態確認3点をすべて満たした。**`LCD-CS`／`LCD-RST`は10 kΩ×1本（active-low、pull-up）、`LCD-BL`は4.7 kΩ×1本（active-high、pull-down）で確定済み。**2026-09-07に`LCD-BL`をブレッドボードへ実装し、3信号すべての実装が完了した。**同日、`EN`を押し続けてESP32をresetに保持した状態（firmware非動作）で3信号を再測定し、`LCD-CS`(GPIO22)＝3.31 V、`LCD-RST`(GPIO16)＝3.31 V、`LCD-BL`(GPIO4)＝0.00 Vを得た（いずれも期待どおりで`EN`押下の有無による変化なし）。**`LCD-CS`／`LCD-RST`は内部pullが無い（`oe=0, ie=0`）ため、この3.31 Vは外部pull-upがfirmwareのHigh駆動と切り分けて有効であることの証拠になる。**`LCD-BL`（GPIO4）は内部weak pull-down（`wpd`）が有効なため、0.00 Vだけでは外部pull-downの存在を切り分けられない（PM指摘、2026-09-07）。**ただし項目6が要求する「reset中に安全な状態（backlight Off）」自体はこの0.00 Vで満たしている** | [gpio-assignment.md](gpio-assignment.md)の`起動時状態を確定させる外部pull`節・`LCD-BLを4.7 kΩにした理由`節・受け入れchecklist、[experiment-log.md](experiment-log.md) `EXP-011`、[#2](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/2) | 2026-09-07 |
