...
| HW-TBD-004 | P0 | **残: interface選択jumperと実装済みI2C addressの現物確認。**module／ICはADXL345（秋月 M-06724）と特定済み。**module boardの資料が現在入手できない**（秋月 M-06724の商品ページは`gM-06724`／`g106724`とも404。2026-08-10確認）ため、jumper構成をIC datasheetから推定せず現物で読む | 現物のjumper／address pin設定の確認、および基板上にregulatorが無いことの確認。ICの根拠は[Analog Devices ADXL345 datasheet](https://www.analog.com/media/en/technical-documentation/data-sheets/adxl345.pdf)（**`analog.com`へは本作業環境から接続できず、内容は未検証**）。特定の根拠: [hardware-bom.md](hardware-bom.md) ACCEL-01 | Accelerometer driverとしきい値 | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1) | Human | Open（範囲を縮小） |
...
| HW-TBD-023 | P0 | **ESP32 boardの`3V3` pinから外部負荷を取ってよい条件。**(a) 外部供給可能電流の定格、(b) board上regulatorの種別（LDO／switching）、(c) **過電流保護／短絡保護の有無と動作**（制限電流、折り返し特性、熱shutdownの有無）。**(b)は解決した: `U2 = AMS1117-3.3`、すなわちLDOである。**(c)も**board levelでは解決した: 回路図上にヒューズ・ポリスイッチは存在しない**（`LESD5D5.0CT1G`×3はESD保護であって過電流保護ではない）。したがって短絡・配線ミス時に電流を制限しうるのは**`AMS1117-3.3` IC自身の内蔵保護だけ**である。**残: (a)の定格と、(c)の内蔵保護の実際の特性。**どちらも**実装品のメーカーが決まらないと確定できない**（`AMS1117-3.3`は複数社が製造する汎用品で、電流制限値・熱shutdownの有無と挙動が社ごとに違う）。**Espressifは`3V3` pinの外部供給可能電流をboard levelで公開していない** | (b)(c)の根拠: [ESP32-DevKitC V4公式回路図](https://dl.espressif.com/dl/schematics/esp32_devkitc_v4-sch.pdf)。経路は`VBUS → D3 BAT760-7（Schottky）→ EXT_5V → U2 AMS1117-3.3 → VDD33 → J2 pin1`、入出力とも22µF（2026-08-10確認）。**残る(a)と(c)は、現物のU2刻印でメーカーを特定し、そのメーカーのdatasheetを当てる。**要件は[power-budget.md](power-budget.md#段階b-2の測定)の`B-2a: 3V3 pinから給電する経路`、確認項目は[hardware-bom.md](hardware-bom.md) `MCU-01`と部品受け入れchecklist | **段階B-2a**。B-2bへ移れば回避できるが、`HW-TBD-024`／`025`は経路によらず必要なため、それらが未解決ならどちらの経路も進まない | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)、[#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（範囲を縮小。`HW-TBD-001`と同じ現物boardで確認できるが、**001の完了を待つ必要はない**） |
