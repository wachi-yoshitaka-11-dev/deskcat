...
|---|---|---|---|---|---|---|---|
| HW-TBD-001 | P0 | **照合は2026-08-13に完了し、一致した。**38pinヘッダ両側のsilkが公式`J2`／`J3`のpin description表と**19pin×2列すべてで一致**した（GNDの位置を含む）。module suffix（ESP-WROOM-32D）と「基板にrevision表示なし」も現物確認済みで、**module刻印`ESPRESSIF` / `ESP32-WROOM-32D`も読了した**（`HW-TBD-031`）。**残: 無し。**close可否は解決手順1–8に従い、影響先（`gpio-assignment.md`、`esp32-rust-toolchain.md`の確定条件）の更新を含めて判断する。**旧記載の「秋月独自基板のため」は根拠が無いため2026-08-10に削除した**（[hardware-bom.md](hardware-bom.md) Revision 29） | **照合先はEspressif公式資料である。正は[ESP32-DevKitC V4公式回路図](https://dl.espressif.com/dl/schematics/esp32_devkitc_v4-sch.pdf)**（`J2`／`J3`はいずれも`CON19X1_2P54`で計38pin。2026-08-10に図面を直接読み、19pin×2列の対応を取得済み）。[公式guide](https://docs.espressif.com/projects/esp-idf/en/v5.1/esp32/hw-reference/esp32/get-started-devkitc.html)のpin description表も同じ並びを示すが、**HTML経由で取得した際に`J3`の一部で番号表示に乱れがあったため、番号の正は回路図とする**。**秋月商品ページの添付資料はESP-WROOM-32Dモジュールとチップのdatasheetだけで、boardのpin配列表も回路図も含まないため照合先にならない。**確定済み部分の根拠: [hardware-bom.md](hardware-bom.md) MCU-01、[gpio-assignment.md](gpio-assignment.md#board識別情報)。**2026-08-13の現物確認で読み取った並びは次のとおりで、公式表と一致した。**左列（`J2`）: `3V3 EN VP VN 34 35 32 33 25 26 27 14 12 GND 13 D2 D3 CMD 5V`。右列（`J3`）: `GND 23 22 TX RX 21 GND 19 18 5 17 16 4 0 2 15 D1 D0 CLK`。**同じ機会に`HW-TBD-023`／`HW-TBD-031`の現物確認も行った** | 最終GPIO割り当て | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1) | Human | **Open（根拠は揃った。closeはこのPRで行わない）。**照合は完了して一致したが、closeすると`002`／`007`／`008`／`009`／`021`／`023`／`030`のblocker表記の更新と`解決済み項目`表への移動が要る。**それらは別途まとめて行う** |
...
| HW-TBD-015 | P1 | **残: 耐久性の確認と、書込み速度がClass表示に達しない原因の切り分け。****使用前のhealth checkは実施済みである**（判定`Partial`。[sd-health-check.md](sd-health-check.md)）。**実施日は`f3`による検査が2026-08-12、registerの読み方の一次資料照合とraw device試験が2026-08-13である。**`f3`が書き込んだ全空き領域（29.80 GiB）へ位置依存dataを書いて読み戻し`Data LOST`は0 sectorであった（容量詐称は確認されず、検査範囲に読み書き不良も確認されなかった）ため、**`妨げる対象`のうちDeploy側は解けた。**識別情報はSamsung EVO Plus microSDHC 32GBと現物print確認済みで、card内部register（`manfid`／`oemid`／`name`／`csd`／`scr`／`ssr`）を読み、2026-08-13に読み方をPhysical Layer Simplified Specification Ver 9.10と照合した。**`32`／`microSDHC`／`U1`はregisterがprintを裏付けたが、`Samsung`だけは裏付けられない**（仕様書がMIDの登録簿を収録していないため）。**この照合は打ち切った。**確認した仕様書とSD-3Cの資料のいずれもMIDと社名の対応表を持たず、この経路で解決する見込みが無いためである（[hardware-bom.md](hardware-bom.md) Revision 38、[sd-health-check.md](sd-health-check.md) Revision 12〜13）。**メーカー表示の正は引き続き現物printであり、`manfid`による照合は`対応Issue`のどちらの範囲にも含まない。****残る2点**（**これが[#117](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/117)の範囲であり、同Issueは`manfid`照合を含まない**）: (a) 耐久性は1回のcheckでは判定できない。(b) 書込み速度6.11 MB/sが`U1`の10 MB/sに達しておらず、**cardとhost controllerのどちらが効いているかを分離していない**（測定は`sd high-speed`で行われ、`U1`の規定が適用されるUHS modeではない）。**FAT32だけでは未達を説明できないことは示した**（raw deviceへの直接書込みでも7.4 MB/s）。**ただし各要因の寄与率は分離していない** | (a) 耐久性: 使用中のlog。(b) 速度: **別のhost controllerで同じcardを測る。**raw device試験は実施済み。実施済み分の根拠: [sd-health-check.md](sd-health-check.md)、識別の根拠: [hardware-bom.md](hardware-bom.md) SD-01 | 耐久性（**Deployは解決済み**） | [#114](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/114)、[#117](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/117) | Human | Open（範囲をさらに縮小） |
...
| HW-TBD-031 | P1 | **搭載moduleと中核chipの識別を、現物の刻印で確かめる。****2026-08-13に要件を書き換えた。**旧要件は「ESP32 chipの刻印の読み取り（反射を避けた撮影、または実体顕微鏡）」だったが、**この要件は非破壊では成立しない。**中核chip `ESP32-D0WD` はESP-WROOM-32Dの**半田付けされた金属シールドの内側**にあり、シールドを外さない限りどのような撮影手段でも見えない。シールドの除去は分解であり、[Hardware Safety Policy](../governance/hardware-safety-policy.md)の範囲でも#1の範囲でもない。**module刻印の読み取りは2026-08-13に完了した**（下記）。**残: 中核chipの識別を、刻印以外の非破壊手段で確認するか、確認しないと決めるか。**候補は`esptool`がflash時に報告するchip情報だが、**これがchip品番の確認に足りるかは未検証**であり、実施は実機Linux限定のflash（[#6](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/6)）まで進まない。**この判断は[ESP32 Rust Toolchain](../toolchains/esp32-rust-toolchain.md)を`Verified`にする条件の再定義を伴うため、本行の範囲を超える**（[#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1)では要件の成立しなさを記録するまでとし、再定義は別途扱う） | **module刻印は読了した**（2026-08-13、斜光＋接写）。シールド上面の記載は次のとおりである。`ESPRESSIF` / `ESP32-WROOM-32D` / `FCC ID : 2AC7Z-ESPWROOM32D` / `IC : 21098-ESPWROOM32D` / `CMIIT ID : 2018DP2467` / 技適 `Ⓡ 211-171102` / KC `R-CRM-esp-ESPWROOM32D` / NCC `CCAH18LP020IT7`。**これによりmodule種別の根拠が、購入履歴と基板silkscreenに加えてmodule自身の刻印になった**（反映先は[hardware-bom.md](hardware-bom.md) `MCU-01`）。**ただしこれはmoduleの品番であってchipの品番ではない。**中核chipが`ESP32-D0WD`であるという記載の出典は[ESP-WROOM-32D datasheet v2.7](https://documentation.espressif.com/esp32-wroom-32d_esp32-wroom-32u_datasheet_en.pdf)のままである | [ESP32 Rust Toolchain](../toolchains/esp32-rust-toolchain.md)の状態を`Verified`にする条件。**ただし条件そのものの再定義が要る**（上記） | [#1](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/1) | Human | Open（**範囲を縮小し、要件を書き換えた。**module刻印は完了。chip刻印は非破壊では読めないことが確定した） |
