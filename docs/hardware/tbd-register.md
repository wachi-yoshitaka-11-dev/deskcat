...
| HW-TBD-034 | P1 | **電源過渡の測定方式と、合格主張を張ってよい範囲。**[power-budget.md](power-budget.md#測定計画)の測定系は`ADC-5V`／`ADC-3V3`をESP32自身のADCで読むが、**そのESP32自身の給電が導かれるrailを測っているため、brownout／resetでsamplingが止まる区間を独立に観測できない。**sample間とburst captureのwindow外にも未観測区間がある。**この状態で「起動時の電源過渡をすべて実測した」とは主張できない。****論点の出所は[#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3)の2026-08-16の追記であり、この行は同日に登録した。****同日に人間が方式1（独立した外部観測）を採ると決定した。**oscilloscopeは購入せず、手持ちのArduino Uno R3を候補とする。**技術ガイドのゲートAの改訂は不要になった。****数値の項目ではない**（数値は`HW-TBD-028`。**ここへ再掲しない**） | **人間による方式の決定。**材料と全数の改訂対象は[power-budget.md](power-budget.md#測定計画)の`ESP32自身のADCは測定対象から独立していない`と`電源過渡の測定方式のdesign review`に置いた（**値も規則もここへ再掲しない**）。方式1は独立した外部観測を導入する案で、**必要帯域の根拠は`TBD`である**（一次資料が要る。[AGENTS.md](../../AGENTS.md) 推測禁止）。**手持ちのArduino Unoが候補にあり、成立すれば購入は不要になりうる**（[hardware-bom.md](hardware-bom.md) Revision 47）。**2026-08-16に現物の刻印で`UNO R3`／`ATMEGA328P-PU`／`MEGA16U2`まで確定した**（通電なし）。**同日に`DS40002061B`でADCの分解能・sample rate・入力範囲・基準電圧も確定した**（値の正は[power-budget.md](power-budget.md#測定計画)。**ここへ再掲しない**）。**それでも採否は決まっていない。**基準電圧が未解決で**校正か外部基準が要り**、USB給電での動作とGND topologyへの影響も未確認である。**方式2（ADC loggingへ限定し合格主張を狭める案）は採らないことになった。**ゲートAの合格主張を狭める＝安全側の要求を下げる方向であり、方式1を採ったことで適用先も無くなった。closeには次の全部が要る。**(1) 測定器の採否、(2) 方式1の実装、(3) 基準電圧の校正（または外部基準の採用）、(4) SRAM 2 KBに対する取得方式、(5) GND topologyへの影響の確認、(6) 必要帯域の一次資料、(7) 4文書（技術ガイド、[hardware-safety-policy.md](../governance/hardware-safety-policy.md)、power-budget.md、#3）の合格主張が一致したことの確認。****(1)〜(6)の値と規則の正はpower-budget.mdであり、ここへ再掲しない。****実装と文書整合だけでcloseしない。****技術ガイドは方式1では無変更でよいため、一致の確認は上の方式1の3点が解けた後になる** | **[技術ガイド](../DeskCat_Microcontroller_Development_Guide.md)のゲートA**（起動時の電源過渡の実測）、および`HW-TBD-009`（電源予算とbackfeed review） | [#3](https://github.com/wachi-yoshitaka-11-dev/deskcat/issues/3) | Human | Open（**2026-08-16に方式1で決定済み。closeはしない。**決まったのは方式であり、**測定器の採否も実装も未了である。**方式1の技術的な未解決は3点（基準電圧の校正、SRAM 2 KBに対する取得方式、GND topologyへの影響）で、正は[power-budget.md](power-budget.md#測定計画)の`方式1で残っている未解決3点`である。**この3点はcloseの条件の一部であって全数ではない。**close条件は`必要な根拠`欄が持つ。**いずれもUSBを扱える端末が要り、Docs / Review端末では実施できない**） |
