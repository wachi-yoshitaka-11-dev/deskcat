...
| 段 | 何か | 値の根拠としての扱い |
|---|---|---|
| **甲: メーカーの仕様書** | datasheet、製品ページの仕様表、回路図、公式SDK文書。**発行元が販売代理店を兼ねる会社であっても、その会社の自社製品についての仕様書はここに入る**（例: 秋月電子通商の`AE-`module） | **そのまま一次資料として使える** |
...
**下限が安全要件へ伝わる経路も無い。**Piが落ちてもservoの危険にはならない。
servoの角度・速度・通信断処理はESP32が強制するため、Piの停止はESP32側では通信断として
扱われる（[AGENTS.md](https://github.com/wachi-yoshitaka-11-dev/deskcat/blob/main/AGENTS.md)の
「プロジェクト境界」）。
