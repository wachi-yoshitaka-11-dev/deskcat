...
def _has_json_option(args):
    """`--json`が付いているかを見る。値（fieldの並び）は見ない。

    `-q`／`--jq`は`--json`と併用が前提のoptionであり、`--json`の検出だけで足りる。
    """
    ...
    for index, stage in enumerate(pipeline):
        ...
        if target in LIMIT_COMMANDS:
            ...
            if limit is not None and not _is_large_enough(limit):
                ...
                # `--json`は機械可読の出力であり、件数を数える意図が濃い呼び出しである。
                _ask(
                    f"`{command_text}`に`--json`が付いているが`--limit`が無い。"
                    " 表示されるのは既定の30件だけであり、"
                    " 件数や不在の根拠にする場合は、実際の総数以上の`--limit`を"
                    " 明示する必要がある。単に一部を見るだけの用途であれば"
                    " このまま進めてよい。"
                    f" 誤検知で頻発する場合は{SKIP_ENV}=1で無効化し、理由を残す。"
                )
