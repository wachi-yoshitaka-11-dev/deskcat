...
ハードウェアの値（部品型番、GPIO、電圧、I2C アドレス、bus 速度と mode、
サーボの PWM・可動域・速度・加速度、しきい値、電源・抵抗・コンデンサ値を含む）を、
**AI の記憶で確定しない。**5項目の内外を問わず、AI が思い出した値をそのまま
値として使わない。

**確定する前に、安全要件5項目に効くかどうかを判定する。**5項目に効く値は、
[Hardware Safety Policy](docs/governance/hardware-safety-policy.md)の
「5項目の中での根拠の水準」が要求する根拠を示すまで `TBD` とする。5項目に
効かない値は、一般値（慣行として使われている値。AI の記憶とは出所が別である）
で開始してよい。**この振り分けを飛ばして、値の種類だけで一律に扱わない。**

**判断の正本は [Hardware Safety Policy](docs/governance/hardware-safety-policy.md) である。**
どの値が5項目に効くか、要求する根拠の水準、安全要件の5項目、値、状態は、
