...
| 開始日／終了日（`Start date`／`Target date`） | **必須** | **起票時に両方設定する。**`Start date`は着手予定日、`Target date`は完了目標である。**どちらも予定であり、確定していなくてよい。**確定していないことを理由に空欄にしない。予定が変わったら更新する。**closeした後は、close実施者が`Target date`をJSTのclose実績日へ設定する** |

この表はIssue itemの規約である。**Issue itemとPull Request itemで、2 fieldはどちらも必須である。
違うのは必須かどうかではなく、`Start date`が実績か予定かである。**Pull Request itemの`Start date`は
作成日という**実績**であり、Issue itemの`Start date`は着手予定日という**予定**である。`Target date`は
どちらも予定（Issueは完了目標、Pull Requestはmerge見込み日）であり、mergeまたはcloseの**完了後**に
実績値へ更新する。[Pull request](#pull-request)節を参照する。
...
既実施巡数と継続承認を確認する。不明は0巡ではない。**通算5巡が終わったら、
人間の明示承認なしに6巡目を開始しない（[打ち切り](#打ち切り)）。
差分更新、commit、rebase、外部review対応、セッション・端末の変更でも通算を戻さない。
...
**無効になるのは最終diffへのPassの証拠であり、作業全体の通算巡数ではない。**
1回のまとまった見直しを1巡と数え、別々に起動したreviewerはそれぞれ1巡とする。
1巡内で両Passを行った場合はPassを2つ記録するが、通算を2巡にはしない。
...
**継続承認は有限の範囲で保存する。**同じIssue/PRへ、人間の判断の出所、判断者、対象Issue、
承認時の通算、続行を許す最終巡数、対象の残存欠陥または範囲を書く。
追加巡数を人間が決め、実装では絶対的な終了巡数へ換算する。固定の追加巡数は設けない。
例えば「D1の修正確認のため通算7巡まで」は8巡目や別目的の継続を許さない。
終了巡数に到達した場合や範囲が変わる場合は再承認が要る。PM/AIの判断、無回答、時間経過、
出所の無い「承認済み」は代替にならない。既存の無期限な承認を新たな無期限許可へ転記しない。
...
`capped`の判断理由は人間の判断、または直近2巡が任意の説明改善のみだったことを記録する。
...
開始の予約はreviewer起動前に保存し、通算5巡後の未承認起動を拒否する。
...
通算が不明なら開始せず照合する。記録の削除・作り直しで上限を回避しない。
