...
impl Limiter {
    ...
    fn braking_velocity(&self, position: f32, direction: f32, dt_s: f32) -> f32 {
        ...
        // 境界ちょうどにいる場合に丸めで負になりうる。0 として扱う。
        let distance = distance.max(0.0);

        let unit = self.limits.max_acceleration().approved() * dt_s;
        if unit <= 0.0 {
            // `最大加速度`が0なら減速できない。動かさないことが唯一の安全側である。
            return 0.0;
        }
        let quantum = unit * dt_s;
        let steps = (((1.0 + 8.0 * distance / quantum).sqrt() - 1.0) / 2.0).floor();
        let ramp = steps.max(0.0) * unit;

        // 残り距離が1 step分の減速量に満たないときの詰め。
        //
        // `steps`だけだと、残り `a*dt^2` 未満のところで上限が0になり、**targetへ着かない。**
        // `distance / dt`は`distance < a*dt^2`より`unit`未満なので、この速度からは
        // 次のstepで`最大加速度`の範囲内で0へ落とせる。境界も越えない。
