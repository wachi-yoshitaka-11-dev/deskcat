...
- 親が同一Issueの巡数を予約した1回分だけを実施する。独自の再試行や巡数のリセットをしない。
  通算・承認・停止の正本は[CONTRIBUTING](../../CONTRIBUTING.md#自己レビュー)。
- 指摘は実体ある欠陥、既存の範囲外問題、任意改善を区別し、出所も分ける。
  前巡の説明に由来していても、誤動作や誤判断を招くものは欠陥として報告する。
  任意の説明を増やす提案だけで再巡を要求しない。親が採否・理由と未解決欠陥を保存する。
...
- **`rg --pre`／`--hostname-bin`／`-z`／`--search-zip` は使えない。**前2つは任意の command を起動する。
  ...
  実測はしていない**）。
- **pipe（`|`）は使える。**区間ごとに先頭 program を allowlist で検査する。
  `cat <file> | wc -l`、`head -n 752 <file> | tail -n +735`、`rg -n <pattern> <file> | head -5`
  はいずれも通る。**区間のどれか1つでも allowlist の外なら拒否される**（`| tee out.txt`、`| sh`）。
  **`||` は pipe ではないため拒否される。**前が失敗したときに後ろが走る。
- **`>` `;` `&` `(` `)` `` ` `` `$` `<` `{` `}` `!` は、引用の外にあれば拒否される。**
  リダイレクト（`>`／`>>`）も拒否のままである。
  **クォートすれば pattern の中で使える。**`rg -n 'a|b' <file>`、`rg -oP '(?<!\\)\|' <file>`、
  `grep -n 'a;b' <file>` は通る。
- **ダブルクォートの中の展開は拒否される。**`"$(id)"`／`"${HOME}"`／`"$USER"`／
  ダブルクォートの中のバッククォートである。**クォートしても bash は実行する。**
  展開させたくないなら**シングルクォートで囲む**（`rg -n '$(id)' <file>` は通る）。
- **`|` `;` `&&` `{` `!` などの区切り語そのものは、クォートしても拒否される。**
  `shlex` は引用符を剥いだ後の語を返すため、**bash が引数として渡す語と区別が付かない。**
  例外にすると、その先が無検査になる（`rg <pattern> '{' cat --pre <command> <file>` で
  rg の option 検査が届かない。2026-09-14 に実測）。**語全体を区切り語にしない形で書く。**
